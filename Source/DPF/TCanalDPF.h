...
private:
  ...
  int FTuboCCNodoIzq; // Guarda la posicion en la estructura de TuboExtremo del
                      // tubo para el NodoIzquierdo
  int FTuboCCNodoDer; // Guarda la posicion en la estructura de TuboExtremo del
                      // tubo para el NodoIzquierdo
  ...
  nmTipoTransCal
      FTipoTransCal; // Tipo de transmision de calor PIPA = 3, ESC = 2, ADM = 1
  ...
  double *
      *FUt; // Variacion del vector solucion con respecto al tiempo (solo CE-SE)
  ...
  double **FFraccionMasicaEspecie1; // Fraccion Masica de cada especie por nodo
                                    // en el instante anterior.
  double **FFraccionMasicaCC; // Fraccion Masica de cada especie en el punto de
                              // partida
  ...
  stResMediosTubo
      *ResultadosMedios; // Estructura para resultados medios en tubos
  ...
  double *FLadoCanalTramo; // Para canales cuadrados y triangulares (suponemos
                           // todos los lados iguales)
  double *FLadoMenorCanalTramo; // Longitud del lado menor del canal, para caso
                                // de canal rectangular
  double *FLadoMayorCanalTramo; // Longitud del lado mayor del canal, para caso
                                // de canal rectangular

  double *FLadoCanal; // Para canales cuadrados y triangulares (suponemos todos
                      // los lados iguales)
  double *FLadoCanalD12;
  double *FLadoCanalS12;
  double *FH0Pared;
  double FF; // Constante de la perdida de carga por friccion en canales
             // cuadrados F=28.454
  ...
  double *Fqreg;    // Calor liberado durante la regeneracion en el volumen de
                    // control del nodo i.
  double *Fq_reac1; // Calor liberado durante la combustion de CO en el volumen
                    // de control del nodo i.
  double *Fq_reac2; // Calor liberado durante la combustion de HC en el volumen
                    // de control del nodo i.
  double **FTasaFraccionMasicaEspecie; // Tasa de variacion de la Fraccion
                                       // Masica de cada especie por nodo.
  double **FFraccionMasicaSalida; // Fraccion masica de cada especie a la salida
                                  // de la pared porosa.
  double
      *FRreg1; // Tasa de regeneracion termica en el nodo i del canal de salida
  double *FRreg2; // Tasa de regeneracion catalitica en el nodo i del canal de
                  // salida
  double *FEficiencia; // Eficiencia del medio poroso en el nodo i del canal de
                       // salida
  ...
  void Transforma2Area(double *v, double *a, double *p, double **U, double Area,
                       double Gamma, double Gamma1, double *Yespecie, int i);
  ...
  void Transforma4Area(double **U1, double **Ufctd, double Area, double Gamma,
                       double Gamma1, double Gamma3, double Gamma4,
                       double Gamma6, int i);
  ...
  void CalculaFuente1Area(double **U, double **V1, double *Area, double *Gamma1,
                          int Nodos);
  ...
  void CalculaFuente2(double **U, double **V2, double *diame,
                      ...
                      double *ViscosidadDinamica, double *q_reac1,
                      double *q_reac2, double **FraccionMasicaSalida,
                      double *H0Pared, int Nodos, double *EspesorSoot,
                      double *Eficiencia, double *Rreg1, double *Rreg2,
                      double *SupEspecifica, double *LongitudVC,
                      double **FraccionMasica);

  void CalculaFuente2Area(double **U, double **V2, double *diame,
                          double *CoefTurbulencia, double *hi, double *rho,
                          double *TempParedTubo, double *Gamma, double *Rmezcla,
                          double *Gamma1, double *LadoCanal,
                          double *VelocidadPared, double *ViscosidadDinamica,
                          double *q_reac1, double *q_reac2,
                          double **FraccionMasicaSalida, double *H0Pared,
                          int Nodos, double *Area, double *EspesorSoot,
                          double *Eficiencia, double *Rreg1, double *Rreg2,
                          double *SupEspecifica, double *LongitudVC,
                          double **FraccionMasica);
  ...
  void TransformaContorno(double *L, double *B, double *E, double *a, double *v,
                          double *p, int modo, double Gamma1, double Gamma3,
  ...
  void Calculo_Caracteristica(double *caracteristica, double *velocidadp,
                              double *asonidop, int ind, double dist, int signo,
...
public:
  ...
  void CabeceraResultadosMedios(std::ostream &medoutput,
                                stEspecies *DatosEspecies) const;

  void ImprimeResultadosMedios(std::ostream &medoutput) const;

  void LeeResultadosInstantaneosCanalDPF(int NumResInstantaneos, char *FileWAM,
                                         fpos_t &filepos);

  void CabeceraResultadosInstantaneos(std::ostream &insoutput,
                                      stEspecies *DatosEspecies) const;

  void ImprimeResultadosInstantaneos(std::ostream &insoutput) const;
  ...
  double GetDiametro(int i) { return FDiametroTubo[i]; };
  double GetFraccionMasicaInicial(int i) { return FComposicionInicial[i]; };
  double GetFraccionMasicaCC(int j, int i) { return FFraccionMasicaCC[j][i]; };
  double GetViscosidadDinamica(int i) { return FViscosidadDinamica[i]; };
  double getTime1() { return FTime1; };
  double getNodoIzq() { return FNodoIzq; };
  double getNodoDer() { return FNodoDer; };
  void putTime1(double valor) { FTime1 = valor; };
  double getTime0() { return FTime0; }
  void putTime0(double valor) { FTime0 = valor; };
  double getDeltaTime() { return FDeltaTime; }
  void putDeltaTime(double valor) { FDeltaTime = valor; };
  ...
  double GetDensidad(int i) { return Frho[i]; }
  double GetArea(int i) { return FArea[i]; }
  double GetVelocidad(int i) { return FVelocidad0[i]; };
  double GetAsonido(int i) { return FAsonido0[i]; };
  double GetPresion(int i) { return FPresion0[i]; };
  double GetCoefTurbulencia(int i) { return FCoefTurbulencia[i]; };
  double getTemperaturaInicial() { return FTini; };
  double getPresionInicial() { return FPini; };
  double getVelocidadMedia() { return FVelMedia; };
  double GetYespecie(int j, int i) { return FFraccionMasicaEspecie[j][i]; };
  double GetYespecieCC(int j, int i) { return FFraccionMasicaCC[j][i]; };
  double GetVelocidadCC(int i) { return FVelocidadCC[i]; };
  double GetDensidadCC(int i) { return FDensidadCC[i]; };
  double GetAreaCC(int i) { return FAreaCC[i]; };
  double GetGamma(int i) { return FGamma[i]; };
  double GetRMezcla(int i) { return FRMezcla[i]; };
  double GetCp(int i) { return FCpMezcla[i]; };
  double GetCv(int i) { return FCvMezcla[i]; };
  int getNumeroEspecies() { return FNumeroEspecies; };
  double Geth(int i) { return Fhi[i]; };
  double Gethext(int i) { return Fhe[i]; };
  double GetH0Pared(int i) { return FH0Pared[i]; };

  nmFormulacionLeyes getFormulacionLeyes() { return FMod.FormulacionLeyes; };
  double GetU0(int i, int j) { return FU0[i][j]; };
  ...
  double GetLadoCanal(int i) { return FLadoCanal[i]; };
};
