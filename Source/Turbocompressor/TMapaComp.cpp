...
void TMapaComp::LeeMapa(std::istream &FileInput) {
  double *W = NULL;
  try {
    // Datos de referencia
...
double TMapaComp::EvaluaRCSplines(double Massflow) {
  try {
    ...
    std::cout << "ERROR: EvaluaRCSpline en el compresor: " << FNumeroCompresor
              << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception("ERROR: EvaluaRCSpline en el compresor: " +
                    std::to_string(FNumeroCompresor) + N.what());
  }
}
  ...
  try {
    ...
    if (FRegComp < FRegMin) {
      ...
      for (int j = 0; j < FNumPuntos; ++j) {
        ...
        FRelCompInt[j] =
            FRelComp[FCurvInf][j] +
            (FRelComp[FCurvInf + 1][j] - FRelComp[FCurvInf][j]) * DeltaN;
      }
    }
...
void TMapaComp::ImprimeMapa() {
  // Stubbed out during I/O refactoring.
  std::cout << "Skipped TMapaComp::ImprimeMapa" << std::endl;
}
