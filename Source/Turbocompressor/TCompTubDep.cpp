...
double TCompTubDep::CalGastoNuevo(double MasaAire) {
  ...
  try {
    ...
    FTemperatura20 =
        FTemperatura10 + (pow(FRelacionCompresion, FGamma5 * 2.) - 1.) *
    ...
    FDensidad2 =
        FDensidad20 * pow(FTemperatura2 / FTemperatura20, 1. / FGamma1);
...
double TCompTubDep::RegulaFalsi() {
  ...
  try {
    ...
    if (!valido) {
      ...
      if (i > 100) {
        std::cout << "ERROR: The interation method in the compressor does not "
                     "converge in 100 iterations"
                  << std::endl;
        throw Exception("ERROR: The interation method in the compressor does "
                        "not converge in 100 iterations");
      }
      GastoNuevo = MasaX;
      return GastoNuevo;
    }
...
void TCompTubDep::CalculaCompresor(double Theta) {
  ...
  try {
    ...
    FGasto1 = FGastoCorregido * __units::BarToPa(FPresion10) /
              Mapa->getPresionRef() / sqrt(FTemperatura10 / Mapa->getTempRef());
    FGasto0 = FGasto1;
    ...
    std::cout << "ERROR: TCompTubDep::CalculaCompresor en el compresor: "
              << FNumeroCompresor << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception("ERROR: TCompTubDep::CalculaCompresor en el compresor: " +
                    std::to_string(FNumeroCompresor) + N.what());
  }
}
  ...
  try {
    ...
    if (FGasto1 > 0.) {
      ...
      case nmPlenum:
        ...
        // Cuando la entrada sea la atmasfera, la composicion se mantendra
        // siempre constante.
      }
    }
...
void TCompTubDep::BusquedaEntradaSalida(
    nmCompressorInlet EntradaCompresor, double AmbientTemperature, int numeroCC,
  ...
  try {
    ...
    case nmPipe:
      FTuboRotor =
          dynamic_cast<TCCCompresor *>(BC[numeroCC - 1].get())->getTuboRotor();
      FExtremoTuboRotor = dynamic_cast<TCCCompresor *>(BC[numeroCC - 1].get())
                              ->getExtremoTuboRotor();
      break;
    case nmPlenum:
      FDeposito =
          dynamic_cast<TCCCompresor *>(BC[numeroCC - 1].get())->getPlenum();
  ...
  case nmAtmosphere:
    if (FCalculoEspecies == nmCalculoCompleto) {
      ...
      FCvAtm = CalculoSimpleCvMezcla(FTemperatura10, FFraccionMasicaEspecie[0],
                                     FFraccionMasicaEspecie[1], FCalculoGamma,
                                     nmMEP);
...
void TCompTubDep::DatosEntradaCompresor(double AmbientTemperature,
  ...
  try {
    ...
    double RMezclaEnt = 0., RMezclaSal = 0., GammaEnt = 0., GammaSal = 0.,
           Cp = 0.;

    switch (FEntradaCompresor) {
    case nmAtmosphere:
      // Calculo de Gamma y R en el compresor. Media de valores a la entrada y
      // la salida.
    ...
    case nmPipe:
      ...
      if (FExtremoTuboRotor == nmLeft) {
        ...
        tentcomp =
            pow2(FTuboRotor->GetAsonido(0) * __cons::ARef) / FGamma / FRMezcla;
        Cp = (FTuboRotor->GetGamma(0) * FTuboRotor->GetRMezcla(0)) /
             (FTuboRotor->GetGamma(0) - 1);
      } else {
        pentcomp =
            __units::BarToPa(FTuboRotor->GetPresion(FTuboRotor->getNin() - 1));
        ventcomp =
            FTuboRotor->GetVelocidad(FTuboRotor->getNin() - 1) * __cons::ARef;
      ...
      FPresion10 =
          pentcomp * pow((FTemperatura10 / tentcomp), (FGamma / FGamma1));
      break;
    }
...
double TCompTubDep::NewDampedSolution(double Mass) {
  ...
  while ((delta > 1e-5 || deltaA > 1e-5) && step < 100) {
    ...
    U2 = Mass_filt / (FGamma * __units::BarToPa(pow(A2 / AA2, FGamma4)) *
                      FAreaSalComp / pow2(A2) / __cons::ARef);
    ...
    if (U2 <= 0)
      ...
      AA2 = A20 * AA1 / A10 /
            sqrt((pow2(A20) - pow2(A10)) / pow2(A10) * FRendimiento + 1);
  ...
  FDensidad20 = __units::BarToPa(FPresion20) / FRMezcla / FTemperatura20;
  FDensidad2 = FGamma *
               __units::BarToPa(pow(FASonidoSalida / FEntropia2, FGamma4)) /
               pow2(FASonidoSalida * __cons::ARef);
