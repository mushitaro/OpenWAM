...
TMapaComp2Tub::TMapaComp2Tub(int i) : TCompressorMap() {
  FNumeroCompresor = i;
...
void TMapaComp2Tub::LeeMapa(std::istream &FileInput) {
  double *W = NULL;
  try {
    // Datos de referencia
    ...
    for (int i = 0; i < FNumCurvasReg; i++) {
      ...
      for (int j = 0; j < FNumCurvasRendMax; j++) {
        ...
        if (FGastoRend[i][j] > FGastoRelComp1[i]) {
          std::cout << "WARNING: Existen puntos de rendimiento en la curva de "
                       "regimen: "
                    << FRegimenCurva[i] << std::endl;
          std::cout << "         con valores de massflow mayores del massflow "
                       "de relacion de compresion 1"
                    << std::endl;
          std::cout
          ...
          std::cout << "         Revisa el mapa. Esto puede inducir errores "
                       "importantes de interpolacion"
                    << std::endl;
          std::cout << "         en el compresor numero " << FNumeroCompresor
                    << std::endl
                    << std::endl;
        }
      }
    ...
    // Se desplan todos los valores un lugar para poder incluir el centro de
    // coordenadas massflow=0, relacion de compresion=1
    ...
    // Obtencion de los splines de las curvas de rendimiento frente a massflow
    // adimensionalizado
      ...
      for (int j = 1; j < FNumCurvasRenAd[i] - 1; j++) {
        ...
        if (FGastoAdim[i][j] <= FGastoAdim[i][j - 1]) {
          std::cout
              ...
              << FRegimenCurva[i] << " rpm esta desordenada" << std::endl;
          std::cout << "         Si se continua con este mapa pueden aparecer "
    ...
    if (W != NULL)
      delete[] W;
    throw Exception("ERROR: LeeMapa en el compresor: " +
  ...
  try {
    ...
    for (int i = 1; i < n; ++i) {
      ...
      if (FSpl[i].h <= 0.) {
        ...
        std::cout << "       Los valores en X no estan ordenados o existen 2 "
                     "puntos situados en el mismo X"
                  << std::endl
                  << std::endl;
    ...
    for (int i = 1; i < n - 1; ++i) {
      ...
      if (Espaciado < 0.1 || Espaciado > 10.) {
        std::cout << "WARNING: Deberias utilizar una distribucion mas uniforme "
                     "entre los valores de X"
                  << std::endl;
        std::cout << "         utilizados para ajustar la spline y asi evitar "
                     "problemas"
                  << std::endl
                  << std::endl;
      }
      ...
      FSpl[i].b =
          (FSpl[i + 1].dif / FSpl[i + 1].h - FSpl[i].dif / FSpl[i].h) * 6.;
    }
...
// catch(Exception &N)
//{
// std::cout << "ERROR: Hermite en el compresor: " << FNumeroCompresor <<
// std::endl; std::cout << "Tipo de error: " << N.what() << std::endl; throw
// Exception("ERROR: Spline en el compresor: "
// +std::to_string(FNumeroCompresor)+N.what());
// }
// }
//
// void TMapaComp2Tub::Hermite(int n,std::vector<double> x,std::vector<double>
// y,std::vector<double> *sol)
//{
// try
//{
//
...
// double TMapaComp2Tub::EvaluaHermite(double punto,int n,double *x,double
// *y,double *sol)
//{
// try
//{
...
// catch(Exception &N)
//{
// std::cout << "ERROR: EvaluaHermite en el compresor: " << FNumeroCompresor <<
// std::endl; std::cout << "Tipo de error: " << N.what() << std::endl; throw
// Exception("ERROR: EvaluaHermite en el compresor: "
// +std::to_string(FNumeroCompresor)+N.what());
// }
// }
//
// double TMapaComp2Tub::EvaluaHermite(double punto,int n,std::vector<double>
// x,std::vector<double> y,std::vector<double> sol)
//{
// try
//{
// double ret_val,h00,h10,h01,h11,t2,t3,t,h;
...
double TMapaComp2Tub::EvaluaRCSplines(double Massflow) {
  try {
    ...
    std::cout << "ERROR: EvaluaRCSpline en el compresor: " << FNumeroCompresor
              << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception(
...
double TMapaComp2Tub::EvaluaRCHermite(double Massflow) {
  try {
    double ret_val = 0., t2 = 0., t3 = 0., t = 0., h00 = 0., h10 = 0., h01 = 0.,
           h11 = 0., h = 0.;
...
// 'rtc' a temperatura ambiente 'AmbientTemperature' e interpola los coeficiente
// del     // polinomio ortogonal que ajusta el rendimiento //
//---------------------------------------------------------------------------
...
void TMapaComp2Tub::InterpolaMapa(double rtc, double AmbientTemperature) {
  ...
  try {
    ...
    if (FRegComp >= FRegMax) {
      ...
      FRelCompBombeoX =
          FRelCompBombeo[FCurvInf + 1] +
          (FRelCompBombeo[FCurvInf + 1] - FRelCompBombeo[FCurvInf]) / FIncReg *
              (FRegComp - FRegMax);

      FGastoRelComp1X =
          FGastoRelComp1[FCurvInf] +
          (FGastoRelComp1[FCurvInf] - FGastoRelComp1[FCurvInf - 1]) / FIncReg *
              (FRegComp - FRegMax);

    } else if (FRegComp < FRegMin) {
      FGastoBombeoX =
          FGastoBombeo[FCurvInf + 1] -
          (FGastoBombeo[FCurvInf + 2] - FGastoBombeo[FCurvInf + 1]) / FIncReg *
              (FRegMin - FRegComp);

      FRelCompBombeoX =
          EvaluaHermite(FGastoBombeoX, FNumCurvasReg + 1, FGastoBombeo,
                        FRelCompBombeo, FCoefSplBombeo);

      FGastoRelComp1X =
          FGastoRelComp1[FCurvInf] -
          (FGastoRelComp1[FCurvInf + 1] - FGastoRelComp1[FCurvInf]) / FIncReg *
              (FRegMin - FRegComp);
    } else {
      FGastoBombeoX =
          FGastoBombeo[FCurvInf + 1] +
    ...
    // Se haya el ultimo massflow del que se tiene informacion para el regimen
    // de giro inferior
    ...
    // Se interpola linealmente la relacion de compresion entre el regimen de
    // giro inferior y superior excepto para el ultimo punto.
    ...
    if (FRegComp < FRegMin) {
      for (int j = 0; j < FNumPuntos; ++j) {
        ...
        FRelCompInt[j] =
            ...
            (FRelCompNuevo[FCurvInf + 1][j] - FRelCompNuevo[FCurvInf][j]) /
      ...
      for (int j = 0; j < FNumPuntos; ++j) {
        ...
        if (FRegComp >= FRegMax) {
          FRelCompInt[j] =
              FRelCompNuevo[FCurvInf][j] +
              (FRelCompNuevo[FCurvInf][j] - FRelCompNuevo[FCurvInf - 1][j]) *
                  DeltaN;
        } else {
          FRelCompInt[j] =
              FRelCompNuevo[FCurvInf][j] +
    ...
    // Se obtienen los coeficientes que ajusta la spline de la curva
    // interpolada.

    Hermite(FNumPuntos + 1, FGastoInt, FRelCompInt, FCoefSplRC);

    // Se interpolan linealmente los coeficientes de cada polinomio ortogonal
    // para el regimen de giro buscado
    /*
     FNumTerms=3;
     Cent=2;
    ...
    std::cout << "ERROR: InterpolaMapa en el compresor: " << FNumeroCompresor
              << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception(
...
double TMapaComp2Tub::EvaluaRendimiento(double MasaAire) {
  ...
  try {
    ...
    while (k >= 0) {
      ...
      prev = ret_val;
      ret_val =
          FCoefdX[k] + (MasaAire - FCoefbX[k]) * prev - FCoefcX[k + 1] * prev2;
      --k;
    }
    return ret_val;
  } catch (std::exception &N) {
...
double TMapaComp2Tub::EvaluaRendSplines(double MasaAire) {
  ...
  try {
    ...
    if (MasaAire < FGastoBombeoX) {
      ...
      if (FRegComp > FRegMax) {
        ret_val =
            RendSup + (RendSup - RendInf) / FIncReg * (FRegComp - FRegMax);
      } else if (FRegComp < FRegMin) {
        ret_val =
            RendInf - (RendSup - RendInf) / FIncReg * (FRegMin - FRegComp);
      } else {
      ...
      RendInf =
          EvaluaHermite(AireAdim, FNumCurvasRenAd[index], FGastoAdim[index],
...
double TMapaComp2Tub::GetRelCompInt(int i) {
  try {
    ...
    std::cout << "ERROR: GetRelCompInt en el compresor: " << FNumeroCompresor
              << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception(
  ...
  try {
    ...
    if (sup > 0) {
      ...
      if (Massflow < FGastoRelComp1[inf]) {
        ...
        std::cout << "WARNING: El siguiente punto puede estar mal calculado:"
                  << std::endl;
        std::cout << "         Relacionde compresion: " << RC << std::endl;
        std::cout << "         Massflow masico:          " << Massflow
                  << std::endl;
        std::cout << "         Deberias aumentar el numero de curvas del mapa"
                  << std::endl
                  << std::endl;
        RCInf = 1.;
      }
      ...
      std::cout << "         Massflow masico:          " << Massflow
                << std::endl;
      std::cout << "         Deberias incluir curvas con menor regimen de giro"
                << std::endl
                << std::endl;
    }
    return ret_val;
...
void TMapaComp2Tub::Cambio_Mapa(double radtip, double radhub,
  ...
  try {
    ...
    // los valores de ra,rb y r2, deberan ser medidos directamente en el
    // compresor y ser leidos desde lee_compresor
    ...
    // FGastoMin=-0.07;
    FNumPuntosGastoNuevo =
        floor(((FGastoMax - FGastoMin) / FIncGasto) + 0.5) + 1;
    ...
    for (int i = 0; i < FNumCurvasReg; ++i) {
      ...
      Fnegrc[1] = pow(1 + 1 / (__Gamma::Cp_x2 * (FTempRef)) *
                              pow2(__units::RPMToRad_s(FRegMin + i * FIncReg)) *
                              (pow2(radrodete) - pow2(r1)),
                      __Gamma::G_9);
      Fnegrc[2] = pow(1 + 1 / (__Gamma::Cp_x2 * (FTempRef)) *
                              pow2(__units::RPMToRad_s(FRegMin + i * FIncReg)) *
                              (pow2(radrodete) - pow2(r1)),
                      __Gamma::G_9);
      ...
      for (int j = 0; j < FNumPuntosGastoNuevo; ++j) {
        if (FGastoMin + j * FIncGasto < FGastoBombeo[i]) {
          FRelCompNuevo[i][j] = EvaluaHermite(FGastoMin + j * FIncGasto, 5,
                                              Fnegmas, Fnegrc, Fsolneg);
    ...
    /*fplot=fopen ("datos.m","w");
     fprintf (fplot, "function [a, NumPuntosGasto, IncGasto, GastoMin,
     GastoRelComp1]=datos \na=["); for (int i=0; i<FNumCurvasReg; ++i) { for(int
     j=0; j<FNumPuntosGastoNuevo;++j){ if(i!=FNumCurvasReg ||
     j!=FNumPunyosGastoNuevo) fprintf (fplot,"%lf;\n ",Faux[i][j]); else fprintf
     (fplot,"%lf ];\n",rcom[i]);

     }
     fprintf (fplot,"NumPuntosGasto= %d; \nIncGasto= %lf; \nGastoMin= %lf;
     \nmarc1= [",FNumPuntosGastoNuevo,FIncGasto,FGastoMin); for (i=0;
     i<FNumCurvasReg; i++) fprintf (fplot,"%lf;\n",FGastoRelComp1[i]); fprintf
     (fplot,"];"); fclose (fplot); */
  } catch (std::exception &N) {
...
void TMapaComp2Tub::ImprimeMapa() {
  // Stubbed out during I/O refactoring to remove dependency on FILE* and fopen.
  // This function appeared to be debug output ("MapaInterp.txt").
  std::cout << "ImprimeMapa skipped." << std::endl;
}

//---------------------------------------------------------------------------
//---------------------------------------------------------------------------

void TMapaComp2Tub::ReadGTPowerMap(std::istream &FileInput, int correct) {
  try {
...
void TMapaComp2Tub::RearrangeGTPowerMap(double rtip, double rhub,
  ...
  try {
    ...
    FGastoMin = 0;
    FGastoMax =
        FGTMass[FGTMass.size() - 1][FGTMass[FGTMass.size() - 1].size() - 1];
      ...
      for (int j = 0; j < 21; j++) {
        ...
        if (FlowMass > FGastoBombeo[i]) {
          if (FlowMass < FGastoRelComp1[i]) {
            FRelComp[i][j] =
                EvaluaHermite(FlowMass, FGTMass[i].size(), FGTMass[i],
                              FGTPres[i], FGTCoefCR[i]);
          } else {
            FRelComp[i][j] = 1.;
          }
    ...
    for (int i = 0; i < FNumCurvasReg; i++) {
      if (FGastoRend[i][0] < FGastoBombeo[i + 1] * 1.001 &&
        ...
        std::cout << "INFO: El punto de bombeo de la curva de "
                  << FRegimenCurva[i] << " rpm tiene rendimiento" << std::endl;
      } else {
        FNumCurvasRenAd[i] = FNumCurvasRen[i] + 2;
        RendEnBombeo = false;
        std::cout << "INFO: El punto de bombeo de la curva de "
