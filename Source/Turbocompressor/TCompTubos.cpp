...
void TCompTubos::CondicionCompresor(double Theta, stTuboExtremo *TuboExtremo,
  ...
  double ErrorVelMax = 0., ErrorVelMin = 0., ErrorCheck = 0., RC = 0., RD = 0.,
         CP = 0.;
  double temp = 0.;
  ...
  try {
    ...
    if (FPrimerCiclo) {
      ...
      double vR = FSignoRotor *
                  (TuboExtremo[FPosicionRotor].Landa -
                   TuboExtremo[FPosicionRotor].Beta) /
                  FGamma3 * __cons::ARef;
      double aR = (TuboExtremo[FPosicionRotor].Landa +
                   TuboExtremo[FPosicionRotor].Beta) /
                  2. * __cons::ARef;
      double pR = __units::BarToPa(pow(
          aR / TuboExtremo[FPosicionRotor].Entropia / __cons::ARef, FGamma4));
      // double dR=FGamma*pR/aR/aR;

      double aS = (TuboExtremo[FPosicionEstator].Landa +
                   TuboExtremo[FPosicionEstator].Beta) /
                  2. * __cons::ARef;
      double vS = FSignoStator *
                  (TuboExtremo[FPosicionEstator].Landa -
                   TuboExtremo[FPosicionEstator].Beta) /
                  FGamma3 * __cons::ARef;
      double pS = __units::BarToPa(pow(
          aS / TuboExtremo[FPosicionEstator].Entropia / __cons::ARef, FGamma4));
      ...
      FRelacionCompresion =
          pS * pow(1 + FGamma3 * vS * vS / aS / aS, FGamma / FGamma1) / pR /
          pow(1 + FGamma3 * vR * vR / aR / aR, FGamma / FGamma1);
      FCoefPresiones =
          (pow(FRelacionCompresion, FGamma1 / FGamma) - 1) / FRendimiento;
      ...
      FPrimerCiclo = false;
      FCoefPresiones =
          pow2(*FAaOut / *FAaIn) * pow(FRelacionCompresion, FGamma1 / FGamma) -
          1;
    }
    ...
    if (TuboCalculado == 10000) {
      ...
      if (!FFlujoDirecto) {
        AA1fin = FAsonidoDep / __cons::ARef * pow(FPresionDep, -FGamma5) *
                 pow(FRelacionCompresion, FGamma5) / sqrt(1 + FCoefPresiones);
      } else {
        AA1fin = *FAaIn;
      }
      FFlowIn = FGamma *
                __units::BarToPa(pow(FA1in * __cons::ARef /
                                         pow(AA1fin * __cons::ARef, FGamma),
                                     1 / FGamma3)) *
                FU1in * __cons::ARef * FAreaIn;
    ...
    FPresionIn =
        __units::BarToPa(pow(*FAaIn * __cons::ARef / FAsonidoIn, -FGamma4));
    ...
    FPresionOut =
        __units::BarToPa(pow(*FAaOut * __cons::ARef / FAsonidoOut, -FGamma4));
    FPreTotalOut =
        FPresionOut * pow(FTempTotalOut / FTempOut, FGamma / FGamma1);
    ...
    FRelacionCompresion = FRelacionCompresion +
                          (1 / FLongitudCaract) * (RC - FRelacionCompresion) *
                              0.5 * fabs(FVelocidadIn + FVelocidadOut) *
                              FDeltaTiempo;
...
void TCompTubos::Biseccion(double *VelIn, double *VelOut, double *AIn,
                           double *AOut, double CarIn, double AaIn,
  ...
  try {
    ...
    while (fabs(FErrorVelIn) > FErrorCarIn && FCuentaVelIn < 200) {
      ...
      *AOut = sqrt(FGamma * FRMezcla * FTempOut);
      FErrorVelIn =
          CarOut - (*AOut - (double)sig * *VelOut * FGamma1 / 2) / __cons::ARef;
...
void TCompTubos::RelacionTubos(
    const std::vector<std::unique_ptr<TCondicionContorno>> &BC, int NumeroCC) {
  try {
    double Cp = 0.;
    ...
    if (FTuboRot->getNodoIzq() == NumeroCC) {
      ...
      // FIndExtRotor=1;
      FAreaRotor =
          __geom::Circle_area(FTuboRot->GetDiametro(FTuboRot->getNin() - 1));
      FIndiceCCRotor = 1;
    } else {
    ...
    if (FTuboEst->getNodoIzq() == NumeroCC) {
      ...
      // FIndExtStator=1;
      FAreaStator =
          __geom::Circle_area(FTuboEst->GetDiametro(FTuboEst->getNin() - 1));
      FIndiceCCStator = 1;
    } else {
...
void TCompTubos::MetodoNewton2D(double *a1, double *a2, double *u1, double *u2,
                                double aa1, double aa2, double cc1, double cc2,
                                double s1, double s2, double k, int sig) {
  try {
    double f1 = 0., f2 = 0., J11 = 0., J12 = 0., J22 = 0., J21 = 0., DetJ = 0.,
           da1 = 0., da2 = 0., Error = 0.;
    double a10 = 0., a20 = 0., v10 = 0., v20 = 0., Lim_u1 = 0., Lim_u2 = 0.,
           aa2new = 0.;
    ...
    if (fabs(*u1 / *a1) < 0.7 && fabs(*u2 / *a2) < 0.7) {
      ...
      do {
        ...
        f2 = (*a2 * *a2 + FGamma3 * *u2 * *u2) / aa2 / aa2 -
             (*a1 * *a1 + FGamma3 * *u1 * *u1) / aa1 / aa1 *
                 pow(FRelacionCompresion, sig * FGamma1 / FGamma);
        ...
        J12 = -FGamma * 2 * s2 / FGamma1 *
              ...
              (*u2 / *a1 + (double)sig);
        J21 = -2 * pow(FRelacionCompresion, (sig * FGamma1 / FGamma)) / aa1 /
              aa1 * (*a1 - (double)sig * *u1);
      ...
      if (cont == 5000 || *u1 * *u2 < 0) {
        ...
        FPreTotalIn =
            FPresionIn * pow(FTempTotalIn / FTempIn, FGamma / FGamma1);
        FPreTotalOut = FPreTotalIn * pow(FRelacionCompresion, FSentidoFlujo);
        FPresionOut =
            FPreTotalOut / pow(1 + FGamma3 * pow2(*u2 / *a2), FGamma / FGamma1);
        FTempTotalOut =
...
void TCompTubos::SolveInletBoundary(double &A, double &U, double &Ao,
  ...
  if (A_dep / A_pipe > 1 + 1e-10) {
    ...
    K3 =
        2 * pow2(*FCarCIn) / FGamma1 -
        pow(FPresionDep / FRelacionCompresion, FGamma1 / FGamma) * pow2(*FAaIn);
  } else {
    FFlujoDirecto = true;
...
void TCompTubos::SolveOutletBoundary(double &A, double &U) {
  ...
  if (rel_CCon_Entropia / Ad > 1 + 1e-10) { // Flujo entrante al deposito
    ...
    FFlowOut = -FAreaOut * FGamma *
               pow(Ad * __cons::ARef / FAsonidoDep, FGamma4) * U *
...
void TCompTubos::NewPropertiesInTheVolume() {
  ...
  if (FFlowOut > 0) {
    H += EntalpiaEntrada(FA2, FU2, MasaOut, FAsonidoDep / __cons::ARef,
                         FMasaDep, FGamma);
  }
...
double TCompTubos::EntalpiaEntrada(double ASonidoE, double VelocidadE,
                                   double MasaE, double ASonidoD, double MasaD,
                                   double Gamma) {
...
void TCompTubos::AsignPipes(
  ...
  if (FTuboRot->getNodoIzq() == NumeroCC) {
    ...
    std::cout << "ERROR: Asigncion tubo entrada compresor " << FNumeroCompresor
              << std::endl;
  }
  if (FTuboEst->getNodoIzq() == NumeroCC) {
    FExtremoStator = nmLeft;
    ...
    std::cout << "ERROR: Asignacion tubo salida compresor " << FNumeroCompresor
              << std::endl;
  }
}
