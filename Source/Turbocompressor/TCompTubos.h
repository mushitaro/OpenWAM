...
private:
  ...
  int FPosicionRotor;   // Posicion en el vector estructura TuboExtremo del tubo
                        // del rotor
  int FPosicionEstator; // Posicion en el vector estructura TuboExtremo del tubo
                        // del estator
  int FIndiceCCRotor;   // Posicion del vector para tomar datos del tubo para la
                        // BC (0 Nodo izquierdo; 1 Nodo derecho)
  int FIndiceCCStator;  // Posicion del vector para tomar datos del tubo para la
                        // BC (0 Nodo izquierdo; 1 Nodo derecho)
  ...
  double
      FErrorCarOut; // Error limite de la caracteristica a la salida (err_out)
  ...
  double
      FTempTotalOutAnt; // Temperature total a la salida anterior (tt_out_ant)
  double FTempTotalIn;
  ...
  bool FPrimerCiclo;
  bool FCheckSentido;
  int FContadorCheckSentido; // Cuenta el numero de cambios de sentido en el
                             // mismo instante.
  ...
  void Solver(double *a1, double *a2, double *u1, double *u2, double aa1,
              double aa2, double cc1, double cc2, double s1, double s2,
              double k, int sig);
...
public:
  ...
  void AsignPipes(const std::vector<std::unique_ptr<TCondicionContorno>> &BC,
                  int NumeroCC);
