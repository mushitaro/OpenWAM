...
void TCompresorDep::CalculaGasto(double TrabajoInsTurbina,
  ...
  try {
    ...
    FGamma = FDepositoRot
                 ->getGamma(); // Se ha de hacer una media de entrada y salida
      ...
      if (NumGastos == 0) {
        ...
        for (int i = 0; i < NumGastos; i++) {
          ...
          Trab[i] = Gastos[i] * DescorrigeGasto * FRMezcla * FGamma4 / 2. *
                    FTemperatura10 * (pow(RelComp[i], FGamma5 * 2.) - 1.) /
                    Rnd[i];
        }
        if (NumGastos > 1) {
          ...
          FTempGasto =
              FTemperatura10 + (pow(RelComp[PuntoMin], FGamma5 * 2) - 1) *
    ...
    std::cout << "ERROR: TCompresorDep::CalculaGasto en el compresor: "
              << FNumeroCompresor << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception("ERROR: CalculaGasto en el compresor: " +
                    std::to_string(FNumeroCompresor) + N.what());
  }
}
