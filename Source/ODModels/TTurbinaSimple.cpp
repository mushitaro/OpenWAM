...
void TTurbinaSimple::AsignaEntradaSalidaCC() {
  try {
    ...
    std::cout << "ERROR: TTurbinaSimple::AsignaEntradaSalidaCC en la turbina "
...
void TTurbinaSimple::CalculaCondicionTurbina(double TimeCalculo) {
  ...
  try {
    ...
    if (DeltaT > 0) {
      ...
      FRhoSalida =
          __units::BarToPa(FPresionSalida) / FTempSalida / FCCSalida[0]->getR();
      ...
      if (dynamic_cast<TCCDeposito *>(FCCEntrada[0])->getSentidoFlujo() ==
          nmEntrante) {
        SentidoEntrada = 1;
      } else if (dynamic_cast<TCCDeposito *>(FCCEntrada[0])
      ...
      FVelocidadEntrada[0] =
          FVelocidadEntrada[0] * __cons::ARef * SentidoEntrada;
      ...
      // ------------------------------------------------------------------------------
      // Calculo de la temperatura isentropica del gas a la salida de la
      // turbina, a partir del transporte de especies.
      // ------------------------------------------------------------------------------
      ...
      FTemp0Entrada[0] =
          FTempEntrada[0] + pow2(FVelocidadEntrada[0]) / (2. * cpte);
      FPresion0Entrada[0] =
          FPresionEntrada[0] *
      ...
      if (fabs(FGastoEntrada[0]) > 1e-10) {
        ...
        if (FTempEntrada[0] < FHTM->NodeTemperature(10)) {
          ...
          FTempEntrada[0] = FTempEntrada[0] -
                            FHTM->Turb_Heat_Flow() / cpte / FGastoEntrada[0];
      ...
      FVelocidadEntrada[0] =
          FGastoEntrada[0] * FRMezcla * FTempEntrada[0] /
          (__units::BarToPa(FPresionEntrada[0]) * FAcTurb->SIn());
      ...
      FGastoCorregido[0] =
          FGastoEntrada[0] * sqrt(FTemp0Entrada[0]) / FPresion0Entrada[0] / 0.1;
      FRegimenCorregido[0] = FRegimen / sqrt(FTemp0Entrada[0]);

      if (FCalculoEspecies == nmCalculoCompleto) {
        cp_med = CpTurbineComplete(
      ...
      TempIsentSal =
          FTemp0Entrada[0] * pow(FPresionSalida / FPresion0Entrada[0],
                                 ((gam_med - 1.0) / gam_med));
      ...
      if (FTipoTurbina == nmTurbineMap) {
        ...
        FMapa->CurrentEffectiveSection(
            FRegimenCorregido[0] / 60., FRelacionExpansion[0], FRack,
      ...
      if ((FGastoEntrada[0] > 0.) &&
        ...
        if ((FEntalpia0Entrada[0] - FEntalpiaIsenSalida[0]) > 0.) {
          FRelacionCinematica[0] =
              __units::RPMToRPS(FRegimen) * __cons::Pi * FDiametroRodete /
        ...
        if (FCalRendTurbina == nmWatson) {
          ...
          if (FRelacionCinematica[0] >= FRcmaxima ||
            ...
            b = FRendmaximo *
                ...
                dd;
            c = FRendmaximo *
                (pow3(FRcmaxima) - 3. * FRcmaxima * pow2(FRcoptima)) / dd;
            d = FRendmaximo * (2. * FRcmaxima * FRcoptima - pow2(FRcmaxima)) /
                dd;
      ...
      FTrabajoTotal +=
          FTrabajoIsen; // Lo usa para calcular el rendimiento medio
...
void TTurbinaSimple::ImprimeResultadosMediosPantalla() {
  try {
    ...
    std::cout << "ERROR: TTurbinaSimple::ImprimeResultadosMediosPantalla en la "
                 "turbina: "
              << FNumeroTurbina << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception(N.what());
...
void TTurbinaSimple::ReadAverageResultsTurb(std::istream &FileInput) {
  try {
    ...
    std::cout << "ERROR: TTurbinaSimple::ReadAverageResultsTurb en la turbina "
              << FNumeroTurbina << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception(N.what());
  }
...
void TTurbinaSimple::CabeceraResultadosMedTurb(std::ostream &medoutput) {
  try {
    std::string Label;

    if (FResMediosTurbina.Trabajo) {
      Label =
          "\t" + PutLabel(511) + std::to_string(FNumeroTurbina) + PutLabel(907);
      medoutput << Label.c_str();
    }
    if (FResMediosTurbina.Rendimiento) {
      Label =
          "\t" + PutLabel(512) + std::to_string(FNumeroTurbina) + PutLabel(901);
      medoutput << Label.c_str();
    }
...
void TTurbinaSimple::AcumulaMedias(double Tiempo) {
  ...
  try {
    ...
    std::cout << "ERROR: TTurbinaSimple::AcumulaMedias en la turbina: "
              << FNumeroTurbina << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception("ERROR: TTurbinaSimple::AcumulaMedias en la Turbine: " +
                    std::to_string(FNumeroTurbina) + N.what());
  }
}
...
void TTurbinaSimple::ImprimeResultadosMedTurb(std::ostream &medoutput) {
  try {
    //if (FResMediosTurbina.Trabajo)
...
void TTurbinaSimple::LeeResultadosInstantTurb(std::istream &FileInput) {
  int nvars = 0, var = 0;
...
void TTurbinaSimple::CabeceraResultadosInstantTurb(std::ostream &insoutput) {
  try {
    std::string Label;

    if (FResInstantTurbina.Potencia) {
      Label =
          "\t" + PutLabel(519) + std::to_string(FNumeroTurbina) + PutLabel(903);
...
void TTurbinaSimple::ResultadosInstantTurb() {
  try {
    ...
    std::cout << "ERROR: TTurbinaSimple::ResultadosInstantTurb en la turbina "
              << FNumeroTurbina << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception(N.what());
  }
...
void TTurbinaSimple::ImprimeResultadosInstantTurb(std::ostream &insoutput) {
  try {
    //if (FResInstantTurbina.Potencia)
