...
TDepVolVariable::TDepVolVariable(int i, int ncv,
                                 ...
                                 bool ThereIsEGR)
    : TDeposito(i, nmDepVolVble, SpeciesModel, numeroespecies, GammaCalculation,
                ThereIsEGR) {
...
void TDepVolVariable::ActualizaPropiedades(double TimeCalculo) {
  ...
  try {
    ...
    FVolumen0 = FVolumen;
    FVolumen =
        CalculaVolumen(FAngulo, FCarrera, FLBiela, FDiametro, FVolumenMuerto);
    ...
    for (int i = 0; i < FNumeroUnionesED; i++) {
      ...
      if (FNumeroDeposito ==
        ...
        printf("ERROR:TDepVolVariable::ActualizaPropiedades en el deposito %d, "
               "union entre depositos %d\n",
               FNumeroDeposito, i);
    ...
    FPressure = __units::PaToBar(pow2(__cons::ARef * FAsonido) * FMasa /
                                 (FGamma * FVolumen));
