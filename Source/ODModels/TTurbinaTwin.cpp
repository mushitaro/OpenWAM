  ...
  try {
    ...
    if (DeltaT > 0) {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        TransformaContorno(&Landa, &Beta, &Entropia, &FAsonidoEntrada[i],
                           &FVelocidadEntrada[i], &FPresionEntrada[i], 1,
        ...
        FRhoEntrada[i] = __units::BarToPa(FPresionEntrada[i]) /
                         FTempEntrada[i] / FCCEntrada[i]->getR();
        FGastoEntrada[i] =
      ...
      for (int i = 0; i < 2; i++) {
        ...
        FTemp0Entrada[i] =
            FTempEntrada[i] + pow2(FVelocidadEntrada[i]) / (2. * cpte[i]);
        ...
        if (FCalculoEspecies == nmCalculoCompleto) {
          cp_med[i] = CpTurbineComplete(
              FFraccionMasicaEspecie[0], FFraccionMasicaEspecie[1],
              FFraccionMasicaEspecie[2], (FTempEntrada[i] + FTempSalida) / 2.);
        } else {
          cp_med[i] = CpTurbineSimple((FTempEntrada[i] + FTempSalida) / 2.,
                                      FFraccionMasicaEspecie[0]);
        }
        TempIsentSal[i] =
            FTemp0Entrada[i] *
            pow(FPresionSalida / FPresion0Entrada[i], (FRMezcla / cp_med[i]));
        ...
        if (FCalculoEspecies == nmCalculoCompleto) {
          ...
          cp_med[i] = CpTurbineSimple((FTempEntrada[i] + TempIsentSal[i]) / 2.,
                                      FFraccionMasicaEspecie[0]);
        }
      ...
      if (FTipoTurbina == nmTurbineMap) {
        ...
        for (int i = 0; i < 2; i++) {
          ...
          FMapa->CurrentEffectiveSection(
              FRegimenCorregido[i] / 60., FRelacionExpansion[i], FRack,
      ...
      for (int i = 0; i < 2; i++) {
        FTrabajoIsen = FGastoEntrada[i] *
                       (FEntalpia0Entrada[i] - FEntalpiaIsenSalida[i]) * DeltaT;
        FTrabajoIsenInstTotal += FTrabajoIsen;
        FTrabajoFluido +=
            FTrabajoIsen * FRendTurbina[i] -
