...
void TDeposito::LeeDatosGeneralesDepositos(const char *FileWAM,
  ...
  try {
    ...
    if (fracciontotal > 1 + 1.e-10 && fracciontotal < 1 - 1e-10) {
      std::cout << "ERROR: Total mass fraction cannot be different than 1. "
                   "Check input data in plenum  "
  ...
  try {
    ...
    for (int i = 0; i < numCC; i++) {
      if (BC[i]->getTipoCC() == nmPipeToPlenumConnection) {
        ...
        if ((dynamic_cast<TCCCompresor *>(BC[i])->getCompressor())
    ...
    /* Comprobacion que los depositos especiales tienen el numero de entradas
    ...
    if (FTipoDeposito == nmTurbinaSimple) {
      ...
      if (FNumeroUniones != 3) {
        std::cout << "ERROR: A venturi must have 3 connections. Plenum: "
                  << FNumeroDeposito << std::endl;
        std::cout << "Total number of connections: " << FNumeroUniones
                  << std::endl;
        throw Exception("");
    ...
    for (int j = 0; j < FNumeroUniones; j++) {
      ...
      for (int i = 0; i < numCC; i++) {
        ...
        if (BC[i]->getTipoCC() == nmCompresor) {
          if (dynamic_cast<TCCCompresor *>(BC[i])
                  ->getCompressor()
                  ->getModeloCompresor() == nmCompOriginal) {
            if (FNumeroDeposito ==
                    dynamic_cast<TCCCompresor *>(BC[i])->getNumeroDeposito() &&
    ...
    if ((FTipoDeposito == nmTurbinaSimple || FTipoDeposito == nmTurbinaTwin) &
        FNumeroUnionesED != 0) {
      std::cout << "ERROR: Una turbina no puede estar unida directamente a un "
                   "deposito. Deposito:"
                << std::endl;
      std::cout << "       Revisa la entrada de datos el apartado de uniones "
                   "entre depositos"
                << std::endl;
      throw Exception("");
    } else if (FTipoDeposito == nmVenturi & FNumeroUnionesED != 0) {
      std::cout << "ERROR: Un venturi no puede estar unido directamente a un "
                   "deposito. Deposito:"
                << std::endl;
      std::cout << "       Revisa la entrada de datos el apartado de uniones "
                   "entre depositos"
      ...
      std::cout << "ERROR: Una union direccional no puede estar unido "
                   "directamente a un deposito"
                << std::endl;
      std::cout << "       Revisa la entrada de datos el apartado de uniones "
                   "entre depositos"
                << std::endl;
...
double TDeposito::EntalpiaEntrada(double ASonidoE, double VelocidadE,
  ...
  try {
    ...
    if (fabs(MasaE) != 0.) {
      xx =
          (ASonidoE * ASonidoE / ASonidoD / ASonidoD - 1.) / __Gamma::G1(Gamma);
...
void TDeposito::AsignaCompresor(TCompresor *Compressor, int sentido) {
  try {
    if (FHayCompresor) {
      std::cout << "ERROR: Each plenum only can have one compressor connected"
                << std::endl;
      std::cout << "       Check the input data" << std::endl;
      throw Exception("");
    } else {
      if (FTipoDeposito != nmDepVolCte) {
        std::cout << "ERROR: Compressor only can be connected to constant "
                     "volume plenums"
                  << std::endl;
        std::cout << "       Check the input data" << std::endl;
        throw Exception("");
...
void TDeposito::CreaUnionED(int numero, int sentido) {
  try {
    if (FTipoDeposito == nmTurbinaSimple || FTipoDeposito == nmTurbinaTwin) {
      std::cout << "ERROR: Turbines can not be connected to plenums directly"
                << std::endl;
      std::cout << "       Check the input data for connections between plenums"
                << std::endl;
      throw Exception("");
    } else if (FTipoDeposito == nmVenturi) {
      std::cout << "ERROR: Venturis can not be connected to plenums directly"
                << std::endl;
      std::cout << "       Check the input data for connections between plenums"
                << std::endl;
      throw Exception("");
    } else if (FTipoDeposito == nmUnionDireccional) {
      std::cout << "ERROR: Directional junctions can not be connected to "
                   "plenums directly"
                << std::endl;
      std::cout << "       Check the input data for connections between plenums"
                << std::endl;
  ...
  try {
    if (!asgFNumUniones) {
      ...
      if (FUnion == NULL) {
        ...
        std::cout << "ERROR: The connections vector is already allocated"
                  << std::endl;
        throw Exception("");
      }
      if (FNumConductos == NULL) {
      ...
      if (FNumNodos == NULL) {
        ...
        std::cout << "ERROR: The nodes number vector is already allocated"
                  << std::endl;
        throw Exception("");
      }
      if (FSentidoFlujo == NULL) {
  ...
  try {
    if (!asgFNumConductos) {
      if (indice < FNumeroUniones) {
        ...
        std::cout << "ERROR: Se intentan definir mas nodos de los que se han "
                     "declarado"
                  << std::endl;
        throw Exception("");
      }
  ...
  try {
    if (!asgFNumNodos) {
      if (indice < FNumeroUniones) {
        ...
        std::cout << "ERROR: The number of connection to the plenum is lower"
                  << std::endl;
        throw Exception("");
      }
      asgFNumNodos = true;
...
void TDeposito::AcumulaResultadosMedios(double Actual) {
  try {
    ...
    if (FResMediosDep.Temperature) {
      FResMediosDep.TemperaturaSUM +=
          __units::KTodegC(pow2(FAsonido * __cons::ARef) /
                           (FGamma * FRMezcla)) *
          Delta;
    }
    ...
    if (FNumResMed > 0) {
      if (FResMediosDep.Pressure) {
        FResMediosDep.PresionMED =
            FResMediosDep.PresionSUM / FResMediosDep.TiempoSUM;
        FResMediosDep.PresionSUM = 0.;
      }
      if (FResMediosDep.Temperature) {
        FResMediosDep.TemperaturaMED =
            FResMediosDep.TemperaturaSUM / FResMediosDep.TiempoSUM;
        FResMediosDep.TemperaturaSUM = 0.;
      }
      if (FResMediosDep.FraccionMasicaEspecies) {
        for (int i = 0; i < FNumeroEspecies - FIntEGR; i++) {
          FResMediosDep.FraccionMED[i] =
              FResMediosDep.FraccionSUM[i] / FResMediosDep.TiempoSUM;
          FResMediosDep.FraccionSUM[i] = 0.;
        }
      }
      FResMediosDep.TiempoSUM = 0;
  ...
  try {
      ...
      if (FCCDeposito[i]->getTipoCC() == nmPipeToPlenumConnection) {
        ...
        if (!(FCCDeposito[i]->getUnionDPF())) {
          ...
          MasaFinal +=
              ...
              FCCDeposito[i]
                  ->GetTuboExtremo(0)
                  .DPF->GetCanal(FCCDeposito[i]->GetTuboExtremo(0).NumeroHaz, 0)
                  ->getNumeroCanales();
#endif
        }
      }
    ...
    for (int i = 1; i < FNumeroUnionesED; i++) {
      if (FCCUnionEntreDep[i]->getTipoCC() == nmUnionEntreDepositos) {
        if (FNumeroDeposito ==
            dynamic_cast<TCCUnionEntreDepositos *>(FCCUnionEntreDep[i])
                ->getNumeroDeposito1()) {
          SignoFlujoED =
              dynamic_cast<TCCUnionEntreDepositos *>(FCCUnionEntreDep[i])
                  ->getSentidoFlujoED1();
...
void TDeposito::SalidaGeneralDep(stEspecies *DatosEspecies) {
  try {
    ...
    if (FNumResMed > 0) {
      ...
      if (FResMediosDep.Pressure)
        std::cout << "  Pressure:     " << FResMediosDep.PresionMED << " bares"
                  << std::endl;
      if (FResMediosDep.Temperature)
