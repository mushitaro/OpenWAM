  ...
  try {
    ...
    if (!(DoubEqZero(DeltaT))) {
      ...
      while (!Converge) {
        ...
        Energia = pow(FMasa / FMasa0 *
                          exp((H + H0) / 2 - (FTrabajoFluido + Heat) * MTemp),
                      __Gamma::G1(FGamma));
      ...
      FAsonido = Ason1;
      // FTemperature = pow(FAsonido, 2) / (FGamma * FRMezcla) *
      // pow(__cons::ARef, 2);
...
void TTurbina::CalculoPotenciaPaso() {
  try {
    ...
    std::cout << "ERROR: TTurbina::CalculoPotenciaPaso en el compresor: "
              << FNumeroTurbina << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception("ERROR: TTurbina::CalculoPotenciaPaso en la turbina: " +
  ...
  try {
    ...
    if (FTipoTurbina == nmTurbineMap) {
      ...
      case 2:
        ...
        break;
      default:
        std::cout << "ERROR: Unknown method to calculate turbine efficiency "
                  << std::endl;
      }
    }
...
// }
// catch(Exception & N) {
// std::cout << "ERROR: TTurbina::PutNumeroTurbina en la turbina " <<
// FNumeroTurbina << std::endl; std::cout << "Tipo de error: " << N.what() <<
...
// }
// catch(Exception & N) {
// std::cout << "ERROR: TTurbina::PutRegimen en la turbina " << FNumeroTurbina
...
double TTurbina::GetRelacionCinematica(int i) {
  try {
    if (FRelacionCinematica != NULL && i < FNumeroEntradas) {
      ...
      std::cout << "ERROR: TTurbina::GetRelacionCinematica: " << FNumeroTurbina
                << std::endl;
...
void TTurbina::AllocateDatosTGV(stDatosTGV *DatosTGV) { FDatosTGV = DatosTGV; }

double TTurbina::CpTurbineSimple(double Temperature, double YBurnt) {

  double R = __R::Burnt * YBurnt +
             (__R::Air * (1 - YBurnt - 0.0164) + 0.0164 * __R::H2O);
  ...
  double CvH2O =
      (22.605 - 0.09067 * RaizdeT +
       (-826.53 * RaizdeT + 13970.1 - 82114 / RaizdeT) / Temperature) *
          __R::H2O -
      __R::H2O;

  double CpMezcla =
      CvBurnt * YBurnt + (CvAir * (1 - YBurnt - 0.0164) + 0.0164 * CvH2O) + R;
...
void TTurbina::AsignAcousticElements(
  ...
  if (FIsAcoustic) {
    ...
    for (int i = 0; i < FNumeroEntradas; i++) {
      FAcTurb->AsignInPipe(RawPipe.data(), i);
    }

    FAcTurb->AsignOutPipe(RawPipe.data());
  }
}
