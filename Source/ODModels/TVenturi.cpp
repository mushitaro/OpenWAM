  ...
  try {
    ...
    if (FCalculoEspecies == nmCalculoCompleto) {
      ...
      FCpMezcla = CalculoCompletoCpMezcla(
          FFraccionMasicaEspecie[0], FFraccionMasicaEspecie[1],
          FFraccionMasicaEspecie[2], 0, __units::degCToK(FTemperature),
          FCalculoGamma, nmMEP);
      ...
      FRMezcla =
          CalculoSimpleRMezcla(FFraccionMasicaEspecie[0],
                               FFraccionMasicaEspecie[1], FCalculoGamma, nmMEP);
      FCvMezcla = CalculoSimpleCvMezcla(
          __units::degCToK(FTemperature), FFraccionMasicaEspecie[0],
          FFraccionMasicaEspecie[1], FCalculoGamma, nmMEP);
    ...
    FPressure = __units::PaToBar(pow2(__cons::ARef * FAsonido) / FGamma /
                                 FVolumen * FMasa);
...
void TVenturi::CalculaVenturi() {
  ...
  // El venturi se considera simetrico, es decir, el fluido puede entrar tanto
  // por el ramal de entrada como por el ramal de salida.
  ...
  try {
    ...
    Velocity =
        SentidoEntrada * dynamic_cast<TCCDeposito *>(FCCEntrada)->getVelocity();
    Speedsound = dynamic_cast<TCCDeposito *>(FCCEntrada)->getSpeedSound();

    if (Velocity > 0.0) {
      if (Speedsound > 0)
        dynamic_cast<TCCDeposito *>(FCCEntrada)
            ->putMachVenturi(Velocity / Speedsound);
      dynamic_cast<TCCDeposito *>(FCCSalida)->putMachVenturi(0.);
      // if(Speedsound > 0) dynamic_cast<TCCDeposito
      // *>(FCCEntrada)->getValvula()->getCRecuperacion() = Velocity /
      // Speedsound; dynamic_cast<TCCDeposito
      // *>(FCCSalida)->getValvula()->getCRecuperacion() = 0.;
    } else {
      ...
      Velocity =
          SentidoSalida * dynamic_cast<TCCDeposito *>(FCCSalida)->getVelocity();
      Speedsound = dynamic_cast<TCCDeposito *>(FCCSalida)->getSpeedSound();
      dynamic_cast<TCCDeposito *>(FCCEntrada)->putMachVenturi(0.);
      if (Speedsound > 0)
        dynamic_cast<TCCDeposito *>(FCCSalida)->putMachVenturi(Velocity /
                                                               Speedsound);
      // dynamic_cast<TCCDeposito
      // *>(FCCEntrada)->getValvula()->getCRecuperacion() = 0.; if(Speedsound >
      // 0) dynamic_cast<TCCDeposito
      // *>(FCCSalida)->getValvula()->getCRecuperacion() = Velocity /
      // Speedsound;
    }
    ...
    if (Velocity > 0. && FRelacionSecciones > 0.) {
      while (Converge < 0.99999 || Converge > 1.00001) {
        if (Mach1 > 0.99999999) {
          ...
          VelGarganta1 = sqrt(((1. + FGamma1 / 2. * pow2(Mach0)) /
                               (FGamma2 * pow2(Mach0) / 2.) * pow2(Velocity)));
        } else {
          VelGarganta1 = FRelacionSecciones * Velocity *
                         pow(TempEntrada / TempGarganta, FGamma6);
        }

        TempGarganta = TempEntrada - (pow2(VelGarganta1 * __cons::ARef) -
                                      pow2(Velocity * __cons::ARef)) /
                                         (2. * FCpMezcla);
      ...
      VelGarganta0 = sqrt(
          (FRendimientoVenturi * pow2(VelGarganta0) - FPerdidasCalor * 2.));
      // dynamic_cast<TCCDeposito
      // *>(FCCLateral)->getValvula()->getCRecuperacion() = (VelGarganta0  *
      // __cons::ARef)/pow(FGamma*FRMezcla*TempGarganta,0.5);
      dynamic_cast<TCCDeposito *>(FCCLateral)
          ->putMachVenturi((VelGarganta0 * __cons::ARef) /
                           pow(FGamma * FRMezcla * TempGarganta, 0.5));
    } else {
      // dynamic_cast<TCCDeposito
      // *>(FCCLateral)->getValvula()->getCRecuperacion() = 0.;
...
void TVenturi::CabeceraResultadosInstantVenturi(std::ostream &insoutput) {
  try {
    ...
    if (FResInstantVenturi.PresionGarganta) {
      Label =
          "\t" + PutLabel(522) + std::to_string(FNumeroVenturi) + PutLabel(908);
      insoutput << Label.c_str();
    }
    if (FResInstantVenturi.MachEntrada) {
      Label =
          "\t" + PutLabel(523) + std::to_string(FNumeroVenturi) + PutLabel(901);
      insoutput << Label.c_str();
    }
    if (FResInstantVenturi.MachGarganta) {
      Label =
          "\t" + PutLabel(524) + std::to_string(FNumeroVenturi) + PutLabel(901);
      insoutput << Label.c_str();
    }
    if (FResInstantVenturi.VelEntrada) {
      Label =
          "\t" + PutLabel(525) + std::to_string(FNumeroVenturi) + PutLabel(909);
      insoutput << Label.c_str();
    }
    if (FResInstantVenturi.VelLateral) {
      Label =
          "\t" + PutLabel(526) + std::to_string(FNumeroVenturi) + PutLabel(909);
      insoutput << Label.c_str();
    }
    if (FResInstantVenturi.GastoEntrada) {
      Label =
          "\t" + PutLabel(527) + std::to_string(FNumeroVenturi) + PutLabel(904);
      insoutput << Label.c_str();
    }
    if (FResInstantVenturi.GastoLateral) {
      Label =
          "\t" + PutLabel(528) + std::to_string(FNumeroVenturi) + PutLabel(904);
      insoutput << Label.c_str();
    }
  ...
  try {
    ...
    if (FResInstantVenturi.PresionGarganta) {
      FResInstantVenturi.PresionGargantaINS =
          FPressure / pow(1 + FGamma1 / 2 *
                                  pow2(dynamic_cast<TCCDeposito *>(FCCLateral)
                                           ->getMachVenturi()),
                          FGamma / FGamma1);
    }
    if (FResInstantVenturi.MachEntrada) {
      if (dynamic_cast<TCCDeposito *>(FCCEntrada)->getMachVenturi() != 0) {
        FResInstantVenturi.MachEntradaINS =
            fabs(dynamic_cast<TCCDeposito *>(FCCEntrada)->getMachVenturi());
      } else {
        FResInstantVenturi.MachEntradaINS =
            -fabs(dynamic_cast<TCCDeposito *>(FCCSalida)->getMachVenturi());
      }
    }
    if (FResInstantVenturi.MachGarganta) {
      FResInstantVenturi.MachGargantaINS =
          fabs(dynamic_cast<TCCDeposito *>(FCCLateral)->getMachVenturi());
    }
    if (FResInstantVenturi.VelEntrada) {
      if (dynamic_cast<TCCDeposito *>(FCCEntrada)->getMachVenturi() != 0) {
        if (dynamic_cast<TCCDeposito *>(FCCEntrada)->getSentidoFlujo() ==
  ...
  try {
    ...
    if (FResMediosVenturi.PresionGarganta) {
      FResMediosVenturi.PresionGargantaSUM +=
          (FPressure / pow(1 + FGamma1 / 2 *
                                   pow(dynamic_cast<TCCDeposito *>(FCCLateral)
                                           ->getMachVenturi(),
                                       2.),
                           FGamma / FGamma1)) *
          Delta;
    ...
    if (FResMediosVenturi.MachEntrada) {
      FResMediosVenturi.MachEntradaSUM +=
          fabs(dynamic_cast<TCCDeposito *>(FCCEntrada)->getMachVenturi()) *
          Delta;
    }
    if (FResMediosVenturi.MachGarganta) {
    ...
    if (FResMediosVenturi.VelEntrada) {
      ...
      FResMediosVenturi.VelEntradaSUM +=
          SentidoEntrada * __cons::ARef * Delta *
          dynamic_cast<TCCDeposito *>(FCCEntrada)->getVelocity();
    }
    if (FResMediosVenturi.VelLateral) {
      if (dynamic_cast<TCCDeposito *>(FCCLateral)->getSentidoFlujo() ==
      ...
      FResMediosVenturi.VelLateralSUM +=
          SentidoLateral * __cons::ARef * Delta *
          dynamic_cast<TCCDeposito *>(FCCLateral)->getVelocity();
    }
    if (FResMediosVenturi.GastoEntrada) {
      FResMediosVenturi.GastoEntradaSUM +=
          -Delta * dynamic_cast<TCCDeposito *>(FCCEntrada)->getMassflow();
    }
    if (FResMediosVenturi.GastoLateral) {
      FResMediosVenturi.GastoLateralSUM +=
          -Delta * dynamic_cast<TCCDeposito *>(FCCLateral)->getMassflow();
    }
  ...
  try {
    if (FResMediosVenturi.PresionEntrada) {
      FResMediosVenturi.PresionEntradaMED =
          FResMediosVenturi.PresionEntradaSUM / FResMediosVenturi.TiempoSUM;
      FResMediosVenturi.PresionEntradaSUM = 0.;
    }
    if (FResMediosVenturi.PresionGarganta) {
      FResMediosVenturi.PresionGargantaMED =
          FResMediosVenturi.PresionGargantaSUM / FResMediosVenturi.TiempoSUM;
      FResMediosVenturi.PresionGargantaSUM = 0.;
    }
    if (FResMediosVenturi.MachEntrada) {
      FResMediosVenturi.MachEntradaMED =
          FResMediosVenturi.MachEntradaSUM / FResMediosVenturi.TiempoSUM;
      FResMediosVenturi.MachEntradaSUM = 0.;
    }
    if (FResMediosVenturi.MachGarganta) {
      FResMediosVenturi.MachGargantaMED =
          FResMediosVenturi.MachGargantaSUM / FResMediosVenturi.TiempoSUM;
      FResMediosVenturi.MachGargantaSUM = 0.;
    }
    if (FResMediosVenturi.VelEntrada) {
      FResMediosVenturi.VelEntradaMED =
          FResMediosVenturi.VelEntradaSUM / FResMediosVenturi.TiempoSUM;
      FResMediosVenturi.VelEntradaSUM = 0.;
    }
    if (FResMediosVenturi.VelLateral) {
      FResMediosVenturi.VelLateralMED =
          FResMediosVenturi.VelLateralSUM / FResMediosVenturi.TiempoSUM;
      FResMediosVenturi.VelLateralSUM = 0.;
    }
    if (FResMediosVenturi.GastoEntrada) {
      FResMediosVenturi.GastoEntradaMED =
          FResMediosVenturi.GastoEntradaSUM / FResMediosVenturi.TiempoSUM;
      FResMediosVenturi.GastoEntradaSUM = 0.;
    }
    if (FResMediosVenturi.GastoLateral) {
      FResMediosVenturi.GastoLateralMED =
          FResMediosVenturi.GastoLateralSUM / FResMediosVenturi.TiempoSUM;
      FResMediosVenturi.GastoLateralSUM = 0.;
    }
    FResMediosVenturi.TiempoSUM = 0;
  } catch (std::exception &N) {
