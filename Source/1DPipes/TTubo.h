...
private:
  ...
  int FTuboCCNodoIzq; //!< Save the position in the struct TuboExtremo of the
                      //!< pipe connected to the left end
  int FTuboCCNodoDer; //!< Save the position in the struct TuboExtremo of the
                      //!< pipe connected to the right end
  ...
  double *FDExtTramo;    //!< Array with the diameter at the end of the parts of
                         //!< the pipe
  double *FDiametroTubo; //!< Diametro de cada nodo de calculo
  double *FDiametroD12;  //!< Diameter interpolated between two cell using the
                         //!< diameters average
  double *FDiametroS12;  //!< Diameter interpolated between two cell using the
                         //!< section average
  ...
  double
      FEspesorIntPrin; //!< Inner thickness of the main layer of the pipe wall
  double
      FEspesorExtPrin;   //!< Outer thickness of the main layer of the pipe wall
  double FEspesorPrin;   //!< Thicknes of the main layer of the pipe wall
  double FDensidadPrin;  //!< Density of the material of the main layer of the
                         //!< pipe wall
  double FCalEspPrin;    //!< Specific heat of the material of the main layer of
                         //!< the pipe wall
  double FConductPrin;   //!< Conductivity of the material of the main layer of
                         //!< the pipe wall
  ...
  nmTipoCalcTempParedTubos
      FTipoCalcTempPared; //!< Type of wall temperature calculation
  double FTIniParedTub;   //!< Initial wall temperatura
  double *FVelPro; //!< Array with the temporal average of the gas velocity at
                   //!< each cell
  ...
  double
      *FAsonido0; //!< Adimensionalized speed of sound at the current time step
  double
      *FVelocidad0;  //!< Adimensionalized gas velocity at the current time step
  ...
  stModeloCalculoTubos FMod; //!< Struct with the options of the numerical
                             //!< scheme used to solve the duct
  ...
  double **FFraccionMasicaEspecie; //!< Mass fraction of each chemical specie at
                                   //!< every cell
  double **FFraccionMasicaCC;      //!< Mass fraction of each chemical specie at
                                   //!< boundaries
  bool FHayEGR;                    //!< The EGR specie is considered?
  int FIntEGR; //!< Integer equal to 1 if EGR is considered else equal to 0

  int FNumEcuaciones; //!< Total number of conservation equation (including
  ...
  double *FCpMezcla;    //!< Specific heat at constant pressure at every
                        //!< calculation cell.
  double *FCvMezcla; //!< Specific heat at constant volume at every calculation
                     //!< cell.
  double FGammaN;
  ...
  //        double 						FTime0;
  //        //!< Time at current time step double
  //        FTime1; 				//!< Time at following time step
  //        double 						FDeltaTime;
  //        //!< Time step
  ...
  stResInstantTubo
      *ResultInstantaneos; //!< Struct with the instantaneous results
  ...
  double
      FControlResMed; //!< Parameter to control when the average is calculated
  ...
  int FCicloActual; // !< Number of the current cycle when there is not engine
                    // in the configuration
  double FDuracionCiclo; // !< Cycle duration (in time) to calculate wall
                         // temperature convergence when there is not engine in
                         // the configuration
  int FNumCiclosSinInerciaTermica; // !< Number of cycles without thermal
                                   // inertia consideration when there is not
                                   // engine in the configuration

  TDPF *FDPFSalidaTubo;  //  DPF connected to the outlet of the duct
  TDPF *FDPFEntradaTubo; //  DPF connected to the inlet of the duct
  bool FHayDPFNodoIzq;   //  bool to identify DPF connected at the inlet of the
                         //  duct
  bool FHayDPFNodoDer;   //  bool to identify DPF connected at the outlet of the
                         //  duct
  ...
  /*! Transfor from the gas velocity, speed of sound and pressure to state
   * vector */
  ...
  /*! Transfor from the gas velocity, speed of sound and pressure to state
   * vector (including the cross section in the state vector) */
  void
  Transforma1Area(const double &v,      //!< Gas velocity
                  const double &a,      //!< Speed of sound
                  const double &p,      //!< Pressure
                  double **U,           //!< State vector
  ...
  /*! Transfor from the state vector to gas  velocity, speed of sound and
   * pressure */
...
public:
  ...
  stCapa GetCapa(int i) { return FCapa[i]; };

  /*! Return the type of monolith channel connected to the duct end */

  int GetTipoCanal(int i) { return FTipoCanal[i]; };
  ...
  TTubo(int SpeciesNumber, //!< Number of different species considered in the
                           //!< simulation.
  ...
  /*!Comment  */
  void HeaderAverageResults(std::ostream &medoutput,  //!< Comment
                            stEspecies *DatosEspecies //!< Comment
  ) const;

  /*!Comment  */
  void ImprimeResultadosMedios(std::ostream &medoutput //!< Comment
  ) const;
  ...
  /*!Comment  */
  void HeaderInstantaneousResults(std::ostream &insoutput,  //!< Comment
                                  stEspecies *DatosEspecies //!< Comment
  ) const;

  /*!Comment  */
  void ImprimeResultadosInstantaneos(std::ostream &insoutput //!< Comment
  ) const;
