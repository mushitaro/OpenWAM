...
void TTubo::LeeDatosGeneralesTubo(std::istream &FileInput) {
  ...
  try {
    ...
    if (fracciontotal > 1 + 1.e-10 && fracciontotal < 1 - 1e-10) {
      std::cout << "ERROR: Total mass fraction must be equal to 1. Check input "
                   "data for pipe  "
...
void TTubo::CalculoPuntosMalla(double ene) {
  ...
  try {
    ...
    if (FTipoMallado != nmDistancia && FTipoMallado != nmAngular) {
      std::cout << "WARNING: No se ha definido una forma correcta de mallado "
                   "en el tubo: "
                << FNumeroTubo << std::endl;
    }
    ...
    for (int i = 1; i < FNin - 1; i++) {
      ...
      FDiametroTubo[i] =
          Interpola(FDExtTramo[ii], FDExtTramo[ii + 1], FLTramo[ii + 1], r1);
    ...
    for (int i = 0; i < FNin - 1; i++) {
      ...
      FDiametroS12[i] =
          sqrt((pow2(FDiametroTubo[i + 1]) + pow2(FDiametroTubo[i])) / 2.);
...
void TTubo::ComunicacionDPF(
  ...
  try {
      ...
      if (CC[FNodoIzq - 1]->getTipoCC() == nmPipeToPlenumConnection) {
        ...
        for (int k = 0; k < Deposito[numDeposito - 1]->getNUniones(); k++) {
          if (Deposito[numDeposito - 1]->GetCCDeposito(k)->getUnionDPF() &&
              !PrimeraVez) {
            FHayDPFNodoIzq = true;
            FTipoCanal[0] = Deposito[numDeposito - 1]
                                ->GetCCDeposito(k)
                                ->GetTuboExtremo(0)
                                .TipoCanal;
      ...
      if (CC[FNodoDer - 1]->getTipoCC() == nmPipeToPlenumConnection) {
        ...
        for (int k = 0; k < Deposito[numDeposito - 1]->getNUniones(); k++) {
          if (Deposito[numDeposito - 1]->GetCCDeposito(k)->getUnionDPF() &&
              !PrimeraVez) {
            FHayDPFNodoDer = true;
            FTipoCanal[1] = Deposito[numDeposito - 1]
                                ->GetCCDeposito(k)
                                ->GetTuboExtremo(0)
                                .TipoCanal;
    ...
    std::cout << "ERROR: TTubo::ComunicacionDPF en el Tubo " << FNumeroTubo
              << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception(N.what());
  }
...
void TTubo::IniciaVariablesFundamentalesTubo() {
  ...
  try {
    ...
    if (FCalculoEspecies == nmCalculoCompleto) {
      ...
      RMezclaIni = CalculoCompletoRMezcla(
          FComposicionInicial[0], FComposicionInicial[1],
          FComposicionInicial[2], 0, FCalculoGamma, nmMEP);
      CpMezclaIni = CalculoCompletoCpMezcla(
          FComposicionInicial[0], FComposicionInicial[1],
          FComposicionInicial[2], 0, __units::degCToK(FTini), FCalculoGamma,
          nmMEP);
      GammaIni = CalculoCompletoGamma(RMezclaIni, CpMezclaIni, FCalculoGamma);

    } else if (FCalculoEspecies == nmCalculoSimple) {

      RMezclaIni = CalculoSimpleRMezcla(
          FComposicionInicial[0], FComposicionInicial[1], FCalculoGamma, nmMEP);
      CvMezclaIni =
          CalculoSimpleCvMezcla(__units::degCToK(FTini), FComposicionInicial[0],
    ...
    double viscgas = 1.4615e-6 * pow150(__units::degCToK(FTini)) /
                     (__units::degCToK(FTini) + 110.4);
    ...
    for (int i = 0; i < FNin; i++) {
      ...
      FFlowMass[i] = FArea[i] * FVelocidadDim[i] *
                     __units::BarToPa(FPresion0[i]) / FRMezcla[i] /
                     FTemperature[i];

      if (FMod.FormulacionLeyes == nmSinArea) {
        Transforma1(FVelocidad0[i], FAsonido0[i], FPresion0[i], FU0, FGamma[i],
                    FGamma1[i], FFraccionMasicaEspecie[i], i);

        Transforma1(FVelocidad1[i], FAsonido1[i], FPresion1[i], FU1, FGamma[i],
                    FGamma1[i], FFraccionMasicaEspecie[i], i);
        ...
        std::cout << "ERROR: El tipo de formulacion de las leyes de "
                     "conservacion no esta bien definido"
                  << std::endl;
        throw Exception("");
      }
  ...
  try {
    ...
    for (int i = 0; i < FNin; i++) {
      ...
      if (FCalculoEspecies == nmCalculoSimple) {
        ...
        FRMezcla[i] = CalculoSimpleRMezcla(FFraccionMasicaEspecie[i][0], 0,
                                           FCalculoGamma, nmMEP);
        double CvMezcla =
...
void TTubo::Transforma1(const double &v, const double &a, const double &p,
                        double **U, const double &Gamma, const double &Gamma1,
...
void TTubo::Transforma1Area(const double &v, const double &a, const double &p,
                            ...
                            const double &Gamma1, double *Yespecie,
                            const int &i) {
#ifdef usetry
  try {
#endif
...
void TTubo::Transforma2(double &v, double &a, double &p, double **U,
                        const double &Gamma, const double &Gamma1,
                        double *Yespecie, const int &i) {
#ifdef usetry
  try {
#endif
...
void TTubo::Transforma2Area(double &v, double &a, double &p, double **U,
                            const double &area, const double &Gamma,
                            const double &Gamma1, double *Yespecie,
                            const int &i) {
#ifdef usetry
  try {
...
void TTubo::Transforma3Area(double **Ufct, double **U, double Area,
  ...
  try {
    ...
    Ufct[2][i] =
        ((U[2][i] - pow2(U[1][i]) / U[0][i] / 2.) * Gamma1) *
        pow((1. +
...
void TTubo::Transforma4Area(double **U1, double **Ufctd, double Area,
                            double Gamma, double Gamma1, double Gamma3,
  ...
  try {
    ...
    while (error > 0.00000001) {
      ...
      fu = vel - Ufctd[0][i] / (Area * Ufctd[2][i] * Gamma4) *
                     pow(2. * Ufctd[1][i], (Gamma / Gamma1)) *
                     pow((2. * Ufctd[1][i] - pow2(vel)), -Gamma6);

      dfu = 1. - Ufctd[0][i] * vel / (Area * Ufctd[2][i] * Gamma) *
                     pow(2. * Ufctd[1][i], (Gamma / Gamma1)) *
...
void TTubo::IniciaVariablesTransmisionCalor(
  ...
  try {
    ...
    for (int i = 0; i < FNin; i++) {
      ...
      dist2 =
          FXref * (double)(FNin - 1 - i) + BC[FNodoDer - 1]->getPosicionNodo();
      if (Minimo(dist1, dist2) >= 10000.)
        FCoefTurbulencia[i] = 1.;
      else
        FCoefTurbulencia[i] =
            1. + 3. * exp(-Minimo(dist1, dist2) / (4 * FDiametroTubo[i]));
      for (int j = 0; j < 3; j++) {
...
void TTubo::LaxWendroff() {
  ...
  try {
    ...
    CalculaFuente2(FU0, FV2, FDiametroTubo, Fhi, Frho, FRe, FTPTubo[0], FGamma,
  ...
  try {
    ...
    if (FNin > 3) {
      ...
      if (FMod.Difusion == nmDamping) {
        ...
        std::cout << "ERROR: Metodo de Difusion mal definido para el FCT en el "
                     "tubo n. "
                  << FNumeroTubo << std::endl;
        throw Exception("");
      }
      ...
      if (FMod.Antidifusion == nmExplicit) {
        ...
        std::cout << "ERROR: Metodo de Antidifusion mal definido para el FCT "
                     "en el tubo n. "
                  << FNumeroTubo << std::endl;
        throw Exception("");
      }
...
void TTubo::LaxWendroffArea() {
  ...
  try {
    ...
    CalculaFuente2Area(FU12, FV2, FArea12, hi12, rho12, Re12, TPTubo12, Gamma12,
                       Rmezcla12, Gamma1_12, Nodos);
...

  {
    ...
    for (int i = 0; i < Nodos; i++) {
      if (!DoubEqZero(U[1][i])) {
        ...
        W[2][i] =
            Gamma[i] * U[2][i] * U1U0 - Gamma1[i] * U[1][i] * pow2(U1U0) / 2.;
        for (int j = 3; j < FNumEcuaciones; j++) {
          W[j][i] = U[j][i] * U1U0;
        }
      } else {
...
void TTubo::CalculaFuente2(double **U, double **V2, double *diame, double *hi,
                           double *rho, double *Re, double *TempParedTubo,
                           double *Gamma, double *Rmezcla, double *Gamma1,
                           int Nodos) {
...
void TTubo::CalculaFuente2Area(double **U, double **V2, double *Area,
                               ...
                               double *TempParedTubo, double *Gamma,
...
void TTubo::TransformaContorno(double &L, double &B, double &E, double &a,
                               double &v, double &p, const int &modo,
                               const double &Gamma1, const double &Gamma3,
                               const double &Gamma4, const double &Gamma5) {
#ifdef usetry
  try {
#endif
    if (modo == 0) {
    ...
    for (int i = 0; i < FNin; i++) {
      ...
      if (-1. >= Machx || Machx > 1.) {
        Machy = Machx / fabs(Machx) *
                sqrt((pow2(Machx) + 2. / FGamma1[i]) /
                     (FGamma4[i] * pow2(Machx) - 1.));
        Sonidoy = FAsonido0[i] * sqrt((FGamma1[i] / 2. * pow2(Machx) + 1.) /
  ...
  try {
    ...
    for (int i = 1; i < FNin - 1; i++) {
      ...
      presion = __units::PaToBar(
          (FU1[2][i] - FU1[1][i] * velocidad * __cons::ARef / 2.0) *
          FGamma1[i] / FArea[i]);
      asonido = sqrt(FGamma[i] * __units::BarToPa(presion) * FArea[i] /
                     FU1[0][i] / __cons::ARef2);
      ...
      if (-1. >= Machx || Machx > 1.) {
        ...
        FU1[0][i] = FGamma[i] * __units::BarToPa(presion) /
    ...
    for (int i = 0; i < FNumResMedios; i++) {
      ...
      for (int j = 0; j < NumVars; j++) {
        ...
        case 9:
          ...
          break;
        default:
          std::cout << "WARNING: El tipo de variable seleccionada para la "
                       "salida de resultados medios no es valida"
                    << std::endl;
        };
      }
    }
...
void TTubo::HeaderAverageResults(std::ostream &medoutput,
...
void TTubo::ImprimeResultadosMedios(std::ostream &medoutput) const {
#ifdef usetry
  try {
#endif
  ...
  try {
    ...
    for (int i = 0; i < FNumResInstant; i++) {
      ...
      for (int j = 0; j < NumVars; j++) {
        ...
        default:
          std::cout << "WARNING: El tipo de variable seleccionada para la "
                       "salida de resultados instantaneos no es valida"
                    << std::endl;
        }
      }
    }
...
void TTubo::HeaderInstantaneousResults(std::ostream &insoutput,
                                       stEspecies *DatosEspecies) const {
#ifdef usetry
  try {
#endif
...
void TTubo::ImprimeResultadosInstantaneos(std::ostream &insoutput) const {
#ifdef usetry
  try {
#endif
...
void TTubo::CalculaResultadosMedios(double Theta) {
  double dist = 0., Vble = 0., d = 0., Rmezcla = 0., Gamma = 0.,
         GastoPonderacion = 0.;
  int n1 = 0, n2 = 0;
#ifdef usetry
  try {
    ...
    for (int i = 0; i < FNumResMedios; i++) {
      ...
      if (n1 >= FNin - 1) {
        ...
        if (ResultadosMedios[i].Velocity)
          ResultadosMedios[i].VelocidadSUM +=
              FVelocidadDim[FNin - 1] * FDeltaTime;
        if (ResultadosMedios[i].Massflow) {
        ...
        if (ResultadosMedios[i].FraccionMasicaEspecies) {
          for (int j = 0; j < FNumeroEspecies - FIntEGR; j++) {
            ResultadosMedios[i].FraccionSUM[j] +=
                FFraccionMasicaEspecie[FNin - 1][j] * GastoPonderacion *
                FDeltaTime;
          }
        }
      } else {
        ...
        if (ResultadosMedios[i].TemperaturaGas ||
            ResultadosMedios[i].FraccionMasicaEspecies) {
          GastoPonderacion =
              fabs(Interpola(FFlowMass[n1], FFlowMass[n2], 1., d));
          ResultadosMedios[i].GastoPonderacionSUM += GastoPonderacion;
        ...
        if (ResultadosMedios[i].FraccionMasicaEspecies) {
          for (int j = 0; j < FNumeroEspecies - FIntEGR; j++) {
            ...
            ResultadosMedios[i].FraccionSUM[j] +=
                Vble * GastoPonderacion * FDeltaTime;
          }
        }
      }
    }
      ...
      for (int i = 0; i < FNumResMedios; i++) {
        ...
        if (ResultadosMedios[i].Massflow) {
          ResultadosMedios[i].GastoMED =
              ResultadosMedios[i].GastoSUM / FTiempoMedSUM;
          ResultadosMedios[i].GastoSUM = 0.;
        }
        if (ResultadosMedios[i].TemperaturaInternaPared) {
          ResultadosMedios[i].TemperaturaInternaParedMED =
              ResultadosMedios[i].TemperaturaInternaParedSUM / FTiempoMedSUM;
          ResultadosMedios[i].TemperaturaInternaParedSUM = 0.;
        }
        if (ResultadosMedios[i].TemperaturaIntermediaPared) {
          ResultadosMedios[i].TemperaturaIntermediaParedMED =
              ResultadosMedios[i].TemperaturaIntermediaParedSUM / FTiempoMedSUM;
          ResultadosMedios[i].TemperaturaIntermediaParedSUM = 0.;
        }
        if (ResultadosMedios[i].TemperaturaExternaPared) {
          ResultadosMedios[i].TemperaturaExternaParedMED =
              ResultadosMedios[i].TemperaturaExternaParedSUM / FTiempoMedSUM;
          ResultadosMedios[i].TemperaturaExternaParedSUM = 0.;
        }
        if (ResultadosMedios[i].NITmedio) {
        ...
        if (ResultadosMedios[i].CoefPelInterior) {
          ResultadosMedios[i].CoefPelInteriorMED =
              ResultadosMedios[i].CoefPelInteriorSUM / FTiempoMedSUM;
          ResultadosMedios[i].CoefPelInteriorSUM = 0.;
        }
        if (ResultadosMedios[i].FraccionMasicaEspecies) {
          for (int j = 0; j < FNumeroEspecies - FIntEGR; j++) {
...
void TTubo::CalculaResultadosInstantaneos() {
  double dist = 0., d = 0., ason = 0., vel = 0., Aa = 0., ason1 = 0., vel1 = 0.,
         Aa1 = 0., ason2 = 0., vel2 = 0., Aa2 = 0.;
  int n1 = 0, n2 = 0;
#ifdef usetry
  try {
#endif
        ...
        if (n1 >= FNin - 1) {
          ...
          if (ResultInstantaneos[i].Velocity)
            ResultInstantaneos[i].VelocidadINS =
                FVelocidad0[FNin - 1] * __cons::ARef;
          if (ResultInstantaneos[i].TemperaturaGas) {
            double temp =
                __units::KTodegC(pow2(FAsonido0[FNin - 1] * __cons::ARef) /
                                 (FGamma[FNin - 1] * FRMezcla[FNin - 1]));
          ...
          if (ResultInstantaneos[i].PresionDerecha) {
            double PreDer =
                pow(((ason + vel) / Aa + 1) / 2., FGamma4[FNin - 1]);
          ...
          if (ResultInstantaneos[i].NIT) {
            double nit = CalculaNIT(
                FAsonido0[FNin - 1], FVelocidad0[FNin - 1], FPresion0[FNin - 1],
          ...
          if (ResultInstantaneos[i].FlujoMasico) {
            ...
            // double
            // dens1=FGamma[n1]*FPresion0[n1]*1e5/pow(FAsonido0[n1]*__cons::ARef,2.);
            double gto1 = FFlowMass[n1];
            // double area2=pow(FDiametroTubo[n2],2)*__cons::Pi_4;
            // double
            // dens2=FGamma[n2]*FPresion0[n2]*1e5/pow(FAsonido0[n2]*__cons::ARef,2.);
          ...
          if (ResultInstantaneos[i].NIT) {
            double nit1 =
                CalculaNIT(FAsonido0[n1], FVelocidad0[n1], FPresion0[n1],
                           FDiametroTubo[n1], FGamma[n1], FRMezcla[n1]);
            double nit2 =
                CalculaNIT(FAsonido0[n2], FVelocidad0[n2], FPresion0[n2],
...
#endif
    double kp = 0.;
    double tem0 = 0., pre0 = 0., area = 0., gto = 0., nit = 0., V = 0., A = 0.,
           V2 = 0., A2 = 0.;
      ...
      if (FTipoTransCal == nmTuboAdmision || FTipoTransCal == nmTuboEscape) {
        for (int i = 0; i < FNin; i++) {
          ...
          case nmAire:
            ...
            Re = rhog * 1. *
                 (FDiametroTubo[i] +
                  2 * (FEspesorIntPrin + FEspesorPrin + FEspesorExtPrin)) /
                 viscext;
            break;
          case nmAgua:
          ...
          if (dtem != 0.) {
            Fhe[i] = Fhe[i] +
                     __cons::Sigma * FEmisividad *
                         (pow4(__units::degCToK(FTPTubo[2][i])) - pow4(FTExt)) /
                         dtem;
          }
        ...
        rhog = 980.;
        viscext =
            ((-2.632351E-09 * temed + 2.737629E-06) * temed - 9.530709E-04) *
                temed +
            1.114642E-01;
        ...
        vel = 5.64268e-7 * Engine[0]->getRegimen() / 60. *
              Engine[0]->getParPotMax() / Engine[0]->getGeometria().NCilin /
              pow(L, 2.);
        ...
        for (int i = 0; i < FNin; i++) {
          ...
          if (FTPTubo[2][i] > 100.) {
            ...
            viscpared =
                ((-2.632351E-09 * Tp2 + 2.737629E-06) * Tp2 - 9.530709E-04) *
                    Tp2 +
                1.114642E-01;
          }

          Fhe[i] = 0.027 * (1 + 24.2 / pow(2.3, 0.7) / pow025(Re)) *
                   pow(Re, 0.8) * cbrt(Pr) * pow(viscext / viscpared, 0.14) *
                   cond / (L / 2.3);
        }
      }
    }
      ...
      for (int i = 0; i < FNin; i++) {
        ...
        for (int j = 0; j < FNumCapas; j++) {
          ...
          if (FCapa[j].EsPrincipal == false) {
            Rcond = log(Dext / Dint) / __cons::Pi_x_2 / FCapa[j].Conductividad /
                    FXref;
            if (EsInterior) {
              ...
              if (FCapa[j].EsFluida) {
                ...
                if (Tint != Text) {
                  Rrad = (1 / FCapa[j].EmisividadInterior +
                         ...
                         (__cons::Sigma * __cons::Pi * Dint * FXref) *
                         (Tint - Text) / (pow4(Tint) - pow4(Text));
            ...
            // Calculo de la resistencia termica radial exterior e interior de
            // la capa principal.
            FResistRadInt[i] += log((Dint + FCapa[j].Espesor) / Dint) /
                                __cons::Pi_x_2 / FCapa[j].Conductividad / FXref;
        ...
        if (i == 0) {
          ...
          if (FHayDPFNodoIzq) {
            if (FTipoCanal[0] == 0) {
              ...
              FResistAxiAnt[i] =
                  FDPFEntradaTubo->getAjustRAxAnt() /
              ...
              FResistAxiAnt[i] =
                  FDPFEntradaTubo->getAjustRAxPos() /
                  (Pi * FDPFEntradaTubo->getConductividadMetal() *
                   (FDPFEntradaTubo->getDiametroEfect() +
                    2 * FDPFEntradaTubo->getEspesorAislante() +
                    2 * FDPFEntradaTubo->getEspesorAire() +
        ...
        for (int j = 0; j < FNumCapas; j++) {
          ...
          if (FCapa[j].EsPrincipal == false) {
            if (EsInterior) {
              ...
              if (!FCapa[j].EsFluida) {
                // Capa de material.
                Cap = FCapa[j].Density * FCapa[j].CalorEspecifico *
                      __geom::Ring_area(Dint, Dext) * FXref;
                FCapInt[i] += Cap;
              }
            } else {
            ...
            // Calculo de la capacidad termica exterior, media e interior de la
            // capa principal.
            FCapInt[i] +=
                FDensidadPrin * FCalEspPrin *
                __geom::Ring_area(DIntPrin, DIntPrin + 0.5 * FEspesorPrin) *
                FXref;
            FCapMed[i] = FDensidadPrin * FCalEspPrin *
                         __geom::Ring_area(DIntPrin + 0.5 * FEspesorPrin,
                                           DIntPrin + 1.5 * FEspesorPrin) *
                         FXref;
            FCapExt[i] += FDensidadPrin * FCalEspPrin *
                          __geom::Ring_area(DIntPrin + 1.5 * FEspesorPrin,
                                            DIntPrin + 2 * FEspesorPrin) *
                          FXref;
            EsInterior = false;
          }
          Dint = Dext;
    ...
    if (FCoefAjusTC != 0) {
      for (int i = 0; i < FNin; i++) {
        ...
        if (DoubEqZero(FVelPro[i])) {
          FRe[i] =
              1E-06; // Para que nunca valga 0 y evitar divisiones por cero.
        } else {
        ...
        case nmPipaEscape:
          ...
          Fhi[i] = 0.1 * pow(FRe[i], 0.8) * 0.709 *
                   pow(viscgas / viscpared, 0.14) * cond / FDiametroTubo[i];
          break;
        default:
...
void TTubo::CalculaTemperaturaPared(
  ...
  double Tpant0 = 0., Tpant1 = 0., Tpant2 = 0., Tpantant = 0., Tpantpos = 0.,
  ...
  try {
    ...
    if (FTipoCalcTempPared != nmTempConstante && FCoefAjusTC != 0) {
      ...
      for (int i = 0; i < FNin; i++) {
        ...
        // Establece la temperatura de la pared en el instante de calculo
        // anterior...
        ...
        if (i == 0) {
          if (BC[FNodoIzq - 1]->getTipoCC() == nmPipesConnection) {
            if (dynamic_cast<TCCUnionEntreTubos *>(BC[FNodoIzq - 1].get())
              ...
              if (BC[FNodoIzq - 1]->GetTuboExtremo(0).Pipe->getNumeroTubo() ==
                ...
                if (BC[FNodoIzq - 1]->GetTuboExtremo(extremo).TipoExtremo ==
                  ...
                  nodo =
                      BC[FNodoIzq - 1]->GetTuboExtremo(extremo).Pipe->getNin() -
                      1;
                }
              } else {
                extremo = 0;
            ...
            if (FHayDPFNodoIzq) {
              Tpantant = __units::degCToK(
                  FDPFEntradaTubo->GetTSuperficie(FNodoDPFEntrada, 2));
            }
          }
#endif
          ...
          if (BC[FNodoDer - 1]->getTipoCC() == nmPipesConnection) {
            if (dynamic_cast<TCCUnionEntreTubos *>(BC[FNodoDer - 1].get())
              ...
              if (BC[getNodoDer() - 1]
                ...
                if (BC[FNodoDer - 1]->GetTuboExtremo(extremo).TipoExtremo ==
                  ...
                  nodo =
                      BC[FNodoDer - 1]->GetTuboExtremo(extremo).Pipe->getNin() -
                      1;
                }
              } else {
                extremo = 0;
            ...
            if (FHayDPFNodoDer) {
              Tpantpos = __units::degCToK(
                  FDPFSalidaTubo->GetTSuperficie(FNodoDPFSalida, 2));
            }
          }
#endif
        } else {
        ...
        if (FResistAxiAnt[i] > 0. && FResistAxiPos[i] > 0.) {
          FTPTubo[1][i] = DeltaTTPared / FCapMed[i] *
                              (1 / FResistRadInt[i] * (Tpant0 - Tpant1) +
                               ...
                               1 / FResistAxiPos[i] * (Tpantpos - Tpant1)) +
                          Tpant1;
        ...
        // Si el tipo de calculo es sin inercia termica o lleva menos de
        // "NumCiclosSinInerciaTermica" ciclos calculando...
        if (FTipoCalcTempPared == nmVariableSinInerciaTermica ||
          ...
          if (Fhi[i] != 0. && Theta > FAnguloTotalCiclo) {
            // Sumatorio de h*Tg*incrt intermedio(para el calculo de la
            // integral).
            FSUMTPTuboPro[0][1][i] +=
                1 / (Ri + FResistRadInt[i]) * Tg * DeltaTTPared;
            // Sumatorio de h*incrt intermedio(para el calculo de la integral).
            FSUMTPTuboPro[1][1][i] +=
                1 / (Ri + FResistRadInt[i]) * DeltaTTPared;
      ...
      if (FCicloTubo != Engine[0]->getCiclo() && FSUMTime > 0.) {
        // ...si (el tipo de calculo es sin inercia termica o lleva menos de
        // "NumCiclosSinInerciaTermica" ciclos calculando) y esta en el segundo
        // ciclo, calcula la temperatura de convergencia
        if ((FTipoCalcTempPared == nmVariableSinInerciaTermica ||
             Theta / FAnguloTotalCiclo <=
                 Engine[0]->getNumCiclosSinInerciaTermica()) &&
          ...
          EsPrimeraVez = true;
          while (ErrorTp >=
                 1) { // Itera hasta conseguir una diferencia entre las
                      // temperaturas de pared menor a 1degC entre Steps.
            ErrorTp = 0.;
            for (int i = 0; i < FNin; i++) {
              // Establece la temperatura de la pared en el instante de calculo
              // anterior...
              ...
              if (i == 0) {
                if (BC[FNodoIzq - 1]->getTipoCC() == nmPipesConnection) {
                  if (dynamic_cast<TCCUnionEntreTubos *>(BC[FNodoIzq - 1].get())
                    ...
                    Tpantant =
                        __units::degCToK(BC[FNodoIzq - 1]
                                             ->GetTuboExtremo(extremo)
                                             .Pipe->GetTPTuboAnt(1, nodo));
                  }
                }
#ifdef ParticulateFilter
                ...
                if (FResistAxiAnt[i] > 0. && FResistAxiPos[i] > 0.) {
                  FTPTubo[1][i] =
                      (FSUMTime * (1 / (Re + FResistRadExt[i]) * Text +
                                   1 / FResistAxiAnt[i] * Tpantant +
                                   1 / FResistAxiPos[i] * Tpantpos) +
                       FSUMTPTuboPro[0][1][i]) /
                      (FSUMTime *
              ...
              FTPTubo[0][i] =
                  ((FSUMTime * 1 / FResistRadInt[i] * Tpant1 +
                    FSUMTPTuboPro[0][0][i]) /
        ...
        if ((FTipoCalcTempPared == nmVariableSinInerciaTermica ||
          ...
          while (ErrorTp >=
            ...
            for (int i = 0; i < FNin; i++) {
              ...
              if (EsPrimeraVez) {
                FTPTubo[1][i] = (FSUMTime / (Re + FResistRadExt[i]) * Text +
                                 FSUMTPTuboPro[0][1][i]) /
              ...
              FTPTubo[0][i] =
                  ((FSUMTime / FResistRadInt[i] * Tpant1 +
                    FSUMTPTuboPro[0][0][i]) /
...
void TTubo::SalidaGeneralTubos(stEspecies *DatosEspecies) const {
  ...
  try {
    ...
    if (FNumResMedios > 0) {
      ...
      printf("__________________________________\n\n\n");
      for (int i = 0; i < FNumResMedios; i++) {
        std::cout << "Average results in point " << i << " place in pipe "
                  << FNumeroTubo;
        ...
        if (ResultadosMedios[i].Pressure)
          std::cout << "Average Pressure    = "
                    << ResultadosMedios[i].PresionMED << " bar" << std::endl;
        if (ResultadosMedios[i].Velocity)
          std::cout << "Average Velocity    = "
                    << ResultadosMedios[i].VelocidadMED << " m/s" << std::endl;
        if (ResultadosMedios[i].Massflow)
          std::cout << "Average Massflow    = " << ResultadosMedios[i].GastoMED
                    << " kg/s" << std::endl;
        if (ResultadosMedios[i].NITmedio)
          std::cout << "NIT Medio         = " << ResultadosMedios[i].NITmedioMED
                    << " Watts" << std::endl
                    << std::endl;
        // std::cout << "Fraccion Masica Media de " << DatosEspecies[i].Nombre
        // << ": " << ResultadosMedios[i].FraccionMED[i] << std::endl;
      }
    }
#ifdef usetry
  } catch (std::exception &N) {
...
inline double TTubo::Maximo(double x, double y) { return x > y ? x : y; }

// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------

inline double TTubo::Minimo(double x, double y) { return x > y ? y : x; }
...
double TTubo::Interpola_Entropia(nmPipeEnd TipoExtremoTubo,
  ...
  try {
    ...
    if (DeltaTiempo < 1e-15 || DoubEqZero(FVelocidadDim[extremo])) {
      ...
      Calculo_Entropia(entropia, velocidadp, ind, dist, signo, DeltaTiempo,
                       indiceCC);
    }
...
void TTubo::Calculo_Entropia(double &entropia, double &velocidadp, int ind,
                             double dist, int signo, double DeltaTiempo,
                             int indiceCC) {
#ifdef usetry
  try {
#endif
    ...
    if (dist > 0. || dist < 1.) {
      ...
      for (int j = 0; j < FNumeroEspecies - FIntEGR; j++) {
        FFraccionMasicaCC[indiceCC][j] =
            Interpola(FFraccionMasicaEspecie[ind][j],
    ...
    FVelocidadCC[indiceCC] = velocidadp; // Se que no hace falta hacerlo
                                         // asi,pero me queda más ordenado.
    ...
    if (!DoubEqZero(FCoefAjusTC)) {
      ...
      double dacal =
          gamma3p * entropiap * q * FCoefAjusTC * DeltaTiempo / pow2(asonidop);
...
double TTubo::Interpola_Caracteristica(double entropia, int signo, int extremo,
  ...
  try {
    ...
    if (DeltaTiempo < 1e-15) {
      Calculo_Caracteristica(caracteristica, velocidadp, asonidop, ind, 0.,
                             signo, entropia, DeltaTiempo);
    } else {
      ...
      Calculo_Caracteristica(caracteristica, velocidadp, asonidop, ind, dist,
                             signo, entropia, DeltaTiempo);
    }
...
void TTubo::Calculo_Caracteristica(double &caracteristica, double &velocidadp,
                                   double &asonidop, int ind, double dist,
                                   int signo, double entropia,
                                   double DeltaTiempo) {
#ifdef usetry
  try {
#endif
    ...
    if (velocidadp != 0. && FCoefAjusFric != 0.) {
      ...
      double dafric =
          signo * gamma1p * (1. + signo * gamma1p * velocidadp / asonidop) * f *
...
void TTubo::CalculaB() {
  ...
  try {
#endif
    double v = 0., p = 0., f = 0., tgas = 0., g = 0., q = 0., diamemed = 0.,
           Rm = 0., Rm1 = 0.;
    double Remed, gamma, gamma1, Rmed, himed, rhomed, twallmed, Vmed, H1, H2,
        Hmed, Amed, rhoAmed;
    ...
    for (int i = 0; i < FNin - 1; i++) {
      ...
      if (FArea[i] != FArea[i + 1] || FCoefAjusFric != 0 || FCoefAjusTC != 0) {
        ...
        if (FArea[i] != FArea[i + 1]) {
          FTVD.Bvector[1][i] +=
              rhomed * Amed * Amed / gamma * (FArea[i] - FArea[i + 1]);
        }
...
void TTubo::CalculaBmen() {
  ...
  try {
#endif
    double v, p, f, tgas, g, q, diamemed, B, Rm, Rm1, gamma, gamma1, Vmed, Amed,
        H1, H2, Hmed, rhoAmed;
    ...
    for (int i = 1; i < FNin; i++) {
      ...
      if (FArea[i] != FArea12[i - 1] || FCoefAjusFric != 0 ||
        ...
        if (FArea[i] != FArea12[i - 1]) {
          FTVD.Bmen[1][i] +=
              rhomed * Amed * Amed / gamma * (FArea12[i - 1] - FArea[i]);
        }
    ...
    for (int i = 0; i < FNin - 1; i++) {
      ...
      if (FArea[i] != FArea12[i] || FCoefAjusFric != 0 || FCoefAjusTC != 0) {
        ...
        if (FArea[i] != FArea12[i]) {
          FTVD.Bmas[1][i] +=
              rhomed * Amed * Amed / gamma * (FArea[i] - FArea12[i]);
        }
...
void TTubo::CalculaMatrizJacobiana() {
  ...
  try {
#endif
    double Rmed = 0., Vmed = 0., Vmed2 = 0., Hmed = 0., Amed = 0., Amed2 = 0.,
           gamma = 0., H1 = 0., H2 = 0., Rmed1 = 0.;
...
void TTubo::TVD_Estabilidad() {
  ...
  try {
    ...
    for (int i = 0; i < FNin - 1; i++) {
      for (int k = 0; k < 3; ++k) {
        FTVD.DeltaU[k][i] =
            FTVD.Qmatrix[k][0][i] * (FU0[0][i + 1] - FU0[0][i]) +
        ...
        // if(FTVD.DeltaU[k][i]!=0) printf("%lf ",FTVD.DeltaB[k][i] /
        // FTVD.DeltaU[k][i] ); printf("\n ");
        // }
...
double TTubo::GetArea(int i) const { return FArea[i]; }

double TTubo::GetAsonido(int i) const { return FAsonido0[i]; }

double TTubo::GetCoefTurbulencia(int i) const { return FCoefTurbulencia[i]; }

double TTubo::GetCpMezcla(int i) const { return FCpMezcla[i]; }

double TTubo::GetCvMezcla(int i) const { return FCvMezcla[i]; }

double TTubo::getDeltaTime() const { return FDeltaTime; }

double TTubo::GetDensidad(int i) const { return Frho[i]; }

double TTubo::GetDiametro(int i) const { return FDiametroTubo[i]; }

double TTubo::GetFraccionMasicaInicial(int i) const {
  return FComposicionInicial[i];
}

double TTubo::GetGamma(int i) const { return FGamma[i]; }

double TTubo::getLongitudTotal() const { return FLongitudTotal; }

double TTubo::getMallado() const { return FMallado; }

int TTubo::getNodoDer() const { return FNodoDer; }

int TTubo::getNodoIzq() const { return FNodoIzq; }

int TTubo::getNumeroTubo() const { return FNumeroTubo; }

double TTubo::GetPresion(int i) const { return FPresion0[i]; }

double TTubo::getPresionInicial() const { return FPini; }

double TTubo::GetRMezcla(int i) const { return FRMezcla[i]; }

double TTubo::getTemperaturaInicial() const { return FTini; }

double TTubo::getTempWallIni() const { return FTIniParedTub; }

double TTubo::getTime0() const { return FTime0; }

double TTubo::getTime1() const { return FTime1; }
...
double TTubo::GetTPTuboAnt(int j, int i) const { return FTParedAnt[j][i]; }

double TTubo::GetVelocidad(int i) const { return FVelocidad0[i]; }

double TTubo::getVelocidadMedia() const { return FVelMedia; }

double TTubo::GetVelPro(int i) const { return FVelPro[i]; }

void TTubo::PutDeltaTime(double valor) { FDeltaTime = valor; }

void TTubo::PutTime0(double valor) { FTime0 = valor; }

void TTubo::PutTime1(double valor) { FTime1 = valor; }

void TTubo::PutTPTubo(int k, int i, double valor) { FTPTubo[k][i] = valor; }

void TTubo::PutVelPro(int i, double valor) { FVelPro[i] = valor; }
