...
protected:
  ...
  double *FRg_int_ext; // Resistencia de convecci�n del tubo interior al gas de
                       // la c�mara de aire
  double *FRg_ext_int; // Resistencia de convecci�n del tubo exterior al gas de
                       // la c�mara de aire
  double *FR_ext;      // Resistencia del tubo exterior con el medio ambiente
  double *FR_int_radiacion; // Resistencia por radiaci�n entre las parades del
                            // tubo interior y las del tubo exterior
  double *FR_int_RadExt;    // Resistencia radial del tubo interno entre el nodo
                            // medio y el exterior
  double
      *FR_int_AxiAnt; // Resistencia axial del tubo interno con el nodo anterior
  double *
      FR_int_AxiPos; // Resistencia axial del tubo interno con el nodo posterior
  double *FR_int_RadInt; // Resistencia radial del tubo interno entre el nodo
                         // medio y el interior
  double *FR_ext_RadExt; // Resistencia radial del tubo externo entre el nodo
                         // medio y el exterior
  double
      *FR_ext_AxiAnt; // Resistencia axial del tubo externo con el nodo anterior
  double *
      FR_ext_AxiPos; // Resistencia axial del tubo externo con el nodo posterior
  double *FR_ext_RadInt; // Resistencia radial del tubo externo entre el nodo
                         // medio y el interior
...
public:
  ...
  virtual void CalculaTemperaturaPared(
      ...
      const std::vector<std::unique_ptr<TCondicionContorno>> &CC) = 0;

  virtual void CalculaTemperaturaParedSinMotor(
      const std::vector<std::unique_ptr<TCondicionContorno>> &CC) = 0;

  virtual void CalculaResistenciasdePared(
      const std::vector<std::unique_ptr<TCondicionContorno>> &CC) = 0;
