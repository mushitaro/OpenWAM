...
inline int matherr(struct exception *a) { return 0; }

// ENUM�s

// ! Different ways to define how the turbine discharge coefficients are
// calculated
enum nmTurbineType {
  nmFixedTurbine,     /* !< The turbine discharge coefficients are constants. */
  nmVariableGeometry, /* !< The turbine discharge coefficients are modified
                         externally. */
  nmTurbineMap /* !< The turbine discharge coefficients are obtained from the
                  turbine map. */
};
...
enum nmAxisSpeedCalculation {
  nmFixed =
      0, /* !< The turbocharger speed is constant during all the dimulation */
  nmVariable = 1 /* !< The turbocharger speed is calculated depending on the
                    turbina and compressor work balance */
};
...
enum nmTipoDescarga {
  nmDescargaAtmosfera = 0,
  nmDescargaRemanso = 1,
  nmDescargaRemansoMatlab = 2
};

enum nmTipoValv { nmValvAdmision = 0, nmValvEscape = 1 };
...
enum nmTipoPP { nmPPLineal = 0, nmPPCuadratica = 1 };

enum nmSentidoFlujo { nmEntrante = 0, nmSaliente = 1, nmParado = 2 };
...
enum nmAjusteRdTurbina { nmConstante = 0, nmCalExtARD = 1 };

enum nmVolumenDeposito { nmDepVolCteBase = 0, nmDepVolVariable = 1 };
...
enum nmTipoGas { nmAireFresco = 0, nmGasEscape = 1 };

// enum nmExtremoTubo {
//	nmLeft = 0, nmRight = 1
// };

enum nmModelo { nmLaxWendroff = 0, nmMacCormak = 1, nmCESE = 2, nmTVD = 3 };

enum nmSubModelo { nmNinguno = 0, nmFCT = 1 };
...
enum nmFormulacionLeyes { nmSimple = 0, nmSinArea = 1, nmConArea = 2 };
...
enum nmRefrigerante { nmAire = 0, nmAgua = 1 };

enum nmTipoMallado { nmDistancia = 1, nmAngular = 2 };

enum nmTipoMotor { nm2T = 0, nm4T = 1 };

enum nmTipoDesfase { nmPersonalizado = 0, nmImpuesto = 1 };

enum nmTipoCombustion { nmMEC = 0, nmMEP = 1 };

enum nmCalculoPAAE { nmPAAECalculada = 0, nmPAAEImpuesta = 1 };

enum nmCalculoPared { nmSinInercia = 0, nmConInercia = 1, nmTempFija = 2 };
...
enum nmRegimenValv { nmPropio = 0, nmMotor = 1 };
...
enum nmTipoCalculoEspecies { nmCalculoSimple = 0, nmCalculoCompleto = 1 };
...
enum nmCalculoGamma {
  nmGammaConstante = 0,
  nmComposicion = 1,
  nmComposicionTemperatura = 2
};

enum nmCombustion { nmFQL = 0, nmEXT = 1, nmACT = 2 };

enum nmTipoControl { nmControlPadm = 0, nmControlMasaAire = 1 };
...
enum nmInputObjet { nmInController = 0, nmInSensor = 1 };
...
enum nmTipoInterpolacion { nmLineal = 0, nmHermite = 1, nmSteps = 2 };

enum nmCompressorMapFormat { nmOldWAMmap = 0, nmSAMmap = 1 };
...
enum nmTipoCanal { nmCanalEntrada = 0, nmCanalSalida = 1 };
...
template <class T> void DestruyeVector(T *pointer) {
  if (pointer != NULL) {
    delete[] pointer;
    pointer = NULL;
  }
...
template <class T> void DestruyeMatriz(T **pointer, int num) {
  if (pointer != NULL) {
    for (int i = 0; i < num; i++) {
      delete[] pointer[i];
    }
    ...
    if (GammaCalculation == nmComposicionTemperatura) {
      ...
      if (TipoCombustible == nmMEC) {
        ...
        CvCombustible =
            ...
            Temperature *
                (6.95372 +
                 Temperature * (-0.00404715 + Temperature * 0.000000910259)) +
            1458487 / (Temperature * Temperature);
      } else if (TipoCombustible == nmMEP) {
        // Octano from NIST data base (1 bar).
        CvCombustible =
            ((4.200E-07 * Temperature - 2.524E-03) * Temperature + 5.834E+00) *
                Temperature +
            1.890E+02 - __R::Gasoline;
      }
    }
    // CvMezcla = CvQuemados * YQuemados + CvCombustible * YCombustible +
    // (CvAire * (1 - YCombustible - YQuemados - 0.0164) + 0.0164 * CvH2O); Sin
    // Humedad en aire
    CvMezcla = CvQuemados * YQuemados + CvCombustible * YCombustible +
...
inline double CalculoSimpleRMezcla(double YQuemados, double YCombustible,
  ...
  if (GammaCalculation != nmGammaConstante) {
    // R = __R::Burnt * YQuemados + RFuel * YCombustible + (__R::Air * (1 -
    // YQuemados - YCombustible - 0.0164) + 0.0164 * __R::H2O); Sin humedad en
    // aire
  ...
  if (GammaCalculation != nmGammaConstante) {
    ...
    if (GammaCalculation == nmComposicionTemperatura) {
      ...
      if (TipoCombustible == nmMEC) {
        ...
        //				CpCombustible = 4184
        //					* (-24.078 + Temperature *
        //(0.25663 - Temperature * (0.00020168 + 0.00000006475 * Temperature))
        //						+ 580800 * RaizdeT) *
        //__R::Gasoline / __R::Universal; //cv = cp - R
        CpCombustible =
            ((4.200E-07 * Temperature - 2.524E-03) * Temperature + 5.834E+00) *
                Temperature +
            1.890E+02;
...
inline double CalculoCompletoRMezcla(double YO2, double YCO2, double YH2O,
  ...
  if (GammaCalculation != nmGammaConstante) {
    R = __R::O2 * YO2 + __R::CO2 * YCO2 + __R::H2O * YH2O +
        RFuel * YCombustible +
...
inline double Interpolacion_bidimensional(double xref, double yref,
                                          double *Mapa_fila, double *Mapa_col,
                                          double **Mapa, int dimension_x,
                                          int dimension_y) {
  int y1, y2, x1, x2;
  ...
  if (yref < Mapa_fila[0]) {
    ...
    if (xref < Mapa_col[0]) {
      ...
      Valor_mapa_fila_yref_columna_xref =
          Mapa[0][x1] + (Mapa[0][x1] - Mapa[0][x2]) * (Mapa_col[x1] - xref) /
    ...
    if (xref < Mapa_col[0]) {
      Valor_mapa_fila_yref_columna_xref =
          Mapa[y1][0] - (Mapa[y1][0] - Mapa[y2][0]) * (Mapa_fila[y1] - yref) /
                            (Mapa_fila[y1] - Mapa_fila[y2]);
    } else if (xref >= Mapa_col[dimension_x - 1]) {
      Valor_mapa_fila_yref_columna_xref =
          Mapa[y1][dimension_x - 1] -
      ...
      Valor_mapa_fila_yref_columna_x1 =
          Mapa[y1][x1] - (Mapa[y1][x1] - Mapa[y2][x1]) *
                             (Mapa_fila[y1] - yref) /
                             (Mapa_fila[y1] - Mapa_fila[y2]);
      Valor_mapa_fila_yref_columna_x2 =
          Mapa[y1][x2] - (Mapa[y1][x2] - Mapa[y2][x2]) *
                             (Mapa_fila[y1] - yref) /
                             (Mapa_fila[y1] - Mapa_fila[y2]);

      /* A continuacion se interpola el valor entre las filas del mapa, para
       * obtener el (valor en xref,yref) */
      Valor_mapa_fila_yref_columna_xref =
          Valor_mapa_fila_yref_columna_x1 -
...
inline double EvaluaHermite(double punto, int n, double *x, double *y,
  ...
  if (punto <= x[0]) {
    ...
    h11 = t3 - t2;
    ret_val =
        h00 * y[k - 1] + h * h10 * sol[k - 1] + h01 * y[k] + h * h11 * sol[k];
  }
...
inline void GetName(const char *original, char *destination, const char *add) {
  int contpunto = (int)strlen(original);
  for (int i = (int)strlen(original) - 1; i >= 0; i--) {
    if (original[i] == '.') {
      contpunto = i;
      break;
...
//	// throw
// Exception(std::to_string(a->name)+"("+std::to_string(a->arg1)+","+std::to_string(a->arg2)+")");
//
//	if (a->arg2 != 0) {
//		MessageBeep(2);
//	}
