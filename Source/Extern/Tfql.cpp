...
Tfql::Tfql(int ncilin) { Fncilin = ncilin; }
...
void Tfql::calcula_angulos_combustion(double *parametros, int i) {
  ...
  try {
    ...
    for (int j = 0; j < Fnley; ++j) {
      ...
      egr =
          pow2((parametros[7] - Fpar_dist[7][j]) / Fmax[7]); // Massflow de EGR
      densidad =
          pow2((parametros[8] - Fpar_dist[8][j]) / Fmax[8]); // Density en 2
