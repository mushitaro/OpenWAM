...
TTGV::~TTGV() { delete[] FDistanciasVastago; }

//---------------------------------------------------------------------------
//---------------------------------------------------------------------------

void TTGV::LeeDatosEntrada(const std::string &Ruta, std::istream &FileInput) {
  std::string FileTGV;

  try {
    FileInput >> FileTGV;
    std::string FullPath = Ruta + FileTGV;

    std::ifstream FichTGV(FullPath.c_str());
    if (!FichTGV.is_open()) {
      std::cout << "ERROR: Fichero de TGV no cargado: " << FullPath
                << std::endl;
    } else {
      FichTGV >> FKc1 >> FKi1 >> FKd1;
      FichTGV >> FKc2 >> FKi2 >> FKd2;
      FichTGV >> FCorr0 >> FCorr1 >> FCorr2;
      FichTGV >> FCorr4 >> FCorr6 >> FCorr8;
      FichTGV >> FCorr10 >> FCorr12;
      FichTGV >> FNumeroDatos_Regimen >> FNumeroDatos_Mf;
      ...
      for (int i = 0; i < FNumeroDatos_Regimen; i++) {
        FichTGV >> FVector_Regimen_mapa[i];
      }
      for (int i = 0; i < FNumeroDatos_Mf; i++) {
        FichTGV >> FVector_Mf_mapa[i];
      }
      for (int i = 0; i < FNumeroDatos_Mf; i++) {
        for (int j = 0; j < FNumeroDatos_Regimen; j++) {
          FichTGV >> FMapa_PosicionTurbina[i][j];
        }
      }
      for (int i = 0; i < FNumeroDatos_Mf; i++) {
        for (int j = 0; j < FNumeroDatos_Regimen; j++) {
          FichTGV >> FMapa_Padmision[i][j];
        }
      }
    }
...
void TTGV::CalculaTurbina(nmTipoControl TipoControl, double MasaFuel,
                          ...
                          double RegTurboCorr, double GastoCorr,
                          double Tiempo) {
  try {
    int contador = 0, x1, x2;
    double Rendimiento1 = 0., Rendimiento2 = 0., AefecEstator = 0.,
           AefecRotor = 0.;
    if (TipoControl == nmControlPadm) {
      ...
      FPresionAdmisionConsigna =
          Interpolacion_bidimensional(Regimen, MasaFuel, FVector_Mf_mapa,
                                      FVector_Regimen_mapa, FMapa_Padmision,
      ...
      FPosicionTurbina =
          FPosicionTurbina + FP + FI +
          FD; // Posicion de la turbina  en % (100% cerrada - 0% abierta)
      ...
      FPosicionTurbina = Interpolacion_bidimensional(
    ...
    if (x1 == 0) { // Distancia del vastago 0 mm
      ...
      Rendimiento1 =
          (0.6491229 * pow3(RelacionCinematica) -
           2.646368 * pow2(RelacionCinematica) + 2.2409 * RelacionCinematica);
      Rendimiento2 =
          (-0.316970 * pow3(RelacionCinematica) -
           1.5838 * pow2(RelacionCinematica) + 2.1257 * RelacionCinematica);
      ...
      Rendimiento1 =
          (-0.316970 * pow3(RelacionCinematica) -
           1.5838 * pow2(RelacionCinematica) + 2.1257 * RelacionCinematica);
      Rendimiento2 =
          (-0.20288 * pow3(RelacionCinematica) -
           1.457777 * pow2(RelacionCinematica) + 2.0414 * RelacionCinematica);
      ...
      Rendimiento1 =
          (-0.20288 * pow3(RelacionCinematica) -
           1.457777 * pow2(RelacionCinematica) + 2.0414 * RelacionCinematica);
      Rendimiento2 =
          (-0.17608 * pow3(RelacionCinematica) -
           1.37028 * pow2(RelacionCinematica) + 1.8979 * RelacionCinematica);
      ...
      Rendimiento1 =
          (-0.17608 * pow3(RelacionCinematica) -
           1.37028 * pow2(RelacionCinematica) + 1.8979 * RelacionCinematica);
      Rendimiento2 =
          (0.00635 * pow3(RelacionCinematica) -
           1.5031 * pow2(RelacionCinematica) + 1.79457 * RelacionCinematica);
      ...
      Rendimiento1 =
          (0.00635 * pow3(RelacionCinematica) -
           1.5031 * pow2(RelacionCinematica) + 1.79457 * RelacionCinematica);
      Rendimiento2 =
          (0.39258 * pow3(RelacionCinematica) -
           1.90361 * pow2(RelacionCinematica) + 1.7190 * RelacionCinematica);
    ...
    AefecEstator = -1.818 * pow2(FDistanciaVastago) + 54.4 * FDistanciaVastago +
                   73.88; // en (mm2)
    ...
    AefecRotor = 59.9801 * GastoCorr - 2.79195 * RegTurboCorr +
                 ...
                 35.9154 + 10.1818 * pow(RelExp, 2.) -
                 29.9162 * RelExp; // en (mm2)
