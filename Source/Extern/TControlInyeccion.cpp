...
void TControlInyeccion::LeeDatosEntrada(const std::string &Ruta,
                                        std::istream &FileInput) {
  std::string FileInyeccion;

  try {
    FileInput >> FileInyeccion;
    std::string FullPath = Ruta + FileInyeccion;

    std::ifstream FichInyeccion(FullPath.c_str());
    if (!FichInyeccion.is_open()) {
      std::cout << "ERROR: Fichero de con datos del inyeccion no cargado: "
                << FullPath << std::endl;
    } else {
      FichInyeccion >> FNumeroDatos_Prail_Regimen >> FNumeroDatos_Mf_Prail >>
      ...
      for (int i = 0; i < FNumeroDatos_Prail_Regimen; i++) {
        FichInyeccion >> FVector_Prail_Regimen_mapa[i];
      }
      for (int i = 0; i < FNumeroDatos_Regimen; i++) {
        FichInyeccion >> FVector_Regimen_mapa[i];
      }
      for (int i = 0; i < FNumeroDatos_Mf; i++) {
        FichInyeccion >> FVector_Mf_mapa[i];
      }
      for (int i = 0; i < FNumeroDatos_Mf_Prail; i++) {
        FichInyeccion >> FVector_Mf_mapaPrail[i];
      }
      for (int i = 0; i < FNumeroDatos_Mf_Prail; i++) {
        for (int j = 0; j < FNumeroDatos_Prail_Regimen; j++) {
          FichInyeccion >> FMapa_Prail[i][j];
        }
      }
      for (int i = 0; i < FNumeroDatos_Mf; i++) {
        for (int j = 0; j < FNumeroDatos_Regimen; j++) {
          FichInyeccion >> FMapa_SOI_Principal[i][j];
        }
      }
      for (int i = 0; i < FNumeroDatos_Mf; i++) {
        for (int j = 0; j < FNumeroDatos_Regimen; j++) {
          FichInyeccion >> FMapa_SOI_Piloto[i][j];
        }
      }
      for (int i = 0; i < FNumeroDatos_Mf; i++) {
        for (int j = 0; j < FNumeroDatos_Regimen; j++) {
          FichInyeccion >> FMapa_Combustible_Piloto[i][j];
        }
      }
    }
    FSOI = new double[2];
...
void TControlInyeccion::CalculaSistemaInyeccion(double MasaFuel,
  ...
  try {
    ...
    // a realizar para realizar el control sobre el sistema de inyeccion, de
    // acuerdo a los requerimientos de ACT. Actualmente ACT necesita como datos
    // para cada ciclo: 1.- Numero de inyecciones. 2.- Presion de inyeccion. 3.-
    // ACT puede calcular hasta con 8 inyecciones de combustible. Para cada una
    // de
    //     ellas se ha de definir el CrankAngle de begining y la masa inyectada.
    //     Hay que dar estos datos para las 8, de modo que aquellas que no son
    //     utilizadas deben tener como angulo de begining el valor 180 y como
    //     masa inyectada 0, de acuerdo a lo que el manual de ACT nos indica.

    FNumeroInyecciones = 2;
    MasaFuel =
        MasaFuel *
        1e6; // En los mapas la masa de fuel esta en mg/cc. En ACT tambien,
    // luego se pasara el dato ya en estas unidades.

    Prail = Interpolacion_bidimensional(
        Regimen, MasaFuel, FVector_Mf_mapaPrail, FVector_Prail_Regimen_mapa,
    ...
    FSOI[1] = Interpolacion_bidimensional(
    ...
    FMasaFuel[0] = Interpolacion_bidimensional(
        Regimen, MasaFuel, FVector_Mf_mapa, FVector_Regimen_mapa,
    ...
    FDiferencia_SOI = Interpolacion_bidimensional(
        Regimen, MasaFuel, FVector_Mf_mapa, FVector_Regimen_mapa,
