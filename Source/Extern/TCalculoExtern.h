...
enum nmTipoSenTG { nmRTCTG = 0 };

enum nmTipoSenTurbina { nmRelaci1 = 0, nmRelaci2 = 1 };

enum nmTipoSenCil { nmPrCil = 0, nmTempCil = 1, nmMfCil = 2, nmAFRCil = 3 };
...
enum nmTipoSenMotor { nmAngulo = 0, nmReg = 1, nmTiempo = 2 };

enum nmTipoSenUED { nmGasto = 0 };
...
private:
  ...
  TRemansoMatlab **FRemansoMatlab; // Objeto para el calculo de la presion y
                                   // temperatura en un deposito de remanso.
  TCoefDescarga **FCoefDescarga;   // Objeto para el calculo del coeficiente de
                                   // descarga de una union a deposito/cilindro
  TAjusteTransCalorCil
      *FAjusteTransCalorCil; // Objeto para el calculo de los coeficientes de
                             // ajuste de transmision de calor en admision y
                             // escape
  TFraccionMasicaCilindro
      *FFraccionMasicaCilindro; // Objeto para imponer la fraccion masica a la
                                // apertura del cilindro.
  TControlInyeccion
      *FControlInyeccion; // Objeto para determinar el numero de inyecciones, la
                          // presion de inyeccion y la duracion y masa inyectada
                          // en cada inyeccion.
  TControlK **FControlK;  // Objeto para calcular la variacion de K con el Re en
                          // cada CC de Perdida de Presion
  char *FRutaTrabajo;

  // Indice que indica la posicion de cada especie en el vector de fraccion
  // masica.
  ...
  // Variables para la determinacion de la correlacion utilizada en las
  // turbinas.
...
public:
  ...
  // ---------------------------------------------------------------------------
  // Variables
  bool getajustbaraba() { return Fajustbaraba; };

  // Funciones
  double GetOutput_dll(int i);

  bool getcontrolmfcomb() { return Fcontrolmfcomb; };

  int getngrafmat() { return Fngrafmat; };

  bool getmodcomb() { return Fmodcomb; };

  bool getcontroliny() { return Fcontroliny; };

  bool getFraccionMasicaEspeciesCil() { return FFraccionMasicaEspeciesCil; };
  ...
  TControlFuel *getFuel() { return GetFuel(); };

  bool getConvergencia() { return FConvergencia; };
  ...
  void Lee_Sens_Motor(const char *FileWAM, fpos_t &filepos,
                      const std::vector<std::unique_ptr<TBloqueMotor>> &Engine,
  ...
  void Calculo_Sensores_Tubos(const std::vector<std::unique_ptr<TTubo>> &Pipe,
                              double deltaT);

  void Calculo_Sensores_Deposito(
      const std::vector<std::unique_ptr<TDeposito>> &Plenum, double deltaT);

  void
  Calculo_Sensores_TG(double deltaT,
                      const std::vector<std::unique_ptr<TEjeTurbogrupo>> &Axis);

  void Calculo_Sensores_Turbina(double deltaT,
                                const std::vector<TTurbina *> &Turbine);

  void Calculo_Sensores_Cilindro(
      double deltaT, const std::vector<std::unique_ptr<TBloqueMotor>> &Engine);

  void Calculo_Sensores_Venturi(double deltaT,
                                const std::vector<TVenturi *> &Venturi);

  void Calculo_Sensores_Motor(
      double deltaT, const std::vector<std::unique_ptr<TBloqueMotor>> &Engine,
      double AcumulatedTime);

  void Calculo_Sensores_UED(
      double deltaT,
      const std::vector<std::unique_ptr<TCondicionContorno>> &BC);
