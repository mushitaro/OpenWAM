...
TCalculoExtern::TCalculoExtern() {
  ...
  // ATENCION: Los siguiente valores determinan el numero de entradas y salidas
  // a Matlab Debes modificar estos numeros segun tus requerimientos.
...
void TCalculoExtern::LlamadaECU(
    double DeltaT, const std::vector<std::unique_ptr<TBloqueMotor>> &Engine) {
  try {

    FTiempo += DeltaT;

    // ATENCION: Asignar los valores que provienen de los sensores. El orden es
    // muy importante.

    /* FInputs[0]=FSensorTubo[0].valact;   // "Presion a la entrada de la
     turbina" FInputs[1]=FSensorTubo[1].valact;   // "Temperature a la entrada
     de la turbina" FInputs[2]=FSensorTubo[2].valact;   // "Massflow masico de
     EGR" FInputs[3]=FSensorTubo[3].valact;   // "Temperature a la salida del
     intercooler" FInputs[4]=FSensorTubo[4].valact;   // "Massflow masico antes
     del intercooler" FInputs[5]=FSensorTubo[5].valact;   // "Presion a la
     salida del intercooler (antes de la ramificacion en el colector"
     FInputs[6]=FSensorMotor[0].valact;  // "Regimen de giro del motor"
     FInputs[7]=FSensorMotor[1].valact;  // "Instante de tiempo del motor"    */

    //     if(Fhayecu){
    //
    //		  //ATENCION: A continuacion se deben asignar los valores de
    // entrada de MATLAB
    //		  //-----------------------------------------------------------------------------
    //		  //
    //		  //-----------------------------------------------------------------------------
    ...
    //			   {
    //					ECU->Input[i]=FEntrada[i];  /*Al igual
    // que el numero de entradas a Matlab (FNumeroInMat), los valores de
    // FEntrada han de ser dados por cada usuario)*/
    //			   }
    //			   ECU->CalculaECU();
    //
    //			   //ATENCION: A continuacion se deben asignar los
    // valores de salida de MATLAB
    //			   //-----------------------------------------------------------------------------
    ...
    /*  if(FInputs[7]>0.6){
     if(fabs(FTGV[0]->ErrorP2)<0.03 & fabs(FTGV[0]->ErrorP3)<0.03 &
     fabs(FEGRV->ErrorEGR)<0.01 &
     fabs(FAjusteTransCalorCil->getErrorGastoAire())<0.03){ FConvergencia=true;
     }
     }
     }    */
    ...
    //**** CALCULO DE LOS COEFICIENTES DE TRANSMISION DE CALOR EN ADMISION Y
    // ESCAPE****//
...
double TCalculoExtern::GetOutput_dll(int i) {
  try {
    if (i < FNOutputsdll) {
      ...
      std::cout << "WARNING: El valor de la salida de la dll se sale de rango"
                << std::endl;
...
TRemansoMatlab *TCalculoExtern::GetTRemansoMatlab(int i) {
  try {
    ...
    //   }else{
    //       std::cout << "WARNING: El valor de la salida de la dll se sale de
...
void TCalculoExtern::LeeFicherosDLL(const char *FileWAM, fpos_t &filepos,
                                    int controlvalv, int nematlab, int ncilin,
...
void TCalculoExtern::Lee_Sens_Tubos(
        ...
        for (int j = 0; j <= nsensortubo - 1; ++j) {
          ...
          case nmTempTubo:
            ...
            FSensorTubo[cont].valreal = __units::KTodegC(
                temp * temp / Pipe[t]->GetGamma(0) /
                Pipe[t]->GetRMezcla(0)); // Como es el instante inicial da igual
                                         // el nodo a que se hace referencia
          ...
          case nmVelTubo:
            ...
            break;
          case nmGastoTubo:
            temp = Pipe[t]->GetAsonido(0) *
                   __cons::ARef; // Como es el instante inicial el nodo es
                                 // arbitrario
            ...
            FSensorTubo[cont].valreal =
                __geom::Circle_area(Pipe[t]->GetDiametro(n1)) * v1 * den;
            FSensorTubo[cont].valact = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valant = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valrealant = FSensorTubo[cont].valreal;
            break;
          case nmN2Tubo:
            if (SpeciesModel == nmCalculoCompleto) {
              ...
              printf("No puede haber sensor de N2 si el calculo de especies no "
                     "es completo. Revisa la lectura.\n");
              throw Exception("");
            }
            FSensorTubo[cont].valreal =
                Pipe[t]->GetFraccionMasica(0, FIndiceTuboN2);
            FSensorTubo[cont].valact = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valant = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valrealant = FSensorTubo[cont].valreal;
            break;
          case nmO2Tubo:
            if (SpeciesModel == nmCalculoCompleto) {
              ...
              printf("No puede haber sensor de O2 si el calculo de especies no "
                     "es completo. Revisa la lectura.\n");
              throw Exception("");
            }
            FSensorTubo[cont].valreal =
                Pipe[t]->GetFraccionMasica(0, FIndiceTuboO2);
            FSensorTubo[cont].valact = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valant = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valrealant = FSensorTubo[cont].valreal;
            break;
          case nmCO2Tubo:
            if (SpeciesModel == nmCalculoCompleto) {
              ...
              printf("No puede haber sensor de CO2 si el calculo de especies "
                     "no es completo. Revisa la lectura.\n");
              throw Exception("");
            }
            FSensorTubo[cont].valreal =
                Pipe[t]->GetFraccionMasica(0, FIndiceTuboCO2);
            FSensorTubo[cont].valact = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valant = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valrealant = FSensorTubo[cont].valreal;
            break;
          case nmH2OTubo:
            if (SpeciesModel == nmCalculoCompleto) {
              ...
              printf("No puede haber sensor de H2O si el calculo de especies "
                     "no es completo. Revisa la lectura.\n");
              throw Exception("");
            }
            FSensorTubo[cont].valreal =
                Pipe[t]->GetFraccionMasica(0, FIndiceTuboH2O);
            FSensorTubo[cont].valact = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valant = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valrealant = FSensorTubo[cont].valreal;
            break;
          case nmHCTubo:
            if (SpeciesModel == nmCalculoCompleto) {
              ...
              printf("No puede haber sensor de HC si el calculo de especies no "
                     "es completo. Revisa la lectura.\n");
              throw Exception("");
            }
            FSensorTubo[cont].valreal =
                Pipe[t]->GetFraccionMasica(0, FIndiceTuboHC);
            FSensorTubo[cont].valact = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valant = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valrealant = FSensorTubo[cont].valreal;
            break;
          case nmSootTubo:
            if (SpeciesModel == nmCalculoCompleto) {
              ...
              printf("No puede haber sensor de Soot si el calculo de especies "
                     "no es completo. Revisa la lectura.\n");
              throw Exception("");
            }
            FSensorTubo[cont].valreal =
                Pipe[t]->GetFraccionMasica(0, FIndiceTuboSoot);
            FSensorTubo[cont].valact = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valant = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valrealant = FSensorTubo[cont].valreal;
            break;
          case nmNOxTubo:
            if (SpeciesModel == nmCalculoCompleto) {
              ...
              printf("No puede haber sensor de NOx si el calculo de especies "
                     "no es completo. Revisa la lectura.\n");
              throw Exception("");
            }
            FSensorTubo[cont].valreal =
                Pipe[t]->GetFraccionMasica(0, FIndiceTuboNOx);
            FSensorTubo[cont].valact = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valant = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valrealant = FSensorTubo[cont].valreal;
            break;
          case nmCOTubo:
            if (SpeciesModel == nmCalculoCompleto) {
              ...
              printf("No puede haber sensor de CO si el calculo de especies no "
                     "es completo. Revisa la lectura.\n");
              throw Exception("");
            }
            FSensorTubo[cont].valreal =
                Pipe[t]->GetFraccionMasica(0, FIndiceTuboCO);
            FSensorTubo[cont].valact = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valant = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valrealant = FSensorTubo[cont].valreal;
            break;
          case nmAireFrTubo:
            if (SpeciesModel == nmCalculoSimple) {
              ...
              printf("No puede haber sensor de Aire Fresco si el calculo de "
                     "especies no es simple. Revisa la lectura.\n");
              throw Exception("");
            }
            FSensorTubo[cont].valreal =
                Pipe[t]->GetFraccionMasica(0, FIndiceTuboAireFresco);
            FSensorTubo[cont].valact = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valant = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valrealant = FSensorTubo[cont].valreal;
            break;
          case nmGasQuemadoTubo:
            if (SpeciesModel == nmCalculoSimple) {
              ...
              printf("No puede haber sensor de Gas Quemado si el calculo de "
                     "especies no es simple. Revisa la lectura.\n");
              throw Exception("");
            }
            FSensorTubo[cont].valreal =
                Pipe[t]->GetFraccionMasica(0, FIndiceTuboGasQuemado);
            FSensorTubo[cont].valact = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valant = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valrealant = FSensorTubo[cont].valreal;
            break;
          case nmEGRTubo:
            if (SpeciesModel == nmCalculoCompleto) {
              if (HayCombustible) {
                if (ThereIsEGR) {
                  ...
                  printf("No puede haber sensor de EGR si no se transporta "
                         "EGR. Revisa la lectura.\n");
                  throw Exception("");
                }
              } else {
                if (ThereIsEGR) {
            ...
            FSensorTubo[cont].valreal =
                Pipe[t]->GetFraccionMasica(0, FIndiceTuboEGR);
            FSensorTubo[cont].valact = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valant = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valrealant = FSensorTubo[cont].valreal;
            break;
          case nmCombustibleTubo:
            if (SpeciesModel == nmCalculoCompleto) {
              if (HayCombustible) {
                ...
                printf("No puede haber sensor de Combustible si no se "
                       "transporta. Revisa la lectura.\n");
                throw Exception("");
              }
            } else {
              if (HayCombustible) {
            ...
            FSensorTubo[cont].valreal =
                Pipe[t]->GetFraccionMasica(0, FIndiceTuboComb);
            FSensorTubo[cont].valact = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valant = FSensorTubo[cont].valreal;
            FSensorTubo[cont].valrealant = FSensorTubo[cont].valreal;
            break;
...
void TCalculoExtern::Lee_Sens_Dep(
    const char *FileWAM, fpos_t &filepos,
...
void TCalculoExtern::Lee_Sens_TG(
    const char *FileWAM, fpos_t &filepos,
        ...
        for (int j = 0; j <= nsensturbina - 1; ++j) {
          ...
          case nmRelaci1:
            FSensorTurbina[cont].valreal =
                Turbine[FSensorTurbina[cont].turbina - 1]
                    ->GetRelacionCinematica(0);
...
void TCalculoExtern::Lee_Sens_Cil(
    const char *FileWAM, fpos_t &filepos,
        ...
        for (int j = 0; j <= nsenscilindro - 1; ++j) {
          ...
          case nmTempCil:
            FSensorCil[cont].valreal =
                Engine[0]
                    ->GetCilindro(FSensorCil[cont].cilindro - 1)
                    ->getTemperature();
      ...
      for (int i = 0; i <= nventuris - 1; ++i) {
        ...
        for (int j = 0; j <= nsensventuri - 1; ++j) {
          ...
          case nmPrGarVent:
            prent = Venturi[v]->getPressure();
            prgar =
                prent / pow(1 + Venturi[v]->getgamma3() *
                                    pow(dynamic_cast<TCCDeposito *>(
                                            Venturi[v]->getCCLateral())
                                            ->getValvula()
                                            ->getCRecuperacion(),
                                        2.),
            ...
            break;
          case nmVelEntVent:
            if (dynamic_cast<TCCDeposito *>(Venturi[v]->getCCEntrada())
                    ->getSentidoFlujo() == nmEntrante) {
              signo = 1; /* Si el flujo es entrante signo de la velocidad
                            positivo */
            } else if (dynamic_cast<TCCDeposito *>(Venturi[v]->getCCEntrada())
                           ->getSentidoFlujo() == nmSaliente) {
              signo = -1; /* Si el flujo es saliente signo de la velocidad
                             negativo */
            }
            velent = dynamic_cast<TCCDeposito *>(Venturi[v]->getCCEntrada())
                         ->getVelocity() *
                     signo * __cons::ARef;
            ...
            break;
          case nmVelLatVent:
            vellat = dynamic_cast<TCCDeposito *>(Venturi[v]->getCCLateral())
                         ->getVelocity() *
                     __cons::ARef;
            ...
            break;
          case nmGastoEntVent:
            gasent = -dynamic_cast<TCCDeposito *>(Venturi[v]->getCCEntrada())
                          ->getMassflow(); /*Massflow entrante positivo.Saliente
                                              negativo. Pedro */
            /* gasent lleva el "-" delante porque se usa el convenio de signos
             * de los depositos,que es el contrario al de las BC, de la que se
             * obtiene el massflow */
            ...
            break;
          case nmGastoLatVent:
            gaslat = dynamic_cast<TCCDeposito *>(Venturi[v]->getCCLateral())
                         ->getMassflow();
...
void TCalculoExtern::Lee_Sens_Motor(
    const char *FileWAM, fpos_t &filepos,
...
void TCalculoExtern::Lee_Sens_UED(
    const char *FileWAM, fpos_t &filepos,
    ...
    if (FNSensUED != 0) {
      for (int i = 0; i <= nUED - 1; ++i) {
        ...
        for (int j = 0; j <= nsensUED - 1; ++j) {
          ...
          case nmGasto:
            FSensorUED[cont].valreal = dynamic_cast<TCCUnionEntreDepositos *>(
                                           BC[FSensorUED[cont].CCUED - 1].get())
                                           ->getGastoImpreso();
...
void TCalculoExtern::Calculo_Sensores_Tubos(
    const std::vector<std::unique_ptr<TTubo>> &Pipe, double deltaT) {
  try {
    ...
    for (int i = 0; i <= FNSensTubos - 1; i++) {
      ...
      if (n1 == FSensorTubo[i].nodos - 1) {
        ...
        case nmTempTubo:
          ...
          FSensorTubo[i].valact =
              ((2 * FSensorTubo[i].ctetiempo - deltaT) * FSensorTubo[i].valant +
               deltaT * FSensorTubo[i].ganancia *
                   (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) /
        ...
        case nmAireFrTubo:
          FSensorTubo[i].valreal =
              Pipe[j]->GetFraccionMasica(n1, FIndiceTuboAireFresco);
          FSensorTubo[i].valact =
              ((2 * FSensorTubo[i].ctetiempo - deltaT) * FSensorTubo[i].valant +
               deltaT * FSensorTubo[i].ganancia *
                   (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) /
        ...
        case nmGasQuemadoTubo:
          FSensorTubo[i].valreal =
              Pipe[j]->GetFraccionMasica(n1, FIndiceTuboGasQuemado);
          FSensorTubo[i].valact =
              ((2 * FSensorTubo[i].ctetiempo - deltaT) * FSensorTubo[i].valant +
               deltaT * FSensorTubo[i].ganancia *
                   (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) /
        ...
        case nmTempTubo:
          temp =
              xit_(Pipe[j]->GetAsonido(n1), Pipe[j]->GetAsonido(n2), 1.0, d) *
              __cons::ARef;
          gamma = xit_(Pipe[j]->GetGamma(n1), Pipe[j]->GetGamma(n2), 1.0, d);
          Rmezcla =
              xit_(Pipe[j]->GetRMezcla(n1), Pipe[j]->GetRMezcla(n2), 1.0, d);
          FSensorTubo[i].valreal =
              __units::KTodegC(temp * temp / gamma / Rmezcla);
          FSensorTubo[i].valact =
              ((2 * FSensorTubo[i].ctetiempo - deltaT) * FSensorTubo[i].valant +
               deltaT * FSensorTubo[i].ganancia *
                   (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) /
        ...
        case nmPrTubo:
          FSensorTubo[i].valreal =
              xit_(Pipe[j]->GetPresion(n1), Pipe[j]->GetPresion(n2), 1.0, d);
          FSensorTubo[i].valact =
              ((2 * FSensorTubo[i].ctetiempo - deltaT) * FSensorTubo[i].valant +
               deltaT * FSensorTubo[i].ganancia *
                   (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) /
        ...
        case nmN2Tubo:
          FSensorTubo[i].valreal =
              xit_(Pipe[j]->GetFraccionMasica(n1, FIndiceTuboN2),
        ...
        case nmO2Tubo:
          FSensorTubo[i].valreal =
              xit_(Pipe[j]->GetFraccionMasica(n1, FIndiceTuboO2),
        ...
        case nmCO2Tubo:
          FSensorTubo[i].valreal =
              xit_(Pipe[j]->GetFraccionMasica(n1, FIndiceTuboCO2),
        ...
        case nmH2OTubo:
          FSensorTubo[i].valreal =
              xit_(Pipe[j]->GetFraccionMasica(n1, FIndiceTuboH2O),
        ...
        case nmHCTubo:
          FSensorTubo[i].valreal =
              xit_(Pipe[j]->GetFraccionMasica(n1, FIndiceTuboHC),
        ...
        case nmSootTubo:
          FSensorTubo[i].valreal =
              xit_(Pipe[j]->GetFraccionMasica(n1, FIndiceTuboSoot),
        ...
        case nmNOxTubo:
          FSensorTubo[i].valreal =
              xit_(Pipe[j]->GetFraccionMasica(n1, FIndiceTuboNOx),
        ...
        case nmCOTubo:
          FSensorTubo[i].valreal =
              xit_(Pipe[j]->GetFraccionMasica(n1, FIndiceTuboCO),
        ...
        case nmEGRTubo:
          FSensorTubo[i].valreal =
              xit_(Pipe[j]->GetFraccionMasica(n1, FIndiceTuboEGR),
        ...
        case nmCombustibleTubo:
          FSensorTubo[i].valreal =
              xit_(Pipe[j]->GetFraccionMasica(n1, FIndiceTuboComb),
...
void TCalculoExtern::Calculo_Sensores_Deposito(
    const std::vector<std::unique_ptr<TDeposito>> &Plenum, double deltaT) {
  try {
    for (int i = 0; i <= FNSensDepositos - 1; i++) {
      ...
      case nmPrDep:
        ...
        FSensorDep[i].valact =
            ((2 * FSensorDep[i].ctetiempo - deltaT) * FSensorDep[i].valant +
             deltaT * FSensorDep[i].ganancia *
                 (FSensorDep[i].valreal + FSensorDep[i].valrealant)) /
...
void TCalculoExtern::Calculo_Sensores_TG(
    double deltaT, const std::vector<std::unique_ptr<TEjeTurbogrupo>> &Axis) {
  try {
    for (int i = 0; i <= FNSensTurbogrupo - 1; i++) {
      ...
      case nmRTCTG:
        ...
        FSensorTG[i].valact =
            ((2 * FSensorTG[i].ctetiempo - deltaT) * FSensorTG[i].valant +
             deltaT * FSensorTG[i].ganancia *
                 (FSensorTG[i].valreal + FSensorTG[i].valrealant)) /
...
void TCalculoExtern::Calculo_Sensores_Turbina(
    double deltaT, const std::vector<TTurbina *> &Turbine) {
  try {
    for (int i = 0; i <= FNSensTurbina - 1; i++) {
      switch (FSensorTurbina[i].tipo) {
      case nmRelaci1:
        FSensorTurbina[i].valreal =
            Turbine[FSensorTurbina[i].turbina - 1]->GetRelacionCinematica(0);
        FSensorTurbina[i].valact =
            ((2 * FSensorTurbina[i].ctetiempo - deltaT) *
                 FSensorTurbina[i].valant +
             deltaT * FSensorTurbina[i].ganancia *
                 (FSensorTurbina[i].valreal + FSensorTurbina[i].valrealant)) /
            (2 * FSensorTurbina[i].ctetiempo + deltaT);
      ...
      case nmRelaci2:
        FSensorTurbina[i].valreal =
            Turbine[FSensorTurbina[i].turbina - 1]->GetRelacionCinematica(1);
        FSensorTurbina[i].valact =
            ((2 * FSensorTurbina[i].ctetiempo - deltaT) *
                 FSensorTurbina[i].valant +
             deltaT * FSensorTurbina[i].ganancia *
...
void TCalculoExtern::Calculo_Sensores_Cilindro(
    double deltaT, const std::vector<std::unique_ptr<TBloqueMotor>> &Engine) {
  try {
    for (int i = 0; i <= FNSensCilindros - 1; i++) {
      switch (FSensorCil[i].tipo) {
      case nmTempCil:
        FSensorCil[i].valreal = Engine[0]
                                    ->GetCilindro(FSensorCil[i].cilindro - 1)
                                    ->getTemperature();
        FSensorCil[i].valact =
            ((2 * FSensorCil[i].ctetiempo - deltaT) * FSensorCil[i].valant +
             deltaT * FSensorCil[i].ganancia *
                 (FSensorCil[i].valreal + FSensorCil[i].valrealant)) /
      ...
      case nmPrCil:
        FSensorCil[i].valreal =
            Engine[0]->GetCilindro(FSensorCil[i].cilindro - 1)->getPressure();
        FSensorCil[i].valact =
            ((2 * FSensorCil[i].ctetiempo - deltaT) * FSensorCil[i].valant +
             deltaT * FSensorCil[i].ganancia *
                 (FSensorCil[i].valreal + FSensorCil[i].valrealant)) /
      ...
      case nmMfCil:
        FSensorCil[i].valreal =
            Engine[0]->GetCilindro(FSensorCil[i].cilindro - 1)->getMasaFuel();
        FSensorCil[i].valact =
            ((2 * FSensorCil[i].ctetiempo - deltaT) * FSensorCil[i].valant +
             deltaT * FSensorCil[i].ganancia *
                 (FSensorCil[i].valreal + FSensorCil[i].valrealant)) /
      ...
      case nmAFRCil:
        FSensorCil[i].valreal =
            Engine[0]->GetCilindro(FSensorCil[i].cilindro - 1)->getAFR();
        FSensorCil[i].valact =
            ((2 * FSensorCil[i].ctetiempo - deltaT) * FSensorCil[i].valant +
             deltaT * FSensorCil[i].ganancia *
                 (FSensorCil[i].valreal + FSensorCil[i].valrealant)) /
...
void TCalculoExtern::Calculo_Sensores_Venturi(
    double deltaT, const std::vector<TVenturi *> &Venturi) {
  try {
    int signo = 0;
    double prent = 0., prgar = 0., velent = 0., vellat = 0., gasent = 0.,
           gaslat = 0.;
    ...
    for (int i = 0; i <= FNSensVenturis - 1; i++) {
      ...
      case nmPrEntVent:
        ...
        FSensorVent[i].valact =
            ((2 * FSensorVent[i].ctetiempo - deltaT) * FSensorVent[i].valant +
             deltaT * FSensorVent[i].ganancia *
                 (FSensorVent[i].valreal + FSensorVent[i].valrealant)) /
      ...
      case nmPrGarVent:
        prent = Venturi[FSensorVent[i].venturi - 1]->getPressure();
        prgar =
            prent / pow(1 + Venturi[FSensorVent[i].venturi - 1]->getgamma3() *
                                pow(dynamic_cast<TCCDeposito *>(
                                        Venturi[FSensorVent[i].venturi - 1]
                                            ->getCCLateral())
                                        ->getValvula()
                                        ->getCRecuperacion(),
                                    2.),
      ...
      case nmVelEntVent:
        if (dynamic_cast<TCCDeposito *>(
                Venturi[FSensorVent[i].venturi - 1]->getCCEntrada())
                ->getSentidoFlujo() == nmEntrante) {
          signo =
              1; /* Si el flujo es entrante signo de la velocidad positivo */
        } else if (dynamic_cast<TCCDeposito *>(
                       Venturi[FSensorVent[i].venturi - 1]->getCCEntrada())
                       ->getSentidoFlujo() == nmSaliente) {
          signo =
              -1; /* Si el flujo es saliente signo de la velocidad negativo */
        }
        velent = dynamic_cast<TCCDeposito *>(
                     Venturi[FSensorVent[i].venturi - 1]->getCCEntrada())
                     ->getVelocity() *
                 signo * __cons::ARef;
        ...
        break;
      case nmVelLatVent:
        vellat = dynamic_cast<TCCDeposito *>(
                     Venturi[FSensorVent[i].venturi - 1]->getCCLateral())
                     ->getVelocity() *
                 __cons::ARef;
      ...
      case nmGastoEntVent:
        gasent = -dynamic_cast<TCCDeposito *>(
                      ...
                      ->getMassflow(); /*Massflow entrante positivo.Saliente
                                          negativo. Pedro */
        /* gasent lleva el "-" delante porque se usa el convenio de signos de
         * los depositos,que es el contrario al de las BC, de la que se obtiene
         * el massflow */
        ...
        break;
      case nmGastoLatVent:
        gaslat = dynamic_cast<TCCDeposito *>(
                     Venturi[FSensorVent[i].venturi - 1]->getCCLateral())
                     ->getMassflow();
...
void TCalculoExtern::Calculo_Sensores_Motor(
    double deltaT, const std::vector<std::unique_ptr<TBloqueMotor>> &Engine,
  ...
  try {
    for (int i = 0; i <= FNSensMotor - 1; i++) {
      ...
      case nmAngulo:
        ...
        FSensorMotor[i].valact =
            ((2 * FSensorMotor[i].ctetiempo - deltaT) * FSensorMotor[i].valant +
             deltaT * FSensorMotor[i].ganancia *
                 (FSensorMotor[i].valreal + FSensorMotor[i].valrealant)) /
...
void TCalculoExtern::Calculo_Sensores_UED(
    double deltaT, const std::vector<std::unique_ptr<TCondicionContorno>> &BC) {
  try {
    for (int i = 0; i <= FNSensUED - 1; i++) {
      switch (FSensorUED[i].tipo) {
      case nmGasto:
        FSensorUED[i].valreal = dynamic_cast<TCCUnionEntreDepositos *>(
                                    BC[FSensorUED[i].CCUED - 1].get())
                                    ->getGastoImpreso();
        FSensorUED[i].valact =
            ((2 * FSensorUED[i].ctetiempo - deltaT) * FSensorUED[i].valant +
             deltaT * FSensorUED[i].ganancia *
                 (FSensorUED[i].valreal + FSensorUED[i].valrealant)) /
