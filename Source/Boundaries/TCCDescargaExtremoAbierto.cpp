...
TCCDescargaExtremoAbierto::TCCDescargaExtremoAbierto(
  ...
  if (TipoCC == nmOpenEndAtmosphere) {
    ...
    printf("ERROR:TCCDescargaExtremoAbierto:Asignacion Tipo BC,en la condicion "
           "de contorno: %d\n",
           FNumeroCC);
...
void TCCDescargaExtremoAbierto::AsignAmbientConditions(
    double Tamb, double Pamb, double *AtmosphericComposition) {
  ...
  if (FCalculoEspecies == nmCalculoCompleto) {
    ...
    CvMezclaDep =
        CalculoSimpleCvMezcla(__units::degCToK(FTemperaturaDep),
                              FComposicion[0], 0, FCalculoGamma, nmMEP);
    GammaDep = CalculoSimpleGamma(RMezclaDep, CvMezclaDep, FCalculoGamma);
  }
  FVelocidadSonidoDep =
      sqrt(__units::degCToK(FTemperaturaDep) * GammaDep * RMezclaDep) /
      __cons::ARef;
}
  ...
  try {
    ...
    if (FTipoDescarga == nmDescargaAtmosfera) { // DESCARGA A LA ATMOSFERA
      ...
      if (fracciontotal < 1. - 1e-10 || fracciontotal > 1. + 1e-10) {
        std::cout << "ERROR: Total mass fraction must be equal to 1. Check the "
                     "input data for boundary condition  "
                  << FNumeroCC << std::endl;
        throw Exception(" ");
      }
      if (FCalculoEspecies == nmCalculoCompleto) {
...
void TCCDescargaExtremoAbierto::CalculaCondicionContorno(double Time) {
  try {
    ...
    if (rel_CCon_entropia / yyy >= 1.000005) { // Flujo Saliente del conducto.
      ...
      *FCD = FTuboExtremo[0].Entropia * 2. * yyy - *FCC;
    ...
    } else if (rel_CCon_entropia / yyy <
               .999995) { // Flujo Entrante al conducto.
      /* ________ */
      /* caso < 1 */
      /* ________ */
      ...
      u_real =
          u_isen *
          FPerdidaExtremo; // Con esta relacion obtenemos la velocidad real.
