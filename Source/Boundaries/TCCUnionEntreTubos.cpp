...
void TCCUnionEntreTubos::ReadBoundaryData(
  ...
  try {
    ...
    /* Ahora al tubo de mayor diametro se le asignara la posicion 1 de los
     vectores y al de menor diametro la posicion 0 */
    if (Pipe[FTubo[0]]->GetDiametro(FNodoFin[0]) >=
        Pipe[FTubo[1]]->GetDiametro(FNodoFin[1])) {
      if (Pipe[FTubo[0]]->getNodoIzq() == FNumeroCC) {
        FTuboExtremo[1].Pipe = Pipe[FTubo[0]].get();
      ...
      if (Pipe[FTubo[1]]->getNodoIzq() == FNumeroCC) {
        FTuboExtremo[0].Pipe = Pipe[FTubo[1]].get();
      ...
      if (Pipe[FTubo[1]]->getNodoIzq() == FNumeroCC) {
        FTuboExtremo[1].Pipe = Pipe[FTubo[1]].get();
        FTuboExtremo[1].TipoExtremo = nmLeft;
        FNodoFin[1] = 0;
        FIndiceCC[1] = 0;
      ...
      if (Pipe[FTubo[0]]->getNodoIzq() == FNumeroCC) {
        FTuboExtremo[0].Pipe = Pipe[FTubo[0]].get();
        FTuboExtremo[0].TipoExtremo = nmLeft;
        FNodoFin[0] = 0;
        FIndiceCC[0] = 0;
...
void TCCUnionEntreTubos::CalculaCondicionContorno(double Time) {
  try {
    ...
    if (flujo < 0.999995) { /* Sentido del flujo: de 0(saliente (out)) a
      ...
      if (TuboCalculado == 1) {
        ...
        *FCC[1] = vel_sonido_In - FGamma3 * vel_In;
        FTuboExtremo[1].Entropia =
            vel_sonido_In * FTuboExtremo[1].Entropia / correc_sonido_In;
      ...
      if (FHayEGR)
        FFraccionMasicaEspecie[FNumeroEspecies - 1] =
            FTuboExtremo[0].Pipe->GetFraccionMasicaCC(FIndiceCC[0],
               ...
               1.000005) { /* Sentido del flujo: de 1(saliente) a 0(entrante) */
      ...
      // Transporte de Especies Quimicas
      // Se actualiza todos los instantes de calculo (al igual que la
      // temperatura en la BC).
      for (int j = 0; j < FNumeroEspecies - 2; j++) {
        FFraccionMasicaEspecie[j] =
...
/* void TCCUnionEntreTubos::Estrechamiento(double CCS,double CCE,double
 rel_entropia, double rel_area,double smag,double *xa1,double *xa2,double
 *xu2,double *xu1)
 {
 try
 {
 ...
 {
 std::cout << "ERROR: TCCUnionEntreTubos::Estrechamiento en la condicion de
 ...
 void TCCUnionEntreTubos::Ensanchamiento(double CCS,double CCE,double
 rel_entropia, double rel_area,double )
 {
 try
 {
 ...
 {
 std::cout << "ERROR: TCCUnionEntreTubos::Ensanchamiento en la condicion de
 contorno: " << FNumeroCC <<  std::endl; std::cout << "Tipo de error: " <<
 N.what() << std::endl; throw Exception(N.what());
 }
 }
