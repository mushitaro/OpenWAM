...
TCCUnionEntreDepositos::TCCUnionEntreDepositos(
    nmTypeBC TipoCC, int numCC, nmTipoCalculoEspecies SpeciesModel,
...
void TCCUnionEntreDepositos::CalculaCoeficientesDescarga(double TiempoActual,
  ...
  try {
    ...
    if (FValvula->getTypeOfValve() == nmLumbrera2T) {
      ...
      FDeltaAngulo = 360. * FRegimen *
                     dynamic_cast<TDepVolVariable *>(FDeposito1)
                         ->getRelacionVelocidades() /
                     60. * FDeltaT;
    ...
    if (FCDEntrada > 2.0 || FCDEntrada < 0.0) {
      printf("ERROR: TCCUnionEntreDepositos::CalculaCoeficientesDescarga, en "
             "calculo coeficiente descarga entrante: %lf, en %lf grados,en la "
             "condicion de contorno: %d\n",
             FCDEntrada, FAnguloActual, FNumeroCC);
      throw Exception(
          "ERROR:TCCUnionEntreDepositos::CalculaCoeficientesDescarga en "
          "calculo coeficiente descarga entrante: " +
          std::to_string(FCDEntrada) + ", en " + std::to_string(FAnguloActual) +
          " grados ");
    }
    if (FCDSalida > 1.0 || FCDSalida < 0.0) {
      printf("ERROR: TCCUnionEntreDepositos::CalculaCoeficientesDescarga, en "
             "calculo coeficiente descarga saliente: %lf, en %lf grados, en la "
             "condicion de contorno: %d\n",
...
double TCCUnionEntreDepositos::InterpolaDeposito(double vizq, double vder,
  ...
  try {
    ...
    if (axid != 0.) {
      ...
      std::cout << "ERROR: valores entrada "
                   "TCCUnionEntreDepositos::InterpolaDeposito: "
                << std::endl;
      throw Exception(" ");
    }
    return ret_val;
...
void TCCUnionEntreDepositos::CalculaCondicionContorno(double Time) {
  try {
    ...
    if (FIndependiente) {
      ...
      /* FGasto es el valor del massflow en modulo. Asi lo necesitan recibir los
       depositos, que ademas conocen el signo por el que han de multiplicar este
       valor. */
      /* FGastoImpreso es el valor del massflow con el signo de acuerdo al
       convenio de WAMer. Es decir, massflow positivo en el sentido del deposito
       con la union (deposito 1) al deposito al que llega el hilo (deposito 2).
       Este massflow es el que se imprime como resultado y el que se pasa a los
       sensores. */

      if (FDeposito1->getPressure() >
          FDeposito2->getPressure()) { /* Flujo del deposito 1 al deposito 2 */
        ...
        FGastoImpreso = FGasto; /* Al imprimir resultados se considera negativo
         si va del deposito del que sale el hilo hacia el que tiene la union */
        ...
        if (FHayEGR)
          FFraccionMasicaEspecie[FNumeroEspecies - 1] =
              FDeposito1->GetFraccionMasicaEspecie(FNumeroEspecies - 1);
        ...
        if (FHayEGR)
          FFraccionMasicaEspecie[FNumeroEspecies - 1] =
              FDeposito2->GetFraccionMasicaEspecie(FNumeroEspecies - 1);
      }
...
void TCCUnionEntreDepositos::CalculaUED() {
  try {
    ...
    if (FPresionDep1 > FPresionDep2) { /* Flujo del deposito 1 al deposito 2 */
      ...
      FSentidoFlujoED1 = -1;  /* Saliente */
      FSentidoFlujoED2 = 1;   /* Entrante */
      FGastoImpreso = FGasto; /* Al imprimir resultados se considera negativo si
       va del deposito del que sale el hilo hacia el que tiene la union */
    ...
    } else if (FPresionDep2 >
               FPresionDep1) { /* Flujo del deposito 2 al deposito 1 */
      FGasto = FCDSalida * gasto_isen;
      FSentidoFlujoED1 = 1;  /* Entrante */
      FSentidoFlujoED2 = -1; /* Saliente */
      FGastoImpreso = -FGasto;
