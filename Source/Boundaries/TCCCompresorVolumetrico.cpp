...
void TCCCompresorVolumetrico::CalculaCondicionContorno(double Time) {
  try {
    ...
    FDensidad = __units::BarToPa(FPresionCV) /
                (FRMezcla * __units::degCToK(FTemperaturaCV));
    ...
    FSonido =
        sqrt(FGamma * __units::degCToK(FTemperature) * FRMezcla) / __cons::ARef;

    /* Potencia */
    FPotencia =
        FC1Potencia * pow3(FPressure) + FC2Potencia * pow2(FPressure) +
        FC3Potencia * FPressure + FC4Potencia +
    ...
    /* printf("ERROR: TCCCompresorVolumetrico::CalculaCondicionContorno No hay
     convergencia en el compresor volumetrico en la condicion de contorno:
     %d.\n",FNumeroCC); printf("Repasar los datos. El compresor esta en
     condiciones supersonicas.\n"); throw Exception("ERROR:
     TCCCompresorVolumetrico::CalculaCondicionContorno No hay convergencia en el
     compresor volumetrico.");*/

    /* Obtencion de las variables de Riemann */ // Tal y como esta planteada
                                                // esta BC, el flujo siempre
                                                // sera entrante al tubo.
...
//      asgNumeroCV=true;
// }else{
//      std::cout << "ERROR: Este Compressor Volumetrico ya tiene numero
//      asignado" << std::endl; throw Exception("");
// }
// }
// catch(Exception &N)
//{
// std::cout << "ERROR: TCCCompresorVolumetrico::PutNumeroCV en la BC " <<
  ...
  try {
    if (FResMediosCV.Potencia) {
      FResMediosCV.PotenciaMED =
          FResMediosCV.PotenciaSUM / FResMediosCV.TiempoSUM;
      FResMediosCV.PotenciaSUM = 0.;
    }
    if (FResMediosCV.Massflow) {
    ...
    if (FResMediosCV.Pressure) {
      FResMediosCV.PresionMED =
          FResMediosCV.PresionSUM / FResMediosCV.TiempoSUM;
      FResMediosCV.PresionSUM = 0.;
    }
    FResMediosCV.TiempoSUM = 0;
  } catch (exception &N) {
