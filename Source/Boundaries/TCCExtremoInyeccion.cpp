...
void TCCExtremoInyeccion::CalculaCondicionContorno(double Time) {
  try {
    ...
    if (FAngap < FDuracionIny && FTheta > 720.) {
      FSonido = sqrt(FGamma * FRMezcla * __units::degCToK(FTemperaturaIny)) /
                __cons::ARef;
      FVelocity = (FSonido - *FCC) / FGamma3;
      FPressure = __units::PaToBar(
          FGastoIny * FRMezcla * __units::degCToK(FTemperaturaIny) /
          (FVelocity * __cons::ARef * FSeccion)); // Cociente entre presion y la
                                                  // presion de referencia(1e5);
      if (FPressure < 0.0) {
        printf("ERROR: TCCExtremoInyeccion::CalculaCondicionContorno Velocity "
               "negativa en inyector %lf,Theta= %lf\n en la condicion de "
               "contorno: %d",
               FVelocity * __cons::ARef, FTheta, FNumeroCC);
        printf("       TCCExtremoInyeccion::CalculaCondicionContorno Presion "
               "negativa en inyector %lf\n",
               FPressure);
        throw Exception("");
      }
...
// catch(Exception &N)
//{
// std::cout << "ERROR: TCCExtremoInyeccion::PutIniIny en la condicion de
...
// catch(Exception &N)
//{
// std::cout << "ERROR: TCCExtremoInyeccion::PutDuracionIny en la condicion de
// contorno: " << FNumeroCC <<  std::endl; std::cout << "Tipo de error: " <<
// N.what() << std::endl; throw Exception(N.what());
// }
// }
