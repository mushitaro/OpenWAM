...
void TCCExternalConnection::UpdateCurrentExternalProperties(
    double U0, double U1, double T0, double T1, double P0, double P1,
    double t) {
...
void TCCExternalConnection::ExternalCharacteristics(double Time) {
  ...
  double deltaEnt =
      Ax * (AA0 - FA_AExt * __cons::ARef) / FA_AExt / __cons::ARef;
...
void TCCExternalConnection::CalculaCondicionContorno(double Time) {
  ...
  if (flujo < 0.999995) {
    ...
    *FCD = A + __Gamma::G_3 * FU_Boundary;
    ...
    double A =
        FA_AExt / (FTuboExtremo[0].Entropia + FA_AExt) * (*FCC + FK_CExt);
    FT_Boundary = pow2(A * __cons::ARef) / __R::Air / __Gamma::G;
