...
TCCCilindro::TCCCilindro(nmTypeBC TipoCC, int numCC,
  ...
  if (TipoCC == nmIntakeValve)
    ...
    printf("ERROR en tipo de valvula TCCCilindro en la condicion de contorno: "
           "%d\n",
           FNumeroCC);
...
void TCCCilindro::AsignaTipoValvula(
...
void TCCCilindro::ReadBoundaryData(
  ...
  try {
    ...
    FileInput >> numid; // Esto es un dato que necesita el WAMer. Los usuarios
                        // de WAM hacemos la vista gorda hasta que se arregle.
    FileInput >> FNumeroCilindro;
...
void TCCCilindro::CalculaCondicionContorno(double Time) {
  try {
    ...
    if (rel_CCon_Entropia / FAd > 1.000005) { // Flujo entrante al cilindro
      ...
      if (FCDEntrada > 0.0001) { /* Abierto */
        ...
        if (FGasto < -1e-5) {
          ...
          FMomento = coef * pow2(__cons::ARef * FGasto) /
                     (200000 * FMotor->getGeometria().Carrera * FGamma *
                      FCilindro->getPressure()) *
                     (pow2(*FCC + *FCD) / 4. +
                      FGamma3 * pow2(((*FCD - *FCC) / FGamma1)));
        }
    ...
    } else if (rel_CCon_Entropia / FAd <
               .999995) { // Flujo saliente del cilindro
      ...
      if (FCDSalida > 0.0001) { /* Abierto */
        ...
        if (FGasto > 1e-5) {
          FMomento =
              -FCilindro->getMomentoAngular() * FGasto / FCilindro->getMasa();
        }
        ...
        if (FHayEGR)
          FFraccionMasicaEspecie[FNumeroEspecies - 1] =
              FCilindro->GetComposicionSaliente(FNumeroEspecies - 1);
      } else { /* Cerrado */
        FMomento = 0.;
        FGasto = 0.;
        *FCD = *FCC;
...
void TCCCilindro::FlujoEntranteCilindro() {
  try {
    ...
    if (Fk < 1)
      Fk = 1;
    vel_son_garganta = FTuboExtremo[0].Entropia *
                       FAd; // Velocity del sonido en la garganta. Adimensional.
    ...
    *FCC = FSonido + Ga3U;
    FRelacionPresionGarganta =
        pow(FSonido / (FTuboExtremo[0].Entropia * FAd), FGamma4);
    ...
    std::cout << "ERROR: TCCCilindro::FlujoEntranteCilindro en la condicion de "
                 "contorno: "
              << FNumeroCC << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception(N.what());
...
void TCCCilindro::FlujoSalienteCilindro() {
  try {
    ...
    a2cr = sqrt(pow2(FCilindro->getSpeedsound() / __cons::ARef) -
                FGamma3 * pow2(u2cr));
    ...
    if (error < 0.) { // Salto de presiones supercritico.
      ...
      // Calcula del massflow. Como es saliente del cilindro, siempre es
      // positivo.
      ...
      if (root_a > 0) {
        FVelocity = sqrt(
            (pow2(FCilindro->getSpeedsound() / __cons::ARef) - pow2(FSonido)) /
            FGamma3);
      } else if (root_a > -1e12) {
        FVelocity = 0;
      } else {
      ...
      Ga3U = FVelocity * FGamma3;
      // Calculo del massflow. Como es saliente del cilindro, siempre es
      // positivo.
      xx = *FCC + Ga3U;
      ...
      FGasto =
          __units::BarToPa(
              FCDSalida * FSeccionValvula * FGamma *
    ...
    *FCC = FSonido - Ga3U;
    ...
    std::cout << "ERROR: TCCCilindro::FlujoSalienteCilindro en la condicion de "
                 "contorno: "
              << FNumeroCC << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception(N.what());
