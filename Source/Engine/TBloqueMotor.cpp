...
void TBloqueMotor::LeeMotor(std::istream &FileInput,
  ...
  try {
    ...
    int NumeroLeyesQuemado = 0, CalculoTempPared = 0, nwiebes = 0,
    ...
    if (FACT) {
      ...
      if (!FHayEGR) {
        ...
        std::cout
            ...
            << std::endl;
        std::cout << "         EGR transport, in the other case, the results"
                  << std::endl;
    ...
    FGeom.CilindradaUnitaria =
        __geom::Cylinder_volume(FGeom.Diametro, FGeom.Carrera);
    ...
    if (SimulationType == nmTransitorioRegimen) {
      ...
      FCoeficienteInercias =
          FRendCajaCambios * pow2(FRelCajaCambios * FRelTrasmision) /
  ...
  try {
    ...
    if (FResMediosMotor.RegimenGiro || FResMediosMotor.Potencia ||
      ...
      FResMediosMotor.RegimenGiroMED =
          FResMediosMotor.RegimenGiroSUM / FResMediosMotor.TiempoSUM;
      FResMediosMotor.RegimenGiroSUM = 0.;
    }
    if (FResMediosMotor.ParNeto || FResMediosMotor.PMN) {
      FResMediosMotor.ParNetoMED =
          FResMediosMotor.ParNetoSUM / FResMediosMotor.TiempoSUM;
      FResMediosMotor.ParNetoSUM = 0.;
    }
    if (FResMediosMotor.PMN) {
      FResMediosMotor.PMNMED =
          FResMediosMotor.ParNetoMED * 16. /
    ...
    if (FResMediosMotor.ParEfectivo || FResMediosMotor.PME ||
      ...
      FResMediosMotor.ParEfectivoMED =
          FResMediosMotor.ParEfectivoSUM / FResMediosMotor.TiempoSUM;
      FResMediosMotor.ParEfectivoSUM = 0.;
    }
    if (FResMediosMotor.PME) {
      FResMediosMotor.PMEMED =
          FResMediosMotor.ParEfectivoMED * 16. /
          (FGeom.NCilin * pow2(FGeom.Diametro) * FGeom.Carrera) / 100000.;
    }
    if (FResMediosMotor.Potencia) {
      FResMediosMotor.PotenciaMED =
          __units::To_kilo(FResMediosMotor.ParEfectivoMED * __cons::Pi_x_2 *
    ...
    if (FResMediosMotor.MasaAdmision) {
      ...
      FResMediosMotor.MasaAdmisionMED =
          FResMediosMotor.MasaAdmisionSUM / FGeom.NCilin;
      FResMediosMotor.MasaAdmisionSUM = 0.;
    }
    ...
    if (FResMediosMotor.MasaAtrapada ||
      ...
      FResMediosMotor.FraccionAireFrescoMED =
          FraccionAireFrescoSUM / FGeom.NCilin;
    }
    if (FResMediosMotor.RendimientoVolumetrico ||
        FResMediosMotor.RendimientoVolumetricoAtm) {
      FResMediosMotor.GastoTuboReferenciaMED =
          FResMediosMotor.GastoTuboReferenciaSUM / FResMediosMotor.TiempoSUM;
      FResMediosMotor.GastoTuboReferenciaSUM = 0.;
    }
    if (FResMediosMotor.RendimientoVolumetrico) {
      FResMediosMotor.DensidadReferenciaMED =
          FResMediosMotor.DensidadReferenciaSUM / FResMediosMotor.TiempoSUM;
      FResMediosMotor.RendimientoVolumetricoMED =
          fabs(FResMediosMotor.GastoTuboReferenciaMED) /
    ...
    if (FResMediosMotor.ParPerdidasMecanicas) {
      FResMediosMotor.ParPerdidasMecanicasMED =
          FResMediosMotor.ParPerdidasMecanicasSUM / FResMediosMotor.TiempoSUM;
      FResMediosMotor.ParPerdidasMecanicasSUM = 0.;
    }
    if (FResMediosMotor.ParResistente) {
      FResMediosMotor.ParResistenteMED =
          FResMediosMotor.ParResistenteSUM / FResMediosMotor.TiempoSUM;
      FResMediosMotor.ParResistenteSUM = 0.;
    }
    if (FResMediosMotor.VelocidadVehiculo) {
      FResMediosMotor.VelocidadVehiculoMED =
          FResMediosMotor.VelocidadVehiculoSUM / FResMediosMotor.TiempoSUM;
      FResMediosMotor.VelocidadVehiculoSUM = 0.;
    }
    if (FResMediosMotor.RendimientoVolumetricoAtm) {
      ...
      FResMediosMotor.RendimientoVolumetricoAtmMED =
          fabs(FResMediosMotor.GastoTuboReferenciaMED) / DensidadAtm /
          FGeom.CilindradaTotal / (FResMediosMotor.RegimenGiroMED / 120.);
    }
    if (FResMediosMotor.Dosado) {
      FResMediosMotor.MasaTuboReferenciaMED =
          FResMediosMotor.MasaTuboReferenciaSUM / FResMediosMotor.TiempoSUM;
    ...
    FResMediosMotor.PMNCicloMED = __units::PaToBar(
        FResMediosMotor.TrabajoNetoMED / FGeom.CilindradaTotal);

    FResMediosMotor.PMBCicloMED = __units::PaToBar(
        FResMediosMotor.TrabajoBombeoMED / FGeom.CilindradaTotal);

    FResMediosMotor.PMICicloMED =
        FResMediosMotor.PMNCicloMED - FResMediosMotor.PMBCicloMED;
    ...
    FResMediosMotor.ParEfectivoCicloMED =
        __units::BarToPa(FResMediosMotor.PMECicloMED) * FGeom.CilindradaTotal /
        (__units::DegToRad(FAngTotalCiclo));

    FResMediosMotor.PotenciaCicloMED =
        __units::To_kilo(__cons::Pi_x_2 * FResMediosMotor.ParEfectivoCicloMED *
                         __units::RPMToRPS(FResMediosMotor.RegimenGiroMED));
    ...
    if (FResMediosMotor.MasaFuelMED == 0) {
      ...
      FResMediosMotor.RendEfectivoMED =
          __units::BarToPa(FResMediosMotor.PMECicloMED) *
          FGeom.CilindradaTotal /
          (FGeom.NCilin * FResMediosMotor.MasaFuelMED * FPoderCalorifico);
      FResMediosMotor.ConsumoEspecificoMED =
          3.6e9 / (FResMediosMotor.RendEfectivoMED * FPoderCalorifico);
    }
...
void TBloqueMotor::AcumulaResultadosMediosBloqueMotor(double TActual,
  ...
  try {
    ...
    if (FResMediosMotor.ParEfectivo || FResMediosMotor.PME ||
        FResMediosMotor.Potencia) {
      FResMediosMotor.ParEfectivoSUM +=
          partotalins - FParPerdidasMecanicas * DeltaT;
    }
    ...
    /* for(int i=0;i<FGeom.NCilin;i++){
     ...
     if(FCilindro[i]->getAnguloActual()>180. &&
     FCilindro[i]->getAnguloActual()<540.){
...
void TBloqueMotor::ModeloDeVehiculo(double Time) {
  try {
    ...
    FParPerdidasMecanicas = __units::BarToPa(FPMPMMotor) *
                            FGeom.CilindradaTotal /
                            (__units::DegToRad(FAngTotalCiclo));
    ...
    if (FRPMControlled) {
      ...
      if (FTipoModelado == nmTransitorioRegimen) {
        ...
        FRoadLoad =
            ...
            FCoefRoadLoad.C0 *
                pow(W * FRadioRueda / (FRelCajaCambios * FRelTrasmision),
                    FCoefRoadLoad.n) +
        ...
        if (FTheta < 7200) {
          FRegimen = __units::Rad_sToRPM(
              ((FParMotor - FParResistente) * FCoeficienteInercias * DeltaT +
               W));
        }
        FVelocidadVehiculo =
