...
enum nmValvula { nmAdmision = 0, nmEscape = 1 };
...
protected:
  ...
  bool FCicloCerrado; // Controla si el cilindro se encuentra en ciclo cerrado
                      // (true) o abierto (false)
  ...
  double FCalorLiberadoCiclo; // Variable para la impresion por pantalla del
                              // resultado.
  ...
  dVector FComposicionCicloCerrado; // 0 -> Quemados, 1 -> Combustible, 2 ->
                                    // Aire Fresco
  ...
  double FGamma4;
  double FGamma6;
  double FGammaCil; // Se usa cuando se opta por calculo en tubos con gamma
                    // constante.
  bool FHayEGR;
  int FIntEGR;
...
public:
  ...
  double GetFraccionMasicaEspecie(int i) { return FFraccionMasicaEspecie[i]; };

  double getParInstantaneo() { return FParInstantaneo; };

  double getTrabajoNeto() { return FTrabajoNeto; };

  double getTrabajoBombeo() { return FTrabajoBombeo; };
  ...
  double getMomentoAngular() { return FMomentoAngular; };

  double getMasaValvAdm() { return FMasaValvAdm; };

  void PutMasaFuel(double valor) { FMasaFuel = valor; }

  double FMasaFuel;

  double getMasaFuel() { return FMasaFuel; }

  double getMasaPorAdmision() { return FMasaPorAdmision; };

  double getMasaAtrapada() { return FMasaAtrapada; };

  stDistribucion getDistribucion() { return FDistribucion; };

  double getAnguloActual() { return FAnguloActual; };

  double getAnguloAnterior() { return FAnguloAnterior; };

  double getSwirlSUM() { return FSwirlSUM; };

  int getNumeroUnionesAdm() { return FNumeroUnionesAdm; };

  int getNumeroUnionesEsc() { return FNumeroUnionesEsc; };
  ...
  double getAFR() { return FAFR; };

  double getTiempoActual() { return FTime1; };

  double getTemperature() { return FTemperature; };

  void PutInicioComb(double valor) { FIniComb = valor; }

  void PutFinComb(double valor) { FFinComb = valor; }

  void PutFQL(double valor) { FCalor.FQL = valor; }

  double getPreMed() { return FPreMed; };

  double getVolumen() { return FVolumen; };

  double getVolumen0() { return FVolumen0; };

  double getGamma() { return FGamma; };

  void PutHayDLL(bool valor) { FHaydll = valor; }

  void PutModComb(bool valor) { FModComb = valor; }
  ...
  double getFraccionAireFresco() { return GetAireFresco(); };

  void PutNumeroInyecciones(int valor) { FIN = valor; };
  void PutPresionInyeccion(double valor);

  void PutSOP(int i, double valor) { FSOP[i] = valor; };

  void PutMasaFuelPorInyeccion(int i, double valor) { FMFI[i] = valor; };
  ...
  void ReadAverageResultsCilindro(std::istream &FileInput);

  void HeaderAverageResultsCilindro(std::ostream &medoutput,
                                    stEspecies *DatosEspecies);

  void ImprimeResultadosMediosCilindro(std::ostream &medoutput);

  void AcumulaResultadosMediosCilindro(double TActual);

  void CalculaResultadosMediosCilindro();

  void ReadInstantaneousResultsCilindro(std::istream &FileInput);

  void HeaderInstantaneousResultsCilindro(std::ostream &insoutput,
                                          stEspecies *DatosEspecies);

  void ImprimeResultadosInstantaneosCilindro(std::ostream &insoutput);
