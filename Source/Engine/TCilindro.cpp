...
TCilindro::TCilindro(TBloqueMotor *Engine, bool ThereIsEGR) {
  ...
  if (FMotor->getACT()) {
    ...
    Ftest_variables[3] =
        372.434622123441; // In-cylinder temperature at inlet valve closing
    ...
    for (int counter = 1; counter < FCAI; counter++) {
      FCAD_injection_rate[counter] =
          FCAD_injection_rate[counter - 1] + ((360. / (FCAI - 1)));
    }
...
void TCilindro::AsignacionCC_Pointers(TCondicionContorno **BC, int numCC) {
  try {
    ...
    Fratioctm = pow2(FMotor->getGeometria().DiametroBowl /
    ...
    // Solo se usa el NIT de las valvulas de escape, pero como antes estaba como
    // resultados por pantalla, lo dejo por lo que pueda pasar, y al menos la
...
void TCilindro::ReadAverageResultsCilindro(std::istream &FileInput) {
  try {
    int Tipovar = 0;
...
void TCilindro::HeaderAverageResultsCilindro(std::ostream &medoutput,
...
void TCilindro::ImprimeResultadosMediosCilindro(std::ostream &medoutput) {
  try {
    // if (FNumVarMed > 0) {
    if (FResMediosCilindro.TrabajoNeto)
...
void TCilindro::AcumulaResultadosMediosCilindro(double TActual) {
  try {
    ...
    if (FNumVarMed > 0) {
      ...
      // double
      // DeltaAngulo=360.*FMotor->PutRegimen(360.*FMotor->getRegimen()/60.*DeltaT);

      FResMediosCilindro.TrabajoNetoSUM +=
          __units::BarToPa(FPreMed) * (FVolumen - FVolumen0);
    ...
    if (FNumVarMed > 0) {
      ...
      if (FResMediosCilindro.PresionMediaNeta ||
        ...
        FResMediosCilindro.PresionMediaNetaMED =
            __units::PaToBar(FResMediosCilindro.TrabajoNetoMED /
      ...
      if (FResMediosCilindro.PresionMediaBombeo ||
        ...
        FResMediosCilindro.PresionMediaBombeoMED =
            __units::PaToBar(-FResMediosCilindro.TrabajoBombeoMED /
                             FMotor->getGeometria().CilindradaUnitaria);
      }
      if (FResMediosCilindro.CalorCombustion) {
      ...
      if (FResMediosCilindro.NITMedio) {
        for (int i = 0; i < FNumeroUnionesEsc; i++) {
          FResMediosCilindro.NITMED[i] =
              FValvEsc[i].NITSUM / FResMediosCilindro.TiempoSUM;
          FValvEsc[i].NITSUM = 0.;
        }
        FResMediosCilindro.NITMedioMED =
            FResMediosCilindro.NITMedioSUM / FResMediosCilindro.TiempoSUM;
        FResMediosCilindro.NITMedioSUM = 0.;
      }
      ...
      if (FResMediosCilindro.RendVolumetrico) {
        ...
        FResMediosCilindro.RendVolumetricoMED =
            FMasaAtrapada / DensidadReferencia /
            FMotor->getGeometria().CilindradaUnitaria;
        FResMediosCilindro.DensidadReferenciaSUM = 0.;
      }
...
void TCilindro::ReadInstantaneousResultsCilindro(std::istream &FileInput) {
  try {
    int numvar = 0, var = 0;
    ...
    FResInstantCilindro.FraccionINS =
        new double[FMotor->getSpeciesNumber() - FIntEGR];
...
void TCilindro::HeaderInstantaneousResultsCilindro(std::ostream &insoutput,
                                                   stEspecies *DatosEspecies) {
  try {
    // if (FNumVarIns > 0) {
...
void TCilindro::ImprimeResultadosInstantaneosCilindro(std::ostream &insoutput) {
  try {
    // if (FNumVarIns > 0) {
  ...
  try {
    ...
    if (FMotor->getGeometria().CoefDeformaciones != 0) {
      double aux = FMotor->getGeometria().CoefDeformaciones /
                   ...
                   pow(FMotor->getGeometria().Diametro /
                           FMotor->getGeometria().DiametroBulon,
                       2.) *
      ...
      if (FDeltaAngulo != 0) {
        double Lant =
            ...
            m * (cos(a - __units::DegToRad(FDeltaAngulo)) +
                 sqrt(pow2((b / m)) -
...
void TCilindro::IniciaVariables() {
  try {
    ...
    if (FMotor->getTheta() > FMotor->GetDesfase(FNumeroCilindro - 1)) {
      FAnguloActual =
          FMotor->getTheta() - FMotor->GetDesfase(FNumeroCilindro - 1);
    } else {
    ...
    FFraccionComienzoCicloCerrado.resize(FMotor->getSpeciesNumber() - FIntEGR,
                                         0.);
    ...
    if (FMotor->getSpeciesModel() == nmCalculoCompleto) {
      RAtmosfera =
          ...
          (FMotor->GetComposicionAtmosfera(7) -
           0.01292 * (1 - FMotor->GetComposicionAtmosfera(2))) *
              __R::N2 +
      ...
      FraccionMolarO2 =
          FMotor->GetComposicionAtmosfera(0) * PMAtmosfera / __PM::O2;
      FraccionMolarH2O =
          FMotor->GetComposicionAtmosfera(2) * PMAtmosfera / __PM::H2O;
    ...
    if (FMotor->getSpeciesModel() == nmCalculoCompleto) {
      ...
      TemperaturaInicial =
          __units::KTodegC(__units::BarToPa(FMotor->getPresionInicial()) *
                           FVolumen / FMotor->getMasaInicial() / FRMezcla);
      FCpMezcla = CalculoCompletoCpMezcla(
          FFraccionMasicaEspecie[0], FFraccionMasicaEspecie[1],
          FFraccionMasicaEspecie[2], FFraccionMasicaEspecieFuel,
          __units::degCToK(TemperaturaInicial), FMotor->getGammaCalculation(),
      ...
      FCvMezcla = CalculoSimpleCvMezcla(
          __units::degCToK(TemperaturaInicial), FFraccionMasicaEspecie[0], 0,
          FMotor->getGammaCalculation(), FMotor->getCombustible());
      FGamma = CalculoSimpleGamma(FRMezcla, FCvMezcla,
                                  FMotor->getGammaCalculation());
      FHcl =
          -1852564 +
          2195 * (30 + 273 + 125); // Temperatura del combustible en K +125, que
                                   // es el incremento de la temperatura de
                                   // combustible hasta la tobera del inyector
    }
    ...
    if (FMotor->GetDesfase(FNumeroCilindro - 1) == 0) {
      ...
      if (FAnguloActual < 180. || FAnguloActual > 540.) {
        ...
        FTemperature = __units::KTodegC(__units::BarToPa(FPressure) * FVolumen /
                                        FMasa / FRMezcla);
        // Como cambia la Temperature, cambia Cp o Cv y por tanto cambia el
        // valor de Gamma.
        if (FMotor->getSpeciesModel() == nmCalculoCompleto) {
          if (FMotor->getSpeciesNumber() == 9) {
          ...
          FCvMezcla = CalculoSimpleCvMezcla(
              __units::degCToK(FTemperature), FFraccionMasicaEspecie[0], 0,
              FMotor->getGammaCalculation(), FMotor->getCombustible());
          FGamma = CalculoSimpleGamma(FRMezcla, FCvMezcla,
                                      FMotor->getGammaCalculation());
        }
    ...
    if (FMotor->getSpeciesModel() == nmCalculoCompleto) {
      FDosadoEstequiometrico =
          1 / ((FXComb + FYComb / 4 - FZComb / 2) * __PM::O2) *
  ...
  try {
    ...
    if (FMotor->getMasaFuel() == 0.) {
      ...
      if (FMotor->getLeyQuemadoBD().size() == 1) {
        ...
        FFinComb = FMotor->getLeyQuemadoBD()[0]
                       .Wiebes[FMotor->getLeyQuemadoBD()[0].Wiebes.size() - 1]
                       .Alpha0 +
                   FMotor->getLeyQuemadoBD()[0]
                       .Wiebes[FMotor->getLeyQuemadoBD()[0].Wiebes.size() - 1]
                       .IncAlpha;
      } else {
        ...
        for (Uint i = 0; i < FMotor->getLeyQuemadoBD().size(); i++) {
          ...
          FAngDesplazado[i] =
              Ang01 - FMotor->getLeyQuemadoBD()[i].Wiebes[k].Alpha0;
          /* Comentado por SHC para avanzar
           for (Uint j = 0; j < FMotor->getLeyQuemadoBD()[i].Wiebes.size(); j++)
           { FMotor->getLeyQuemadoBD()[i].Wiebes[j].Inicia =
           FMotor->getLeyQuemadoBD()[i].Wiebes[j] .Alpha0 + FAngDesplazado[i];
           }
           */
          ...
          Finaliza = FMotor->getLeyQuemadoBD()[i]
                         .Wiebes[FMotor->getLeyQuemadoBD()[i].Wiebes.size() - 1]
                         .Inicia +
                     FMotor->getLeyQuemadoBD()[i]
                         .Wiebes[FMotor->getLeyQuemadoBD()[i].Wiebes.size() - 1]
                         .IncAlpha;
...
double TCilindro::CalculaCalorLiberado(double x) {
  try {
    ...
    double Numerador = 0;
    double Denominador = 0;
    double dist = 0., e = 0., b = 0., ley = 0., DistMax = 0., DistMin = 0.,
           Weight = 0.;
    ...
    std::cout << "ERROR: CalculaCalorLiberado en cilindro: " << FNumeroCilindro
              << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception(N.what());
  }
  ...
  try {
    ...
    if (x <= FMotor->getLeyQuemadoBD()[ley].Wiebes[wiebe].Inicia) {
      ...
      if ((xxx * FMotor->getLeyQuemadoBD()[ley].Wiebes[wiebe].C) > 10.) {
        ...
        ret_val =
            1. - 1. / exp(xxx * FMotor->getLeyQuemadoBD()[ley].Wiebes[wiebe].C);
      }
    }
    return ret_val;
  } catch (std::exception &N) {
    std::cout << "ERROR: TCilindro:fun_wiebe en cilindro: " << FNumeroCilindro
              << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception(N.what());
  }
...
void TCilindro::CalculaTemperaturasPared() {
  ...
  try {
    double ViscGas = 0., ViscPared = 0., Vel = 0., Re = 0., Cond = 0.,
           hExt = 0.;
    ...
    Fo = (FMotor->getParedCilindro().Conductividad /
          ...
          FMotor->getParedCilindro().CalorEspecifico) *
    ...
    if (Fo > 0.5 || Fo * (1 + Bii) > 0.5 || Fo * (1 + Bie) > 0.5) {
      std::cout << "WARNING: Condiciones de calculo inestables para la trans. "
                   "de calor en el cilindro: "
                << FNumeroCilindro << std::endl;
    }
    ...
    if (FMotor->getCalculoPared() == nmConInercia &&
      ...
      if (Fh > 0.) {
        hi2 =
            1. / (1. / Fh + FMotor->getParedCilindro().Espesor /
                                FMotor->getParedCilindro().Conductividad / 2.);
        hi3 = 1. / (1. / Fh + FMotor->getParedCilindro().Espesor /
                                  FMotor->getParedCilindro().Conductividad);
        FTempPared[0].CilindroSUMup +=
            hi3 * __units::degCToK(FTemperature) * FDeltaT;
        FTempPared[0].CilindroSUMdown += hi3 * FDeltaT;
        FTempPared[1].CilindroSUMup +=
            hi2 * __units::degCToK(FTemperature) * FDeltaT;
        FTempPared[1].CilindroSUMdown += hi2 * FDeltaT;
        FTempPared[2].CilindroSUMup +=
            Fh * __units::degCToK(FTemperature) * FDeltaT;
        FTempPared[2].CilindroSUMdown += Fh * FDeltaT;
      }
      if (FAnguloActual < FAnguloAnterior && FNumeroCiclo > 1) {
        ho2 =
            1 / (1 / hExt + FMotor->getParedCilindro().Espesor /
                                FMotor->getParedCilindro().Conductividad / 2.);
        ho1 = 1 / (1 / hExt + FMotor->getParedCilindro().Espesor /
                                  FMotor->getParedCilindro().Conductividad);
        FTempPared[2].Cylinder = __units::KTodegC(
            (FTiempoCiclo * hExt *
    ...
    // SE HAN COMENTADO LOS CALCULOS IDENTICOS AL CILINDRO PARA AHORRAR TIEMPO
    // DE CALCULO ViscGas=3.27e-7; ViscPared=1.8e-7;
    ...
    Fo = (FMotor->getParedPiston().Conductividad /
          ...
          FMotor->getParedPiston().CalorEspecifico) *
         FDeltaT / (pow2(FMotor->getParedPiston().Espesor) / 4.);
    ...
    if (Fo > 0.5 || Fo * (1 + Bii) > 0.5 || Fo * (1 + Bie) > 0.5) {
      std::cout << "WARNING: Condiciones de calculo inestables para la trans. "
                   "de calor del piston en el cilindro: "
                << FNumeroCilindro << std::endl;
    }
    ...
    if (FMotor->getCalculoPared() == nmConInercia &&
      ...
      if (Fh > 0.) {
        hi2 = 1. / (1. / Fh + FMotor->getParedPiston().Espesor /
                                  FMotor->getParedPiston().Conductividad / 2.);
        hi3 = 1. / (1. / Fh + FMotor->getParedPiston().Espesor /
                                  FMotor->getParedPiston().Conductividad);
      ...
      if (FAnguloActual < FAnguloAnterior && FNumeroCiclo > 1) {
        ho2 = 1 / (1 / hExt + FMotor->getParedPiston().Espesor /
                                  FMotor->getParedPiston().Conductividad / 2.);
        ho1 = 1 / (1 / hExt + FMotor->getParedPiston().Espesor /
                                  FMotor->getParedPiston().Conductividad);
        FTempPared[2].Piston = __units::KTodegC(
            (FTiempoCiclo * hExt *
                 __units::degCToK(FMotor->getTempRefrigerante()) +
    ...
    // SE HAN COMENTADO LOS CALCULOS IDENTICOS AL CILINDRO PARA AHORRAR TIEMPO
    // DE CALCULO ViscGas=3.27e-7; ViscPared=1.8e-7; Cond=0.67;
    Vel = 5.64268e-7 * FMotor->getRegimen() / 60. * FMotor->getParPotMax() /
          FMotor->getGeometria().NCilin /
    ...
    Fo = (FMotor->getParedCulata().Conductividad /
          ...
          FMotor->getParedCulata().CalorEspecifico) *
         FDeltaT / (pow2(FMotor->getParedCulata().Espesor) / 4.);
    ...
    if (Fo > 0.5 || Fo * (1 + Bii) > 0.5 || Fo * (1 + Bie) > 0.5) {
      std::cout << "WARNING: Condiciones de calculo inestables para la trans. "
                   "de calor de la culata en el cilindro: "
                << FNumeroCilindro << std::endl;
    }
    ...
    if (FMotor->getCalculoPared() == nmConInercia &&
      ...
      if (Fh > 0.) {
        hi2 = 1. / (1. / Fh + FMotor->getParedCulata().Espesor /
                                  FMotor->getParedCulata().Conductividad / 2.);
        hi3 = 1. / (1. / Fh + FMotor->getParedCulata().Espesor /
                                  FMotor->getParedCulata().Conductividad);
      ...
      if (FAnguloActual < FAnguloAnterior && FNumeroCiclo > 1) {
        ho2 = 1 / (1 / hExt + FMotor->getParedCulata().Espesor /
                                  FMotor->getParedCulata().Conductividad / 2.);
        ho1 = 1 / (1 / hExt + FMotor->getParedCulata().Espesor /
                                  FMotor->getParedCulata().Conductividad);
        FTempPared[2].Culata = __units::KTodegC(
            (FTiempoCiclo * hExt *
                 __units::degCToK(FMotor->getTempRefrigerante()) +
...
void TCilindro::CalculaFuelMEP(double MasaAire) {
  ...
  FMasaFuel =
      fabs(MasaAire) * FMotor->getDosadoInicial() * FDosadoEstequiometrico;
...
// }
// catch(Exception & N) {
// std::cout << "ERROR: TCilindro::PutMasaFuel en el cilindro: " <<
// FNumeroCilindro << std::endl; std::cout << "Tipo de error: " << N.what() <<
...
// }
// catch(Exception & N) {
// std::cout << "ERROR: TCilindro::PutPresionInyeccion en el cilindro: " <<
// FNumeroCilindro << std::endl; std::cout << "Tipo de error: " << N.what() <<
// std::endl; throw Exception(N.what());
// }
// }
...
// }
// catch(Exception & N) {
// std::cout << "ERROR: TCilindro::PutNumeroInyecciones en el cilindro: " <<
// FNumeroCilindro << std::endl; std::cout << "Tipo de error: " << N.what() <<
// std::endl; throw Exception(N.what());
// }
// }
...
// }
// catch(Exception & N) {
// std::cout << "ERROR: TCilindro::PutInicioComb en el cilindro: " <<
// FNumeroCilindro << std::endl; std::cout << "Tipo de error: " << N.what() <<
// std::endl; throw Exception(N.what());
// }
// }
...
// }
// catch(Exception & N) {
// std::cout << "ERROR: TCilindro::PutFinComb en el cilindro: " <<
// FNumeroCilindro << std::endl; std::cout << "Tipo de error: " << N.what() <<
// std::endl; throw Exception(N.what());
// }
// }
...
// }
// catch(Exception & N) {
// std::cout << "ERROR: TCilindro::PutFQL en el cilindro: " << FNumeroCilindro
// << std::endl; std::cout << "Tipo de error: " << N.what() << std::endl; throw
// Exception(N.what());
// }
// }
...
void TCilindro::DefineCombustion() {
  try {
    if (FMotor->getACT()) {
      FCalcComb = nmACT;
      if (FModComb) {
        std::cout << "WARNING: Se pretende calcular la combution mediante ACT "
                     "y mediante"
                  << std::endl;
        std::cout
...
// }
// catch(Exception & N) {
// std::cout << "ERROR: TCilindro::PutCalculadoPaso en el cilindro: " <<
// FNumeroCilindro << std::endl; std::cout << "Tipo de error: " << N.what() <<
// std::endl; throw Exception(N.what());
// }
// }
...
// }
// catch(Exception & N) {
// std::cout << "ERROR: TCilindro::PutHayDLL en el cilindro: " <<
// FNumeroCilindro << std::endl; std::cout << "Tipo de error: " << N.what() <<
// std::endl; throw Exception(N.what());
// }
// }
...
void TCilindro::CalculoNIT() {
  try {
    ...
    FNIT = FCpMezcla * __units::degCToK(FTemperature) *
           (1 - pow(FPressure, arriba));
    massflow = 0.;
    for (int j = 0; j < FNumeroUnionesEsc; ++j) {
      massflow += dynamic_cast<TCCCilindro *>(FCCValvulaEsc[j])->getMassflow();
      FValvEsc[j].NIT =
          FNIT * dynamic_cast<TCCCilindro *>(FCCValvulaEsc[j])->getMassflow();
  ...
  try {
    if (FNumVarMed > 0) {
      ...
      std::cout << "INFO:_________________________________________"
                << std::endl;
      if (FResMediosCilindro.RendVolumetrico)
        std::cout << "INFO: Volumetric efficiency intake manifold (refered to "
                     "pipe n. "
                  << FMotor->getTuboRendVol()->getNumeroTubo()
                  << "): " << FResMediosCilindro.RendVolumetricoMED << " (-)"
                  << std::endl;
      if (FResMediosCilindro.TrabajoNeto)
        std::cout << "INFO: Net work:   " << FResMediosCilindro.TrabajoNetoMED
                  << " (J)" << std::endl;
      if (FResMediosCilindro.PresionMediaNeta)
...
double TCilindro::GetAireFresco() {
  try {
    ...
    std::cout << "ERROR: TCilindro::GetAireFresco en la condicion de contorno: "
              << FNumeroCilindro << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception(N.what());
  }
...
double TCilindro::GetComposicionSaliente(int i) {
  try {
    ...
    std::cout << "ERROR: TCilindro::GetComposicionSaliente en la condicion de "
                 "contorno: "
              << FNumeroCilindro << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception(N.what());
