...
void TCilindro2T::ActualizaPropiedades(double TiempoActual) {
  try {
    ...
    if (FCicloCerrado && FPrimerInstanteCicloCerrado) {
      ...
      if (FMotor->getACT()) {
        ...
        CALCULUS_OF_INJECTION_RATE(FIN, FSOP, FMFI, FSOI, FEOI,
                                   Ftest_variables[0], FCAI, Ftest_variables[5],
                                   FA_TASA, FB_TASA, FC_TASA, FD_TASA,
                                   Finjection_rate, FCAD_injection_rate);

        ACT(Fengine_parameters, Fengine_model_constants, Ftest_variables,
            Finjection_rate, FCAD_injection_rate, Fsize_inlet_inj, FIN, FSOI,
            FEOI, FCAI, FCAD_exit, FHRF_exit, FROHR_exit, Fp_cyl_exit,
            Fdp_da_cyl_exit, FT_cyl_exit, FH_cooler_exit, Fmean_var_exit,
            Fheat_transfer, Finjection_rate_exit, Faccum_injection_rate_exit,
            dataIN, &dataOUT);
        ...
        if (FMotor->getFTipoDatosIny() == 1) {
          ...
          for (int i = 0; i < FNumIny; i++) {
            ...
            if (FAnguloInjeccion[i] < 0) {
              FAnguloInjeccion[i] =
                  FAnguloInjeccion[i] + FMotor->getAngTotalCiclo();
            }
            //						else {
            //							FAnguloInjeccion[i]
            //= FAnguloInjeccion[i];
          ...
          if (FMotor->getFAngIniIny() < 0) {
            FAnguloInjeccion[0] =
                FMotor->getFAngIniIny() + FMotor->getAngTotalCiclo();
          } else {
          ...
          if (FMotor->getLeyQuemadoBD()[0].Wiebes.size() == 4) {
            ...
            FTInyeccion[1] =
                (FFinComb - FIniComb) / (FMotor->getRegimen() * 6.) / 4.;
            FPercentInyeccion[0] = FMotor->getLeyQuemadoBD()[0].Wiebes[0].Beta;
            FPercentInyeccion[1] = 1. - FPercentInyeccion[0];
            // Si la combustión principal empieza en angulo negativo, hay que
            // sumar 360º
            ...
            FTInyeccion[0] =
                (FFinComb - FIniComb) / (FMotor->getRegimen() * 6.) / 4.;
            FAnguloInjeccion[0] =
                FIniComb - FAnguloRetrasoCombustion +
                FMotor->getAngTotalCiclo(); // Se asume que la inyección empieza
                                            // cuando empieza la FQL
            FPercentInyeccion[0] = 1;
          }
        }
      ...
      // Queda una ultima opcion, que se realice el calculo con la dll. En ese
      // caso se el valor a FIniComb y FFinComb ya se le habra dado a partir de
      // la DLL con una property write.

      std::cout << "INFO: Fuel mass:           " << FMasaFuel * 1e6 << " (mg)"
                << std::endl;
    ...
    if (FMotor->getGammaCalculation() == nmGammaConstante) {
      if (FMotor->getSpeciesModel() == nmCalculoCompleto) {
        if (FMotor->getSpeciesNumber() == 9) {
          FFraccionMasicaEspecieFuel =
              0; // No se tiene en cuenta el combustible
        ...
        FCpMezcla = CalculoCompletoCpMezcla(
            FFraccionMasicaEspecie[0], FFraccionMasicaEspecie[1],
            FFraccionMasicaEspecie[2], FFraccionMasicaEspecieFuel,
            __units::degCToK(FTemperature), nmComposicionTemperatura,
            FMotor->getCombustible());
        FGamma =
            CalculoCompletoGamma(FRMezcla, FCpMezcla, nmComposicionTemperatura);

      } else if (FMotor->getSpeciesModel() == nmCalculoSimple) {
        FRMezcla = CalculoSimpleRMezcla(
            FComposicionCicloCerrado[0], FComposicionCicloCerrado[1],
            FMotor->getGammaCalculation(), FMotor->getCombustible());
        // FRMezcla =  287*FComposicionCicloCerrado[2]
        // + 55.95*FComposicionCicloCerrado[1] +
        // 285.4*FComposicionCicloCerrado[0];
        FCvMezcla = CalculoSimpleCvMezcla(
            __units::degCToK(FTemperature), FComposicionCicloCerrado[0],
            FComposicionCicloCerrado[1], nmComposicionTemperatura,
            FMotor->getCombustible());
        FGamma =
            CalculoSimpleGamma(FRMezcla, FCvMezcla, nmComposicionTemperatura);
      }
      ...
      if (FMotor->getSpeciesModel() == nmCalculoCompleto) {
        ...
        FCpMezcla = CalculoCompletoCpMezcla(
            FFraccionMasicaEspecie[0], FFraccionMasicaEspecie[1],
            FFraccionMasicaEspecie[2], FFraccionMasicaEspecieFuel,
            __units::degCToK(FTemperature), FMotor->getGammaCalculation(),
            FMotor->getCombustible());
        ...
        FCvMezcla = CalculoSimpleCvMezcla(
            __units::degCToK(FTemperature), FComposicionCicloCerrado[0],
            FComposicionCicloCerrado[1], FMotor->getGammaCalculation(),
            FMotor->getCombustible());
    ...
    FDeltaAngulo =
        FMotor->getAngTotalCiclo() * FMotor->getRegimen() / 60. * FDeltaT;
    ...
    if (FAnguloActual > FMotor->getAngTotalCiclo()) { // Hector 2T
      ...
      FNumeroCiclo++;
      // std::cout << "INFO: El cilindro " << FNumeroCilindro << " comienza el
      // ciclo " << FNumeroCiclo << std::endl;
    }
    ...
    if (FMotor->getACT()) {
      for (int i = 0; i < FNumeroUnionesAdm; i++) {
        FPresionMedAdm +=
            FCCValvulaAdm[i]->GetTuboExtremo(0).Pipe->GetPresion(0) * FDeltaT /
            FNumeroUnionesAdm;
      }
      for (int i = 0; i < FNumeroUnionesEsc; i++) {
        FPresionMedEsc +=
            FCCValvulaEsc[i]->GetTuboExtremo(0).Pipe->GetPresion(0) * FDeltaT /
            FNumeroUnionesEsc;
      }
    ...
    if (!FCicloCerrado && FDeltaAngulo > 0.) {
      ...
      for (int i = 0; i < FNumeroUnionesAdm; i++) {
        FMasaValvAdm =
            ...
            FDeltaT; // signo - xq el massflow entrante se calcula negativo en
                     // la BC. Asi en el cilindro sera positivo si es entrante y
                     // viceversa.
        if (FAnguloActual - FDeltaAngulo < FDistribucion.CA &&
            FAnguloActual >
                FDistribucion.CA) { // FDistribucion.CA es el CrankAngle de
                                    // cierre de la admision.
      ...
      for (int i = 0; i < FNumeroUnionesEsc; i++) {
        ...
        if (FAnguloActual - FDeltaAngulo < FDistribucion.CE &&
          ...
          masavalesc = masavalesc *
                       ...
                       FDeltaAngulo; // FDistribucion.CE es el CrankAngle de
                                     // cierre del escape.
        }
    ...
    if (FPressure > FMotor->getPresionAmb() &&
      ...
      z = FGamma4 * (pow(FPresionCarter / FPressure, 2 / FGamma) -
                     pow(FPresionCarter / FPressure, FGamma2 / FGamma));
      FGastoBlowBy = FMotor->getGeometria().CDBlowBy * 3.5e-5 *
                     FMotor->getGeometria().Diametro *
                     __units::BarToPa(FPressure) *
    ...
    for (int i = 0; i < FNumIny; i++) {
      if (FAnguloActual > FAnguloInjeccion[i] &&
        ...
        // En el angulo de begining de la combusti�n se empieza a introducir el
        // combustible Se pasa a estado de injeccion verdadero. FMasaFuel =
        // FMotor->getMasaFuel();
        FInyeccion = true;
        FTasaFuel = 0.;
        ind = i;
    ...
    if (FInyeccion) {
      if (FMotor->getACT()) {
        FTasaFuel =
            Interp1(FAnguloComb, FCAD_injection_rate, Finjection_rate, FCAI) /
            1000.;
        ...
        // FFuelInstant representa la cantidad de masa de fuel introducida en un
        // instante de calculo.
        ...
        FFuelInstant = FTasaFuel * FDeltaT;
        // Se va acumulando la masa de fuel para comprobar que no super el valor
        // de combustible fijado por cilindro y ciclo.
      }
      if ((FFuelAcum + FFuelInstant) > FMasaFuel * FPercentInyeccion[ind]) {
        // La inyeccion se corta cuando se alcanza el valor de combustible
        // total.
    ...
    // AL CIERRE DE LA VALVULA DE ADMISION SE OBTIENE LA MASA TOTAL POR ADMISION
    // Y ESCAPE, LA MASA ATRAPADA EN EL CILINDRO, EL COMBUSTIBLE (PARA LOS MEP
    // EN FUNCION DE LA MASA POR ADMISION) LA PRESION Y LOS ANGULOS DE INICIO Y
    ...
    if (FAnguloActual > FDistribucion.CA &&
      ...
      // Transporte de Especies Quimicas. Se inician las masas de cada especie
      // en el ciclo cerrado.
      if (FMotor->getSpeciesModel() == nmCalculoCompleto) {
        if (FMotor->getSpeciesNumber() == 9) {
          ...
          // FComposicionCicloCerrado[2]=FFraccionMasicaEspecie[0]+FFraccionMasicaEspecie[1]+FFraccionMasicaEspecie[2]+FFraccionMasicaEspecie[7];
          // // Aire fresco
          ...
          // Combustible
          // FComposicionCicloCerrado[2]=FFraccionMasicaEspecie[0]+FFraccionMasicaEspecie[1]+FFraccionMasicaEspecie[2]+FFraccionMasicaEspecie[8];
          // // Aire fresco
          FComposicionCicloCerrado[2] =
              FFraccionMasicaEspecie[0] / FMotor->GetComposicionAtmosfera(0);
          FComposicionCicloCerrado[0] =
    ...
    if (FMotor->getCombustible() == nmMEP ||
      ...
      if (FAnguloActual >= 0. && FAnguloActual <= 90.) {
        Fequis = Fratioctm + 1 / (pow(cosh(FAnguloActual / 100.), 40.) +
                                  Fratioctm / (1 - Fratioctm));
      } else if (FAnguloActual >= 270. && FAnguloActual <= 360.) {
        Fequis =
            Fratioctm + 1 / (pow(cosh((360. - FAnguloActual) / 100.), 40.) +
                             Fratioctm / (1 - Fratioctm));
      } else {
        Fequis = 0.;
      }
      ...
      if (FAnguloComb < FIniComb || FAnguloComb > FFinComb || FMasaFuel == 0.) {
        ...
        Fc2 = 0.001; // antes 3.24e-3  (ahora 0.001)MOTOR DE CAMARA ABIERTA
                     // ;  6.22e-3  MOTOR DE CAMARA DIVIDIDA
      }
      // COEFICIENTE DE PELICULA DE WOSCHNI
      double deltaP = __units::BarToPa(
          FPressure - FPresionRCA * pow(FVolumenCA / FVolumen, 1.36));
      if (deltaP < 0.)
        deltaP = 0.;
      if (FAnguloActual >= 90. && FAnguloActual <= 180.) {
        // CARRERA DE ESCAPE
        Fh = 1.2e-2 * pow(__units::BarToPa(FPressure), 0.8) *
             pow(__units::degCToK(FTemperature), -0.53) *
             pow(FMotor->getGeometria().Diametro, -0.2) *
             pow((6.18 * FCm + 0.417 * FCu) * FMotor->getAjusteTranCalEsc() +
        ...
        Fh = 1.2e-2 * pow(__units::BarToPa(FPressure), 0.8) *
             pow(__units::degCToK(FTemperature), -0.53) *
             pow(FMotor->getGeometria().Diametro, -0.2) *
             pow((6.18 * FCm + 0.417 * FCu) * FMotor->getAjusteTranCalAdm() +
                     Fc2 * deltaP * FMotor->getGeometria().CilindradaUnitaria /
                         FMasaAtrapada / FRMezcla,
                 0.8);
    ...
    FCalor.TransCilindro =
        Fh * (FTempPared[0].Cylinder - FTemperature) * 4 *
    ...
    FCalor.TransTotal =
        (FCalor.TransCilindro + FCalor.TransPiston + FCalor.TransCulata) *
        FDeltaT;
    ...
    if (FCicloCerrado) {
      if (FAnguloComb < FIniComb || FAnguloComb > FFinComb || FMasaFuel == 0.) {
        ...
        FMasaEspecieCicloCerrado[1] =
            FMasaEspecieCicloCerrado[1] + FFuelInstant; // Combustible
        ...
        if (FMairequem > (FMasaEspecieCicloCerrado[2])) {
          ...
          if (!FSaturado) {
            printf("WARNING: Cylinder %d does not have enough air at "
                   "CrankAngle %lf\n",
          ...
          FMasaEspecieCicloCerrado[0] =
              FMasaEspecieCicloCerrado[0] + FMairequem + FMfquem -
          ...
          FMasaEspecieCicloCerrado[1] =
              ...
              FFuelInstant; // Combustible
          FMasaEspecieCicloCerrado[2] =
              FMasaEspecieCicloCerrado[2] - FMairequem -
              FComposicionCicloCerrado[2] * FMasaBlowBy; // Aire Fresco
        }
    ...
    if (FCicloCerrado) {
      if (FMotor->getSpeciesModel() == nmCalculoCompleto) {
        if (FMotor->getACT()) {
          ...
          if (FMotor->getSpeciesNumber() == 9) {
            if (!FSaturado) {
              FMolesCombQuemado =
                  FMfquem / (FXComb * 12.01 + FYComb * 1.01 +
                             FZComb * 16); // Moles de combustible quemado en el
                                           // incremento temporal calculado
            } else {
              FMolesCombQuemado =
                  mfquefin / (FXComb * 12.01 + FYComb * 1.01 +
                              FZComb * 16); // Moles de combustible quemado en
                                            // el incremento temporal calculado
            }
            ...
            FFraccionMasicaEspecie[7] =
                (FMasaEspecie[7] - FFraccionMasicaEspecie[7] * FMasaBlowBy) /
                FMasa; // Y N2
            FFraccionMasicaEspecie[3] =
                FComposicionCicloCerrado[1]; // Combustible que queda sin quemar
            ...
            FFraccionMasicaEspecie[8] =
                (FMasaEspecie[8] - FFraccionMasicaEspecie[7] * FMasaBlowBy) /
                FMasa; // Y N2
      ...
      if (FHayEGR) {
        FFraccionMasicaEspecie[FMotor->getSpeciesNumber() - 1] =
            ...
            FMasa;
        FMasaEspecie[FMotor->getSpeciesNumber() - 1] =
            FFraccionMasicaEspecie[FMotor->getSpeciesNumber() - 1] * FMasa;
      }
    }
    ...
    // (bar)" << std::endl; std::cout << "INFO: Imposed temperature at E.O.:  "
    ...
    FPresion0 = FPressure;
    FPressure = __units::PaToBar(__units::degCToK(FTemperature) * FMasa *
                                 FRMezcla / FVolumen);
    if (FAnguloActual > FDistribucion.AE &&
        FAnguloAnterior <= FDistribucion.AE) {
      ...
      std::cout << "INFO: Calculated pressure at E.O.:    " << FPressure
                << " (bar)" << std::endl;
      std::cout << "INFO: Calculated temperature at E.O.: " << FTemperature
                << " (\260C)" << std::endl;
      if (FMotor->getCalculoDePAAE() == nmPAAEImpuesta) {
        FPressure = FMotor->getPresionAAE();
        FTemperature = __units::KTodegC(__units::BarToPa(FPressure) * FVolumen /
                                        FMasa / FRMezcla);
        FAsonido = sqrt(FRMezcla * FGamma * __units::degCToK(FTemperature));
        std::cout << "INFO: Imposed pressure at E.O.:     " << FPressure
                  << " (bar)" << std::endl;
        std::cout << "INFO: Imposed temperature at E.O.:  " << FTemperature
                  << " (\260C)" << std::endl;
      }
    ...
    FParInstantaneo = __units::BarToPa(FPressure - FMotor->getPresionAmb()) *
                      area * R * sin(a - b) / cos(b);
    ...
    FDensidadReferencia =
        FMotor->getTuboRendVol()->GetGamma(FMotor->getNodoMedio()) *
        __units::BarToPa(
            FMotor->getTuboRendVol()->GetPresion(FMotor->getNodoMedio())) /
        pow2(FMotor->getTuboRendVol()->GetAsonido(FMotor->getNodoMedio()) *
             __cons::ARef);
    ...
    if (FAnguloActual > FDistribucion.AA && FAnguloActual < FDistribucion.CE) {
      if (MasaAdmInstante > 0. && MasaEscInstante < 0.) {
        // Cortocircuito con sentido Admision hacia Escape.    (massflow
        // positivo)
        MasaCortocircuitoAdm = MasaAdmInstante * FAlphaEscape / __cons::Pi;
        MasaCortocircuitoEsc =
            MasaEscInstante * (__cons::Pi_2 - FAlphaEscape) / __cons::Pi;
        ...
        for (int j = 0; j < FMotor->getSpeciesNumber() - 2; j++) {
          ...
          FComposicionSaliente[j] =
              FFraccionMasicaEspecie[j] *
                  (MasaEscInstante + FMasaCortocircuito) / MasaEscInstante -
        ...
        // Cortocircuito con sentido Escape hacia Admision.   (massflow
        // negativo)
        MasaCortocircuitoAdm =
            MasaAdmInstante * (__cons::Pi_2 - FAlphaAdmision) / __cons::Pi;
        ...
        for (int j = 0; j < FMotor->getSpeciesNumber() - 2; j++) {
          ...
          FraccionCC = FraccionCC / NumeroUnionesEntrante;
          // La masa por la valvula de admision sera negativa, por ser saliente
          // del cilindro. La masa de cortocircuito de escape a admision es
          // negativa.
          FComposicionSaliente[j] =
              FFraccionMasicaEspecie[j] *
                  (MasaAdmInstante - FMasaCortocircuito) / MasaAdmInstante +
        ...
        // No hay cortocircuito, pues en este instante entra o sale massflow por
        // todas las valvulas.
...
void TCilindro2T::VariableInicialesCicloACT() {
  try {
    ...
    // Measured air mass
    Ftest_variables[2] =
        FMasaAtrapada * 1000; // In-cylinder air mass at inlet valve closing
    Ftest_variables[3] = __units::degCToK(
        FTemperature); // In-cylinder temperature at inlet valve closing
    ...
    // Ftest_variables[8]=FMotor->getGeometria().CDBlowBy;          //Blow-by
    // coefficient Ftest_variables[9]=FMotor->getPresionAmb()*1e5; //Atmosphere
    // pressure
    ...
    Ftest_variables[13] =
        __units::degCToK(FTempPared[0].Piston); // Piston temperature
    if (FMotor->getSpeciesModel() == nmCalculoCompleto) {
    ...
    std::cout << "____________________________________________________"
              << std::endl;
    std::cout << "INFO: Engine speed:        " << Ftest_variables[0] << " (rpm)"
              << std::endl;
    std::cout << "INFO: Intake mass:         " << Ftest_variables[1]
              << " (kg/s)" << std::endl;
    std::cout << "INFO: Trapped mass:        " << Ftest_variables[2] << " (g)"
              << std::endl;
    std::cout << "INFO: Temperature at IVC:  " << Ftest_variables[3] << " (K)"
              << std::endl;
    std::cout << "INFO: Fuel mass:           " << Ftest_variables[4] << " (mg)"
              << std::endl;
    std::cout << "INFO: Injection pressure:  " << Ftest_variables[5] << " (bar)"
              << std::endl;
    std::cout << "INFO: Intake pressure:     " << Ftest_variables[6] << " (Pa)"
              << std::endl;
    std::cout << "INFO: Exhaust pressure:    " << Ftest_variables[7] << " (Pa)"
              << std::endl;
    std::cout << "____________________________________________________"
              << std::endl;
  } catch (std::exception &N) {
    std::cout << "ERROR: TCilindro2T::VariableInicialesCicloACT en cilindro: "
              << FNumeroCilindro << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception(N.what());
  }
