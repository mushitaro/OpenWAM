...
void TCilindro4T::ActualizaPropiedades(double TiempoActual) {
  try {
        ...
        if (FMotor->getFTipoDatosIny() == 1) {
          ...
          if (FMotor->getLeyQuemadoBD()[0].Wiebes.size() == 4) {
            ...
            if (FMotor->getCombustible() == nmMEC) {
              ...
              FTInyeccion[0] = (720 - FIniComb - FDistribucion.CA) /
                               (FMotor->getRegimen() * 6.) / 2.;
    ...
    if (!FInyeccion) {
      for (int i = 0; i < FNumIny; i++) {
        if (FAnguloActual > FAnguloInjeccion[i] &&
          ...
          // En el angulo de begining de la combusti�n se empieza a introducir
          // el combustible Se pasa a estado de injeccion verdadero. FMasaFuel =
          // FMotor->getMasaFuel();
          FInyeccion = true;
          FTasaFuel = 0.;
          ind = i;
    ...
    if (getenv("OPENWAM_VOLDIAG") && FNumeroCilindro == 1 &&
        FMotor->getCiclo() >= 4 && FAnguloActual > 360. && FAnguloActual < 600.) {
      // Sample sparsely (every ~10 deg) in a CONVERGED cycle (>= 4), not the
      // cycle-1 startup transient.
      static double s_lastTheta = -1e9;
      if (fabs(FAnguloActual - s_lastTheta) > 10.0) {
    ...
    if (FAnguloActual >= 0. && FAnguloActual <= 180.) {
      ...
      Fequis = Fratioctm + 1 / (pow(cosh((720. - FAnguloActual) / 100.), 40.) +
                                Fratioctm / (1 - Fratioctm));
    } else {
      Fequis = 0.;
    }
    FCu =
        __units::RPMToRad_s(FMotor->getRegimen()) * Fequis * Fctorbadmp * FKctm;
    ...
    if (FAnguloComb < FIniComb || FAnguloComb > FFinComb || FMasaFuel == 0.) {
      ...
      Fc2 = 0.001; // antes 3.24e-3  (ahora 0.001) MOTOR DE CAMARA ABIERTA
                   // ;  6.22e-3  MOTOR DE C�MARA DIVIDIDA
    }
    ...
    if (FCicloCerrado) {
      if (FAnguloComb < FIniComb || FAnguloComb > FFinComb || FMasaFuel == 0.) {
        ...
        FMasaEspecieCicloCerrado[1] =
            FMasaEspecieCicloCerrado[1] + FFuelInstant;
        // Combustible
        FMasaEspecieCicloCerrado[2] =
            FMasa - FMasaEspecieCicloCerrado[0] - FMasaEspecieCicloCerrado[1];
        // Aire Fresco
      ...
      if (FMotor->getSpeciesModel() == nmCalculoCompleto) {
        if (FMotor->getACT()) {
          ...
          if (FMotor->getSpeciesNumber() == 9) {
            if (!FSaturado) {
              FMolesCombQuemado =
                  FMfquem / (FXComb * 12.01 + FYComb * 1.01 + FZComb * 16);
            ...
            FFraccionMasicaEspecie[8] =
                (FMasaEspecie[8] - FFraccionMasicaEspecie[8] * FMasaBlowBy) /
                FMasa;
            // Y N2
...
void TCilindro4T::VariableInicialesCicloACT() {
  try {
    ...
    std::cout << "ERROR: TCilindro4T::VariableInicialesCicloACT en cilindro: "
              << FNumeroCilindro << std::endl;
    std::cout << "Tipo de error: " << N.what() << std::endl;
    throw Exception(N.what());
  }
