...
private:
  ...
  TTubo *FTuboRendVol;
  int FNodoMedio;
  int FNumTuboRendVol; /* Pipe de la admision al que se refiere el rendimiento
                          volumetrico */
  ...
  dVector FX_dat;
  double FFuelTasaInt;
  int TipoInterp; // Tipo de interpolación: 1. Lineal, 2. Hermite
                  // (spinlines), 3. Step
  ...
  // int FCiclo; // Controla el numero de ciclo de la ejecucion. (para salida de
  // resultados)
  bool FPrimeravezAcumulaFuel; // Variable para controlar en el 1er ciclo la
                               // salida en resultados de MFuel.
  bool FPrimeravezAcumulaMasaAtrapada; // Variable para controlar en el 1er
                                       // ciclo la salida en resultados del Rend
                                       // Volumetrico.
...
public:
  bool getACT() { return FACT; };

  double getMixtureProcessCte() { return FMixtureProcessCte; };

  stGeometria getGeometria() { return FGeom; };

  stWoschni getWoschni() { return FWoschni; };
  ...
  double getMasaFuel() { return FMasaFuel; };

  void setMasaFuel(double fuel) { FMasaFuel = fuel; }

  double getMasaInicial() { return FMasaInicial; };

  double getPresionAAE() { return FPresionAAE; };

  double getPresionInicial() { return FPresionInicialRCA; };

  stTemperaturasPared getTempInicial() { return FTempInicial; };
  ...
  int getCiclo() { return FCiclo; }

  double getAngTotalCiclo() { return FAngTotalCiclo; };

  double GetDesfase(int i);

  nmTipoMotor getEngineType() { return FTipoMotor; };

  nmTipoCombustion getCombustible() { return FCombustible; };

  double getPoderCalorifico() { return FPoderCalorifico; };

  double getDensidadCombustible() { return FDensidadCombustible; };

  double getRendimientoCombustion() { return FRendimientoCombustion; };

  double getDosadoInicial() { return FDosadoInicial; };

  stPropTermicas getParedPiston() { return FParedPiston; };

  stPropTermicas getParedCulata() { return FParedCulata; };

  stPropTermicas getParedCilindro() { return FParedCilindro; };

  nmCalculoPared getCalculoPared() { return FCalculoPared; };

  double FAjusteTranCalAdm;

  double getAjusteTranCalAdm() { return FAjusteTranCalAdm; }
  void PutATCAdm(double valor);

  double FAjusteTranCalEsc;

  double getAjusteTranCalEsc() { return FAjusteTranCalEsc; }

  void PutATCEsc(double valor);

  double getParPotMax() { return FParPotMax; };

  const std::vector<stLeyQuemadoBD> getLeyQuemadoBD() { return FLeyQuemadoBD; };

  double getLQRegMax() { return FLQRegMax; };

  double getLQMfMax() { return FLQMfMax; };

  double getLQMaMax() { return FLQMaMax; };

  double getPresionAmb() { return FPresionAmbiente; };

  double getTemperaturaAmb() { return FTemperaturaAmbiente; };

  double getTempRefrigerante() { return FTempRefrigerante; };

  int getNumCiclosSinInerciaTermica() {
    return FNumeroCiclosSinInerciaTermica;
  };

  int getNumTuboRendVol() { return FNumTuboRendVol; };

  TCilindro *GetCilindro(int i);

  TTubo *getTuboRendVol() { return FTuboRendVol; };

  int getNodoMedio() { return FNodoMedio; };

  double getAFRMedio() { return FAFRMedio; };

  nmCalculoPAAE getCalculoDePAAE() { return FCalculoDePAAE; };

  nmTipoCalculoEspecies getSpeciesModel() { return FCalculoEspecies; };

  int getSpeciesNumber() { return FNumeroEspecies; };

  bool getImponerComposicionAE() { return FImponerComposicionAE; };

  double GetComposicionInicial(int i);

  double GetComposicionAtmosfera(int i);

  nmCalculoGamma getGammaCalculation() { return FCalculoGamma; };

  stInjectionSys getInjectionSys() { return FInjectionSys; };

  stInjecPulse getInjecPulse(int i) { return FInjecPulse[i]; };

  int getFTipoDatosIny() { return FTipoDatosIny; };

  int getFNumeroInyecciones() { return FNumeroInyecciones; };

  dVector getFAngIny() { return FAngIny; };

  dVector getFTIny() { return FTIny; };

  dVector getFPercentIny() { return FPercentIny; };

  double getFAngIniIny() { return FAngIniIny; };

  double getFAStepIny() { return FAStepIny; };
  ...
  void ImprimeResultadosMediosBloqueMotor(std::ostream &medoutput);
