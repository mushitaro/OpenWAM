...
void TSensor::AsignaObjeto(TObject *Object) {
  ...
  if (FObjectSensed == nmSensTubo) {
    ...
    FNode1 = ceil(tmp);
    FDelta =
        (FDistancia - (double)FNode0 * ((TTubo *)FObjectPointer)->getXRef()) /
        ((TTubo *)FObjectPointer)->getXRef();
  }
}

double TSensor::Output() { return FCurrentValue; }
...
void TSensor::ActualizaMedida(double Time) {
  ...
  if (Update || !FInicia) {
    ...
    case nmSensTubo:
      ...
      case nmTemperature:
        tmp0 =
            pow2(((TTubo *)FObjectPointer)->GetAsonido(FNode0) * __cons::ARef) /
            ((TTubo *)FObjectPointer)->GetGamma(FNode0) /
            ((TTubo *)FObjectPointer)->GetRMezcla(FNode0);
        tmp1 =
            pow2(((TTubo *)FObjectPointer)->GetAsonido(FNode1) * __cons::ARef) /
...
void TSensor::CabeceraResultadosMedSensor(std::ostream &medoutput) {
  try {
    std::string Label;

    if (FResMediosSensor.Output) {
      Label =
          "\t" + PutLabel(707) + std::to_string(FNumeroSensor) + PutLabel(901);
...
void TSensor::CabeceraResultadosInsSensor(std::ostream &insoutput) {
  try {
    std::string Label;
...
void TSensor::ImprimeResultadosMedSensor(std::ostream &medoutput) {
  try {
    std::string Label;
...
void TSensor::ImprimeResultadosInsSensor(std::ostream &insoutput) {
  try {
    std::string Label;
  ...
  try {
    ...
    if (FResMediosSensor.Output) {
      FResMediosSensor.OutputMED =
          FResMediosSensor.OutputSUM / FResMediosSensor.TiempoSUM;
      FResMediosSensor.OutputSUM = 0.;
    }
    if (FResMediosSensor.Input) {
      FResMediosSensor.InputMED =
          FResMediosSensor.InputSUM / FResMediosSensor.TiempoSUM;
      FResMediosSensor.InputSUM = 0.;
    }
    FResMediosSensor.TiempoSUM = 0;
...
int TSensor::ObjectID() { return FObjectID; };
