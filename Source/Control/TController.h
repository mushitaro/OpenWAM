...
/*! This oject is used to control parameter in the engine (rack positions, fuel
 * injected ...) */
class TController {
protected:
  std::vector<TSensor *>
      FSensor; //!< Array with the pointers of the sensor inputs
  ...
  stResMediosCtrl
      FResMediosCtrl; //!< Struct with the average results of the controllers
  stResInstantCtrl
      FResInstantCtrl; //!< Struct with the average results of the controllers
...
public:
  ...
  virtual void
  AsignaObjetos(TSensor **Sensor,        //!< Array with the sensor objects
  ...
  virtual void ImprimeResultadosInsControlador(std::ostream &insoutput) = 0;
...
struct stGainInputSensor : stGainInput {
  ...
  stGainInputSensor(TSensor *insens) : stGainInput() { InSensor = insens; };

  double Output(double t) { return InSensor->Output(); };
};

struct stGainInputController : stGainInput {

  stGainInputController(TController *inctrl) : stGainInput() {
    InController = inctrl;
  };

  double Output(double t) { return InController->Output(t); };
};
#endif
