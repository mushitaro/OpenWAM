...
TGain::TGain(int i) : TController(nmGainCtrl, i) {
  fID = i + 1;
  FGain = 1.;
}
...
void TGain::CabeceraResultadosInsControlador(std::ostream &insoutput) {
...
void TGain::ImprimeResultadosMedControlador(std::ostream &medoutput) {
...
void TGain::ImprimeResultadosInsControlador(std::ostream &insoutput) {
...
void TGain::IniciaMedias() { FResMediosCtrl.OutputSUM = 0.; }
...
void TGain::AcumulaResultadosMediosController(double Actual) {
  ...
  /* Lo que se hace en esta funcion se realiza dentro del calculo del eje, para
   asi poder llevar a cabo la salida de resultados medios por pantalla. */
