  ...
  try {
    ...
    case nmLamina2D:
      ...
      for (int veces = 0; veces <= FBucle; ++veces) {
        FLev2[0] = FLev2[2];
        FLev2[FNodosLamina + FNodosFijos + 2] =
            2 * FLev2[FNodosLamina + FNodosFijos + 1] -
        ...
        FLev3[FNodosLamina + FNodosFijos + 1] =
            2 * FLev3[FNodosLamina + FNodosFijos] -
            FLev3[FNodosLamina + FNodosFijos - 1];
...
void TLamina::GetCDin(double Time) {
  ...
  case nmLamina2D:
    for (int i = 0; i < (FNodosLamina + FNodosFijos + 2); ++i) {
      FFuerza[i] = __units::BarToPa(deltaP) * FAnchoPetalo * FNumPestanyas /
                   FDensidad / FAreaTrans;
      if (i < FNodosFijos + 1) {
        FFuerza[i] = 0.;
      }
