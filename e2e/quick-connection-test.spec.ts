...
test('クイック接続テスト', async ({ page }) => {
  ...
  await page.getByTestId('project-name-input').fill('クイック接続テスト');
  ...
  // キャンバス上でコンポーネントをクリック（予測位置）
  const canvas = page.getByTestId('canvas-editor');
  
  // 1回目の接続作成
  console.log('=== 1回目の接続作成 ===');
  await canvas.click({ position: { x: 200, y: 200 } }); // 1つ目のコンポーネント予測位置
  await page.waitForTimeout(1000);
  ...
  if (isTargetMode) {
    await canvas.click({ position: { x: 350, y: 200 } }); // 2つ目のコンポーネント予測位置
    ...
    if (hasSuccessMessage && firstConnectionCount !== initialConnectionCount) {
      // 2回目の接続作成（重複テスト）
      ...
      console.log('1回目の接続作成に失敗');
    }
  } else {
    console.log('最初のコンポーネントクリックに失敗 - 位置を調整が必要');
  }
});