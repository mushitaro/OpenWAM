...
## 2. 吸気系トポロジー（Type 12 プレナムレス設計）— legacy (eq_tube model="plenum")

> **注**: 本節のEQ系（閉じた中央141ccプレナム + 6スタブ）は **legacy モデル**。実車実測
> (2026-07) により EQ 系は **ICV経由でメインプレナムに帰還する共通レール**と判明した。
> 新トポロジーは「§2b 吸気トポロジー v2 (rail)」を参照。plenum/chain モデルは回帰
> bisect 用に温存され、`intake.eq_tube.model` で切り替える。
...
## 2b. 吸気トポロジー v2 (rail) — 実測ジオメトリ (2026-07)

実車実測（オーナー計測）に基づく新デフォルト。`PLAN_PARTLOAD_CALIBRATION.md` §0 参照。

### 実測値（新デフォルト）

| 項目 | legacy モデル | 実測 (v2) |
|---|---|---|
| プレナム容積 | 10.5 L | **22.9 L** |
| 吸気ダクト | 350mm×φ200 直管 | **長さ400mm、入口φ190 → プレナム側 550×190 スロット開口**（テーパは d_start/d_end の面積スケジュールで表現、出口 Deq=√(4·0.55·0.19/π)≈365mm） |
| パネルフィルタ | 20mm×φ300 | **550×190 接続面（Deq≈365mm）、厚み20mm** |
| ランナー上流（スロットル→EQ分岐） | 15mm | **10mm** |
| ランナー下流（EQ分岐→ポート分割） | 25mm | **60mm** |
| EQ系 | 閉じた中央141ccプレナム + φ30×75スタブ×6 | **共通レール φ21×570mm** が6ランナーに直接タップ、**レール中央付近**から **φ21×250mm ゴムパイプ → ICV → Plenum_Main へ帰還** |

### rail モデルのトポロジー（`intake.eq_tube.model="rail"`）

```
Runner_Upper_i --[Type12 ①]-- EqRail_Tap_i (φ30×30mm 数値床) --[Type12 レールティーi]--
レールティー1..6 を EqRail_Seg_1..5 (φ21, 各114mm, friction 0.03) で連結
return_tap="center": 中央セグメント(ティー3-4間)を2分割し7個目のティーを挿入
    → EqRail_Return (φ21×250mm) → [Type 11 + ICV固定Cdバルブ] → Plenum_Main
```

- **ICV**: Type-11接続上の固定Cdバルブ（INTAKE_V2マウスバルブと同機構）。
  `icv_sigma` = 帰還パイプ断面に対する有効開口率。**フィットパラメータ**（Phase 4Aで校正、
  実測不能）。バルブ index = `26 + n_cyl (+1 if INTAKE_V2)` = 通常 32。
- **物理的意味**: ICV経由の帰還はスロットルを迂回するバイパス流路（アイドル/低負荷の
  吸気経路）。Stage-49 の「低ペダル域の有効面積 3〜5 倍不足」ギャップを物理的に埋める
  レバー（φ21帰還パイプ面積 ≈346mm²、6気筒共有）。
- **タップ径の数値床 (リスクR1)**: 物理はφ21だが、φ52ランナー:φ21タップ = 面積比6.1:1 は
  Type-12 の安定床 (~3:1、Stage 35: φ10 は −10MW の NaN 源) を超えるため、既定は
  **タップφ30**（`rail_tap_diameter`）。物理φ21 は `OPENWAM_EQ_RAIL_TAP_DIA=0.021` で
  A/B 可能。レールセグメント自体のφ21はティー側がφ30なので面積比~2:1で安全。
- レール容積 ~197cc + 帰還パイプ ~87cc が**分布定数**で入る（legacy は 141cc 集中定数）。
- 全ティーは3パイプ以下を維持（4パイプ直タップは作らない — chain モデルと同じ流儀）。
- パイプ数: 38 (legacy吸気) + 6タップ差し替え + セグメント5(+1 center分割) + Return 1 =
  吸気45本 (center時)、排気開始 pid は 46（`_exhaust_pid_start` が自動追従）。
- env オーバーライド: `OPENWAM_EQ_RAIL`(=1でconfigによらずrail)、`OPENWAM_EQ_RAIL_DIA`、
  `OPENWAM_EQ_RAIL_TAP_DIA`、`OPENWAM_EQ_RAIL_FRIC`、`OPENWAM_ICV_SIGMA`（env > config > default）。
...
- OpenWAMはパイプ壁温・外気温を**摂氏**で読む（`degCToK()`）が、ジェネレータが
  ...
  - 吸気壁温: スノーケル/フィルタ `300→27`、ベルマウス/ランナー `313→40`、
    ポート `400→127`（すべて℃）
- さらに吸気の**空気源プレナム**も同バグ：`Ambient_Intake`(空気源1000m³)・
  `Plenum_Main`(10.5Lエアボックス)・`Equalization_Tube` が 300/313→573/586K。
  ambient/40℃に修正（`Ambient_Exhaust`出口も25℃に）。
- 排気管（700/800℃）・エンジンブロック（"60"℃）は元から正しい摂氏のため不変。
- 効果: 収束VE 50%→~57%（壁修正）。空気源修正はVE不変＝**熱源は境界温度でない**。
- 診断結論（Stage 16、確定）: 残差は**排気背圧でも吸気絞りでもバルブタイミングでも
  なく、ソルバの数値アーティファクト**。実測で：排気背圧正常(1.0 bar)、吸気供給正常
  (1.0 bar)、捕捉チャージ574K。決定的に：①IVOを振っても捕捉T/VEは~570K/~67%で平坦
  （オーバーラップ0–64°・IVC 30–110°ABDCで不変）、②**燃焼を無効化しても吸気~570K**。
  熱源ゼロでベルマウスが570Kは熱力学的に不可能＝**ソルバが吸気ガスに非物理的エネルギー
  を生成（or 質量取り扱い誤りで密度過小）**。次セッションは吸気エネルギー収支の計装から。
  詳細は `EXHAUST_STABILIZATION_NOTES.md` Stage 16（Stage 15の背圧説は誤りとして訂正済み）。
