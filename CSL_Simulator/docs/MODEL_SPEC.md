...
**IVO/IVC再較正（`scripts/ivo_sweep.py`）:** 吸気デュレーション増（260→268）で
IVC が遅角するため IVO ベースを再スイープ。結果、最適IVCはRPMで遅角（3000で
50°→5000-6000で70° ABDC）し、各RPMの最適点で **sim VE 96-102%＝stock比±7%でほぼ
一致**。Stage 12の「一様0.4x」は単一の誤った固定IVC（90° ABDC＝旧IVO360）の
アーティファクトと判明。デフォルト `base_open_intake` を **360→340**（IVC≈70°
ABDC、高回転寄りの最適中心）に変更。RPM依存の最適はVANOSマップ
（`kf_evan1_soll`）が担う。詳細は `EXHAUST_STABILIZATION_NOTES.md` Stage 14。
