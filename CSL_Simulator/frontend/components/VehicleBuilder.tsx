...
const VehicleBuilder = () => {
    ...
    const renderSelectionParams = () => {
        if (!selection) return <div className="text-slate-500 text-sm italic p-4">Select a component to view parameters.</div>;
        const { type } = selection;

        const InputRow = ({ label, value, onChange, unit }: any) => (
            <div className="mb-3">
                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">{label} {unit && `(${unit})`}</label>
                <input
                    type="number"
                    value={value}
                    onChange={(e) => onChange(e.target.value)}
                    className="w-full bg-slate-900 border border-slate-700 rounded px-3 py-2 text-sm text-slate-200 focus:border-slate-500 outline-none transition-colors font-mono"
                />
            </div>
        );
        ...
        const SectionHeader = ({ title, id }: { title: string, id: string }) => (
            <div className="mb-6 border-b border-slate-800 pb-2">
                ...
                <div className="text-xs text-slate-500 font-mono mt-1">SIM ID: {id}</div>
            </div>
        );
            ...
            <div className="p-4">
                ...
                {type === "environment" && (
                    <>
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-800">
                            <label className="block text-xs font-bold text-slate-400 mb-2">ADVANCED / GLOBAL</label>
                ...
                {(type === "intake_duct" || type === "plenum") && (
                    <>
                        ...
                        <div className="text-[10px] text-slate-600 mb-2">Slot empty = circular exit (= inlet dia). Measured car: 550×190.</div>
                        ...
                        {type === "plenum" && (
                            <div className="text-[11px] text-slate-600 mt-3 leading-snug">
                ...
                {type === "itb" && (
                    <>
                        ...
                        <div className="flex items-center gap-2 mb-4">
                            <input type="checkbox" checked={config.intake.itb.fitted} onChange={(e) => updateConfig("intake", "itb.fitted", e.target.checked)} className="rounded border-slate-700 bg-slate-900" />
                            <span className="text-sm font-medium text-slate-300">ITB Fitted</span>
                        </div>
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-800">
                            <h4 className="text-xs font-bold text-slate-500 mb-2">THROTTLE MODEL (Butterfly)</h4>
                            ...
                            <div className="text-[10px] text-slate-600">γ&gt;1 = progressive metering (validated 1.4)</div>
                        </div>
                    </>
                )}
                ...
                {type === "eq_tube" && (
                    <>
                        ...
                        <div className="flex items-center gap-2 mb-4">
                            <input type="checkbox" checked={config.intake.eq_tube.enabled} onChange={(e) => updateConfig("intake", "eq_tube.enabled", e.target.checked)} className="rounded border-slate-700 bg-slate-900" />
                        ...
                        {config.intake.eq_tube.enabled && (
                            <>
                                <div className="mb-3">
                                    ...
                                    <select value={config.intake.eq_tube.model} onChange={(e) => updateConfig("intake", "eq_tube.model", e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded px-3 py-2 text-sm text-slate-200 outline-none">
                                ...
                                {config.intake.eq_tube.model === "rail" ? (
                                    <>
                                        ...
                                        <div className="text-[10px] text-slate-600 mb-2">実車ブランチはレール側 φ21 へ絞る(テーパ表現; Stage 70)。空欄 = ストレート(レガシー)。実 φ10 は数値限界で直接表現不可。</div>
                                        ...
                                        <div className="mb-3">
                                            <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Return Tap</label>
                                            <select value={config.intake.eq_tube.return_tap} onChange={(e) => updateConfig("intake", "eq_tube.return_tap", e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded px-3 py-2 text-sm text-slate-200 outline-none">
                                        ...
                                        <div className="text-[10px] text-slate-600">φ21×570 rail taps all runners; returns to plenum through the ICV (throttle bypass). Tap φ30 = numerical floor (R1). ICV σ is calibrated, not measured.</div>
                                    </>
                                ) : (
                                    <>
                                        ...
                                        <div className="text-[10px] text-slate-600">φ30 min stable; mistune detunes cyl-2 collapse</div>
                                    </>
                                )}
                            </>
                        )}
                ...
                {type === "head_return" && (
                    <>
                        ...
                        <div className="flex items-center gap-2 mb-4">
                            <input type="checkbox" checked={config.intake.head_return.enabled} onChange={(e) => updateConfig("intake", "head_return.enabled", e.target.checked)} className="rounded border-slate-700 bg-slate-900" />
                            <span className="text-sm font-medium text-slate-300">Fitted (owner car: yes)</span>
                        </div>
                        {config.intake.head_return.enabled && (
                            <>
                                ...
                                <div className="text-[10px] text-slate-600 mb-2">ヘッド(カムカバー)→プレナムのブローバイ戻りホース(Stage 70-71 実測: φ15×250mm)。音響的にはエアボックスの Helmholtz 側枝。容積はカムカバー実効値(実測 2L 仮置き; オーナー承認済み)。</div>
                                <div className="mt-3 pt-3 border-t border-slate-800">
                                    <h4 className="text-xs font-bold text-slate-500 mb-2">ADVANCED</h4>
                ...
                {type === "runner" && (
                    <>
                        ...
                        <div className="mt-3 pt-3 border-t border-slate-800">
                            <h4 className="text-xs font-bold text-slate-500 mb-2">RAM TUNING</h4>
                            ...
                            <div className="text-[10px] text-slate-600">Length Scale shifts the ram-resonance rpm; Friction Mult broadens it (Q).</div>
                        </div>
                        <div className="text-xs text-slate-600 mt-2">Note: intake geometry is global for all runners.</div>
                    </>
                )}
                ...
                {type === "cylinder" && (
                    <>
                        ...
                        <div className="mt-4">
                            ...
                            <h4 className="text-xs font-bold text-slate-500 mb-2 mt-3">EXHAUST VALVE</h4>
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-800">
                            <h4 className="text-xs font-bold text-slate-500 mb-2">PORT GEOMETRY</h4>
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-800">
                            <h4 className="text-xs font-bold text-slate-500 mb-2">HEAD / FLOW</h4>
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-800">
                            <h4 className="text-xs font-bold text-slate-500 mb-2">COMBUSTION (Wiebe)</h4>
                ...
                {type === "header" && (
                    <>
                        ...
                        <div className="text-[10px] text-slate-600 mb-2">一次管出口の内径。空欄 = 集合部径へテーパ(レガシー)。実車は φ37.6 のまま(テーパ無し; Stage 71 実測)。</div>
                ...
                {type === "collector" && (
                    <>
                        ...
                        <div className="text-[10px] text-slate-600 mb-2">集合部本体(Col_Out 管)の長さ。実車 90mm(Stage 71 実測; 旧ハードコード 500mm)。</div>
                        <div className="mt-3 pt-3 border-t border-slate-800">
                            <h4 className="text-xs font-bold text-slate-500 mb-2">EXHAUST PORT JUNCTION</h4>
                            ...
                            <div className="text-[10px] text-slate-600">≤0 = plenumless Type-12 (validated); &gt;0 = small plenum/cyl</div>
                        </div>
                    </>
                )}
                    ...
                    <>
                        ...
                        {type === "section1" && (() => {
                            ...
                            return (
                                <div className="flex flex-col gap-4">
                                    <div>
                                        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Layout</label>
                                        <select
                                            ...
                                            className="w-full bg-slate-900 border border-slate-700 rounded px-3 py-2 text-sm text-slate-200 outline-none"
                                        >
                                        ...
                                        {config.exhaust.catalyst.installed && (
                                            <div className="flex flex-col gap-2">
                                                <div className="flex flex-col">
                                                    ...
                                                    <select value={config.exhaust.catalyst.location} onChange={(e) => updateConfig("exhaust", "catalyst.location", e.target.value)} className="bg-slate-950 border border-slate-700 text-xs p-1 rounded">
                                                ...
                                                <div className="text-[10px] text-slate-600">実車触媒 180×φ105.6(Stage 71 実測、壁厚 1.2mm 換算)。</div>
                                            </div>
                                        )}
                                    </div>

                                    <div className="p-3 bg-slate-900 border border-slate-800 rounded">
                                        <h4 className="text-[10px] font-bold text-slate-500 mb-2 border-b border-slate-800 pb-1">CROSSOVER → CAT (両バンク共通)</h4>
                                        ...
                                        <div className="text-[10px] text-slate-600">クロス管 → 直管 → テーパ → 触媒(実車: 440 + 120mm; Stage 71 実測)。0 = レガシー(クロス直後に触媒)。</div>
                                    </div>

                                    {(secConfig.layout === "X-Pipe" || secConfig.layout === "H-Pipe") ? (
                                        <>
                                            <div className="p-3 bg-slate-900 border border-slate-800 rounded mt-4">
                                                <h4 className="text-[10px] font-bold text-slate-500 mb-2 border-b border-slate-800 pb-1">FRONT SEGMENT (PRE-CROSSOVER)</h4>
                                            ...
                                            <div className="p-3 bg-slate-900 border border-slate-800 rounded mt-2">
                                                <h4 className="text-[10px] font-bold text-slate-500 mb-2 border-b border-slate-800 pb-1">REAR SEGMENT (POST-CROSSOVER)</h4>
                                                ...
                                                <InputRow label="Diameter" unit="mm" value={secConfig.diameter} onChange={(v: any) => updateConfig("exhaust", `${bankKey}.diameter`, v)} />
                                                <div className="text-[10px] text-slate-600 mt-2">*Diameter is shared</div>
                                            </div>
                                        </>
                                    ) : (
                                        <div className="p-3 bg-slate-900 border border-slate-800 rounded mt-4">
                        ...
                        {type === "section2" && (
                            <div className="flex flex-col gap-3">
                                <div>
                                    <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Layout</label>
                                    <select value={config.exhaust.section2.layout} onChange={(e) => updateConfig("exhaust", "section2.layout", e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded px-3 py-2 text-sm text-slate-200 outline-none">
                                ...
                                {config.exhaust.section2.layout === "H-Pipe" && (
                                    <>
                                        ...
                                        <div className="text-[10px] text-slate-600">H 管の位置。実車は触媒直後 80mm(Stage 72 純正図面; 旧ハードコード 400)。1600rpm セルに ±40pp 級の感度(Stage 73 除去分離)。</div>
                                    </>
                                )}
                                        ...
                                        <div className="flex flex-col gap-2 mt-2">
                                            {config.exhaust.section2.layout === "H-Pipe" && (
                                                <div className="flex flex-col gap-1">
                                                    <label className="text-[10px] text-slate-500">LOCATION (vs H-PIPE)</label>
                                                    <select value={config.exhaust.section2.resonator_location} onChange={(e) => updateConfig("exhaust", "section2.resonator_location", e.target.value)} className="bg-slate-950 border border-slate-700 text-xs p-1 rounded outline-none text-slate-300">
                                            ...
                                            <div className="text-[10px] text-slate-600">貫通吸音レゾネーター(実車 φ90×300 @ H後 800mm; Stage 72 純正図面)。</div>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        {type === "muffler" && (
                            <>
                                ...
                                <div className="text-[10px] text-slate-600 mb-2">実車外形 1000×300×160mm ≈ 48L(オーナー実測; 内容積 ~46L)。生成側は 30L を下限に持ち上げる点に注意。</div>
                                ...
                                <div className="mb-3">
                                    <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Internal Model</label>
                                    <select value={config.exhaust.section3.internal_model ?? "single"} onChange={(e) => updateConfig("exhaust", "section3.internal_model", e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded px-3 py-2 text-sm text-slate-200 outline-none">
                                        <option value="single">Single volume (legacy)</option>
                                        <option value="chambers">Multi-pass chambers (実車)</option>
                                    </select>
                                    <div className="text-[10px] text-slate-600 mt-1">chambers = 前室 → 180°/360° 通し管 → 後室 → テール(Stage 72 オーナー図面)。中域 5300/6300 に ±20-30pp 級の感度。</div>
                                </div>
                                {config.exhaust.section3.internal_model === "chambers" && (
                                    <div className="p-3 bg-slate-900 border border-slate-800 rounded">
                                        <h4 className="text-[10px] font-bold text-slate-500 mb-2 border-b border-slate-800 pb-1">MULTI-PASS INTERNALS</h4>
                                        ...
                                        <div className="text-[10px] text-slate-600 mb-2">ファンネル状開口(DME 文書 p37)。0 = ストレート。φ90 プローブで 5300 +3.8 / 6300 +2.4。</div>
                                        ...
                                        <div className="text-[10px] text-slate-600">通し管長は 1000mm ケースへの再スケール値(1130/1930)。旧 1700/2900 は 6300 を発散させる(Stage 73)。</div>
                                    </div>
                                )}
                            </>
                        )}
    ...
    return (
        <div className="h-screen max-h-screen bg-slate-950 text-slate-300 font-sans flex flex-col overflow-hidden selection:bg-slate-800">

            {/* 1. MINIMAL HEADER */}
            <div className="h-14 flex items-center justify-between px-6 bg-slate-950 z-20 flex-shrink-0">
                <div className="flex items-center gap-6">
                    <div className="font-semibold text-slate-100 tracking-tight">OpenWAM <span className="text-slate-500 font-normal">CSL Simulator</span></div>

                    {/* Minimal Tabs */}
                    <div className="flex gap-1 bg-slate-900 p-1 rounded-md">
                        <button
                            onClick={() => setMainTab("builder")}
                            className={`px-4 py-1.5 text-xs font-medium rounded transition-colors ${mainTab === "builder" ? "bg-slate-800 text-slate-100 shadow-sm" : "text-slate-500 hover:text-slate-300"}`}
                        >
                            Builder
                        </button>
                        <button
                            onClick={() => setMainTab("simulation")}
                            className={`px-4 py-1.5 text-xs font-medium rounded transition-colors ${mainTab === "simulation" ? "bg-slate-800 text-slate-100 shadow-sm" : "text-slate-500 hover:text-slate-300"}`}
                        >
                            Simulation
                        </button>
                        <button
                            onClick={() => setMainTab("live")}
                            className={`px-4 py-1.5 text-xs font-medium rounded transition-colors ${mainTab === "live" ? "bg-slate-800 text-slate-100 shadow-sm" : "text-slate-500 hover:text-slate-300"}`}
                        >
                            Live (DS2)
                        </button>
                    </div>
            ...
            <div className="flex-1 relative overflow-hidden">
                ...
                {mainTab === "builder" && (
                    <div className="flex flex-row h-full">
                        ...
                        <div className="order-2 w-[340px] flex-shrink-0 my-6 mr-6 rounded-lg border border-slate-800 bg-slate-950/60 flex flex-col overflow-hidden">
                            {/* Global / Project Header */}
                            <div
                                onClick={() => setSelection({ type: "environment" })}
                                className={`px-4 py-3 border-b border-slate-800 cursor-pointer flex items-center justify-between group transition-colors flex-shrink-0
                                    ${selection?.type === 'environment' ? 'bg-slate-800/50' : 'hover:bg-slate-800/30'}`}
                            >
                                <div>
                                    <div className="text-xs font-bold text-slate-200">Global Settings</div>
                                    <div className="text-[10px] text-slate-500 font-mono">Environment & Simulation</div>
                                </div>
                                <div className={`w-1.5 h-1.5 rounded-full ${selection?.type === 'environment' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]' : 'bg-slate-700 group-hover:bg-slate-500'}`} />
                            </div>
                ...
                {mainTab === "simulation" && (
                    <div className="h-full flex flex-row">
                        ...
                        <div className="flex-1 bg-slate-950 p-6 flex flex-col gap-6">
                            ...
                            <div className="flex-1 border border-slate-800 rounded-lg bg-slate-950/60 relative overflow-hidden flex flex-col">
                                ...
                                <div className="flex-1 relative">
                                    {loading && (
                                        <div className="absolute inset-0 flex items-center justify-center flex-col gap-4">
                                            <div className="w-6 h-6 border-2 border-slate-700 border-t-neutral-100 rounded-full animate-spin"></div>
                                            ...
                                            {progress && progress.total > 0 && (
                                                <div className="w-64 h-1.5 bg-slate-800 rounded overflow-hidden">
                                    ...
                                    {!loading && !runData && !tuneData && resultView !== "validation" && (
                                        <div className="absolute inset-0 flex items-center justify-center flex-col gap-3 text-slate-600 text-sm">
                                            {optimizing ? (
                                                <>
                                    ...
                                    {!loading && runData && resultView === "summary" && (
                                        <div className="absolute inset-0 overflow-auto p-4 flex flex-col gap-6 divide-y divide-slate-800 animate-in fade-in duration-500">
                        ...
                        <div className="w-[340px] flex-shrink-0 my-6 mr-6 rounded-lg border border-slate-800 bg-slate-950/60 p-6 flex flex-col gap-8 z-10 overflow-y-auto">

                            {/* Actions */}
                            <div className="flex flex-col gap-4">
                                <div>
                                    <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Run Simulation</h3>
                                    <button
                                        onClick={() => handleRun("wot_quick")}
                                        disabled={loading || optimizing}
                                        className="w-full py-3 bg-slate-100 hover:bg-white text-black font-semibold rounded text-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                                    >
                                    ...
                                    <button
                                        ...
                                        disabled={loading || optimizing}
                                        className="mt-2 w-full py-2 border border-slate-700 hover:bg-slate-800 text-slate-300 rounded text-xs transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                                    ...
                                    <button
                                        ...
                                        disabled={loading || optimizing}
                                        className="mt-2 w-full py-1.5 border border-slate-800 hover:bg-slate-800 text-slate-400 rounded text-[11px] transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                                    ...
                                    {(loading || optimizing) && (
                                        <button
                                            ...
                                            className="mt-2 w-full py-1.5 border border-red-900/60 text-red-400 hover:bg-red-900/40 rounded text-[11px] font-semibold transition-all flex items-center justify-center gap-2"
                                ...
                                <div className="pt-4 border-t border-slate-800">
                                    <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Tuning <span className="text-slate-600 normal-case">(VANOS · WOT)</span></h3>
                                    {/* preference (§6.C): user picks the goal, the objective stays internal */}
                                    <div className="flex gap-1 bg-slate-900 p-1 rounded-md mb-2">
                                    ...
                                    <button
                                        ...
                                        disabled={loading || optimizing}
                                        className="w-full py-2 border border-slate-700 hover:bg-slate-800 text-slate-200 rounded text-xs font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                            ...
                            <div className="mt-auto">
                                ...
                                <div className="mb-2">
                                    <div className="text-[10px] text-slate-600 mb-1">プリセット</div>
                                    <div className="flex gap-1">
                                        <button onClick={() => applyPreset("v14")}
                                            className={`flex-1 py-1.5 rounded text-[11px] border ${activePreset === "v14" ? "border-emerald-600 text-emerald-400 bg-emerald-900/40" : "border-slate-800 text-slate-400 hover:border-slate-600"}`}
                                            title="Stage 74 実測デジタルツイン(実測吸排気寸法・マフラー内部構造・ヘッド戻り管)">
                                            v14 オーナー実車
                                        </button>
                                        <button onClick={() => applyPreset("legacy")}
                                            className={`flex-1 py-1.5 rounded text-[11px] border ${activePreset === "legacy" ? "border-emerald-600 text-emerald-400 bg-emerald-900/40" : "border-slate-800 text-slate-400 hover:border-slate-600"}`}
                                ...
                                <div className="flex gap-2 text-slate-400">
                                    ...
                                    <button onClick={handleConfigLoad} className="flex-1 py-2 border border-slate-800 hover:border-slate-600 rounded flex items-center justify-center" title="Load config JSON (project)">
                                        <Upload size={14} />
                                    </button>
                                    <button onClick={handleConfigSave} className="flex-1 py-2 border border-slate-800 hover:border-slate-600 rounded flex items-center justify-center" title="Save config JSON (project)">
                                        <Save size={14} />
                                    </button>
                                </div>
                                ...
                                <div className="flex gap-2 mt-2">
                                    <button onClick={handleSheetDownload}
                                        className="flex-1 py-2 border border-slate-800 hover:border-slate-600 rounded flex items-center justify-center gap-1.5 text-[11px] text-slate-300"
                            ...
                            <div className="mt-4 border-t border-slate-800 pt-4">
                                <div className="h-24 font-mono text-[10px] text-slate-600 overflow-y-auto">
