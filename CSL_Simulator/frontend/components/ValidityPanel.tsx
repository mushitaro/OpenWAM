...
const DOT: Record<TrafficStatus, string> = {
    ...
    red: "bg-red-500 shadow-[0_0_8px_rgba(244,63,94,0.5)]",
};
        ...
        <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="text-[11px] font-mono text-slate-400 text-right">
                    ...
                    {overall.any_red_health && <div className="text-red-400 font-bold">health flags!</div>}
                </div>
            </div>

            {/* per-row table */}
            <div className="overflow-auto rounded-lg border border-slate-800 bg-slate-950/50">
                    ...
                    <tbody>
                        {rows.map((row, i) => (
                            <tr key={i} className="border-t border-slate-800 text-slate-300">
                                ...
                                <td className="p-1.5 text-slate-400" title="excl. 3900-5300 model-limit band">{fmt(row.r_ex_limit ?? null)}</td>
                                ...
                                <td className={`p-1.5 ${row.n_gated > 0 ? "text-red-400" : "text-slate-500"}`}>{row.n_gated}</td>
