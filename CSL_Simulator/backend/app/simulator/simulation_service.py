    ...
    async def run_ve_map_generation(self, config, model_name="ve_map_sim",
        ...
        async def run_point(rpm, load_tps):
            async with sem:
                ...
                # NaN gate: PERSISTENT NaN only (any NaN after cycle 5). The
                # measured geometry shows a recoverable cyl-6 exhaust-valve NaN
                # at cycle ~2 that converges balanced (Stage-25 precedent) --
                # rejecting on the transient would gate out healthy cells.
                low = output.lower()
                nan_pos = low.rfind("nan")
                _ve_ends = [m.end() for m in re.finditer(r"Mtrap:[0-9.]+ g", output)]
                _cut = (_ve_ends[29] if len(_ve_ends) >= 30
                        else (_ve_ends[-1] if _ve_ends else 0))
